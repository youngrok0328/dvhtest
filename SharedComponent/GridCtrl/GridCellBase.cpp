    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        else
        {
            CPen lightpen(PS_SOLID, 1, ::GetSysColor(COLOR_3DHIGHLIGHT)),
                darkpen(PS_SOLID, 1, ::GetSysColor(COLOR_3DDKSHADOW)), *pOldPen = pDC->GetCurrentPen();
...
void CGridCellBase::OnClickUp(CPoint PointCellRelative)
{
    UNUSED_ALWAYS(PointCellRelative);
    TRACE2("Mouse Left btn up in cell at x=%i y=%i\n", PointCellRelative.x, PointCellRelative.y);
}
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/)
{
    ...
    CFont *pOldFont = NULL, *pFont = GetFontObject();
    if (pFont)
...
CSize CGridCellBase::GetCellExtent(CDC* pDC)
{
    ...
    if (nImage >= 0)
    {
        ...
        {
            ImageSize = CSize(Info.rcImage.right - Info.rcImage.left, Info.rcImage.bottom - Info.rcImage.top);
...
BOOL CGridCellBase::PrintCell(CDC* pDC, int /*nRow*/, int /*nCol*/, CRect rect)
{
    ...
    if (pGrid->GetShadedPrintOut())
    {
        ...
        if (IsFixed())
            ...
            crBG = (GetBackClr() != CLR_DEFAULT && GetBackClr() != pDefaultCell->GetBackClr()) ? GetBackClr()
                                                                                               : CLR_DEFAULT;
