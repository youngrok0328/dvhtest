...
public:
    CInPlaceDateTime(CWnd* pParentWnd, // parent
...
CGridCellDateTime::CGridCellDateTime(DWORD dwStyle)
    : CGridCell()
{
    Init(dwStyle);
}
...
BOOL CGridCellDateTime::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    m_bEditing = TRUE;

    // CInPlaceDateTime auto-deletes itself
    m_pEditWnd = new CInPlaceDateTime(
        GetGrid(), rect, m_dwStyle | DTS_UPDOWN, nID, nRow, nCol, GetTextClr(), GetBackClr(), GetTime(), nChar);
    return TRUE;
}
...
{
    if (m_pEditWnd)
        ((CInPlaceDateTime*)m_pEditWnd)->EndEdit();
}
...
#ifdef _WIN32_WCE
        CString strTemp;
        strTemp.Format(_T("%02d:%02d:%02d"), m_cTime.GetHour(), m_cTime.GetMinute(), m_cTime.GetSecond());
        SetText(strTemp);
#else
...
#ifdef _WIN32_WCE
        CString strTemp;
        strTemp.Format(_T("%02d/%02d/%02d"), m_cTime.GetMonth(), m_cTime.GetDay(), m_cTime.GetYear());
        SetText(strTemp);
#else
...
CInPlaceDateTime::CInPlaceDateTime(CWnd* pParentWnd, CRect& rect, DWORD dwStyle, UINT nID, int nRow, int nColumn,
    COLORREF crFore, COLORREF crBack, CTime* pcTime, UINT nFirstChar)
{
    m_crForeClr = crFore;
    m_crBackClr = crBack;
    m_nRow = nRow;
    ...
    if (!Create(dwStl, rect, pParentWnd, nID))
    {
        return;
    }
...
{
    if ((nChar == VK_PRIOR || nChar == VK_NEXT || nChar == VK_DOWN || nChar == VK_UP || nChar == VK_RIGHT
            || nChar == VK_LEFT)
