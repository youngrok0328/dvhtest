...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_nSortColumn = -1;
    m_pfnCompare = NULL;
    m_pfnVirtualCompareLess = NULL;
    m_pfnVirtualCompareGreater = NULL;
...
{ // coming from a previous GetLayer (ignored if not same number of column, or the same revision number)
    if (pLayer[0] != LAYER_SIGNATURE)
        return;
    if (pLayer[1] != GetColumnCount())
        return;
...
void CGridCtrl::EraseBkgnd(CDC* pDC)
{
    ...
    CBrush FixedRowColBack(GetDefaultCell(TRUE, TRUE)->GetBackClr()),
        ...
        FixedColBack(GetDefaultCell(FALSE, TRUE)->GetBackClr()), TextBack(GetDefaultCell(FALSE, FALSE)->GetBackClr());
    ...
    {
        CRect CellRect(__max(nFixedColumnWidth, rect.left), __max(nFixedRowHeight, rect.top), rect.right, rect.bottom);
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    int maxVisibleRow = VisCellRange.GetMaxRow(), maxVisibleCol = VisCellRange.GetMaxCol();
...
{
    return (range.GetMinRow() >= 0 && range.GetMinCol() >= 0 && range.GetMaxRow() >= 0 && range.GetMaxCol() >= 0
        && range.GetMaxRow() < m_nRows && range.GetMaxCol() < m_nCols && range.GetMinRow() <= range.GetMaxRow()
...
{
    SetSelectedRange(
        Range.GetMinRow(), Range.GetMinCol(), Range.GetMaxRow(), Range.GetMaxCol(), bForceRepaint, bSelectCells);
}

void CGridCtrl::SetSelectedRange(
    int nMinRow, int nMinCol, int nMaxRow, int nMaxCol, BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells /*=TRUE*/)
{
    ...
    if (nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0 && nMaxRow < m_nRows && nMaxCol < m_nCols
...
// selects columns
void CGridCtrl::SelectColumns(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
    ...
    if (GetSingleColSelection())
        SetSelectedRange(
            GetFixedRowCount(), currentCell.col, GetRowCount() - 1, currentCell.col, bForceRedraw, bSelectCells);
    else
        SetSelectedRange(GetFixedRowCount(), min(m_SelectionStartCell.col, currentCell.col), GetRowCount() - 1,
            max(m_SelectionStartCell.col, currentCell.col), bForceRedraw, bSelectCells);
}

// selects rows
void CGridCtrl::SelectRows(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
    ...
    if (GetSingleRowSelection())
        SetSelectedRange(currentCell.row, Left, currentCell.row, GetColumnCount() - 1, bForceRedraw, bSelectCells);
    else
        SetSelectedRange(min(m_SelectionStartCell.row, currentCell.row), Left,
            __max(m_SelectionStartCell.row, currentCell.row), GetColumnCount() - 1, bForceRedraw, bSelectCells);
}

// selects cells
void CGridCtrl::SelectCells(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    if (!m_bEnableSelection)
        return;
    ...
    SetSelectedRange(min(m_SelectionStartCell.row, row), min(m_SelectionStartCell.col, col),
        __max(m_SelectionStartCell.row, row), __max(m_SelectionStartCell.col, col), bForceRedraw, bSelectCells);
}
...
void CGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCTSTR strText)
{
    ...
    if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0)
    {
        ...
        if (strCurrentText != strText)
        {
            ...
            if (ValidateEdit(nRow, nCol, strText) && SendMessageToParent(nRow, nCol, GVN_ENDLABELEDIT) >= 0)
            {
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
        ...
        while (!strLine.IsEmpty())
        {
            ...
            if (IsValid(TargetCell))
            {
                ...
                SetItemState(
                    TargetCell.row, TargetCell.col, GetItemState(TargetCell.row, TargetCell.col) & ~GVIS_SELECTED);
...
void CGridCtrl::OnBeginDrag()
{
    ...
    {
        SendMessageToParent(
            GetSelectedCellRange().GetTopLeft().row, GetSelectedCellRange().GetTopLeft().col, GVN_BEGINDRAG);
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
...
DROPEFFECT CGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
    // Any text data available for us?
    ...
    {
        Valid = m_LastDragOverCell.row >= 0 && m_LastDragOverCell.row < GetFixedRowCount()
            && m_LastDragOverCell.col >= GetFixedColumnCount();
    }
...
BOOL CGridCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT /*dropEffect*/, CPoint /* point */)
{
    m_MouseMode = MOUSE_NOTHING;
    if (m_CurCol == -1)
    {
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
    ...
    if ((point.y - start.y < m_nResizeCaptureRange && idCurrentCell.row != 0) || endy - point.y < m_nResizeCaptureRange)
    {
        return TRUE;
    }
    else
...
BOOL CGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
    ...
    if ((point.x - start.x < m_nResizeCaptureRange && idCurrentCell.col != 0) || endx - point.x < m_nResizeCaptureRange)
    {
        return TRUE;
    }
    else
...
CCellRange CGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, BOOL bForceRecalculation /*=FALSE*/)
{
    int i;
    CRect rect;
    GetClientRect(rect);
...
CCellRange CGridCtrl::GetVisibleFixedCellRange(LPRECT pRect /*=NULL*/, BOOL bForceRecalculation /*=FALSE*/)
{
    int i;
    CRect rect;
    GetClientRect(rect);
...
int CGridCtrl::InsertColumn(
    LPCTSTR strHeading, UINT nFormat /* = DT_CENTER|DT_VCENTER|DT_SINGLELINE */, int nColumn /* = -1 */)
{
...
CCellID CGridCtrl::GetNextItem(CCellID& cell, int nFlags) const
{
    ...
    else if ((nFlags & GVNI_BELOW) == GVNI_BELOW && (nFlags & GVNI_TORIGHT) == GVNI_TORIGHT)
    ...
    else if ((nFlags & GVNI_ABOVE) == GVNI_ABOVE)
    {
        for (int row = cell.row - 1; row >= GetFixedRowCount(); row--)
        {
            ...
            if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED)
                || (nFlags & GVNI_FOCUSED && nState & GVIS_FOCUSED)
                || (nFlags & GVNI_SELECTED && nState & GVIS_SELECTED)
                || (nFlags & GVNI_READONLY && nState & GVIS_READONLY) || (nFlags & GVNI_FIXED && nState & GVIS_FIXED)
                || (nFlags & GVNI_MODIFIED && nState & GVIS_MODIFIED))
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
    SetSortAscending(bAscending);
    ResetSelectedRange();
...
int CALLBACK CGridCtrl::pfnCellDoubleCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    UNUSED_ALWAYS(lParamSort);
    ...
    const double nValue1 = ::_ttof(pCell1->GetText());
    const double nValue2 = ::_ttof(pCell2->GetText());
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data, int low, int high)
{
    ...
    if (GetVirtualMode())
    {
        ASSERT(m_pfnVirtualCompareLess);
        ASSERT(m_pfnVirtualCompareGreater);

        m_CurCol = m_arColOrder[nCol];
        //       std::stable_sort(m_arRowOrder.begin() + m_nFixedRows, m_arRowOrder.end(), bAscending ? m_pfnVirtualCompareLess : m_pfnVirtualCompareGreater);
        ...
        if (bAscending)
        {
            std::stable_sort(m_arRowOrder.begin() + m_nFixedRows, m_arRowOrder.end(),
                [this, parentWnd](int lhs, int rhs)
                {
                    return m_pfnVirtualCompareLess(parentWnd, m_CurCol, lhs, rhs);
                });
        }
        else
        {
            std::stable_sort(m_arRowOrder.begin() + m_nFixedRows, m_arRowOrder.end(),
                [this, parentWnd](int lhs, int rhs)
                {
                    return m_pfnVirtualCompareGreater(parentWnd, m_CurCol, lhs, rhs);
                });
        }
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
    ...
    */
    ...
    while (rectCell.right > rectWindow.right && rectCell.left > GetFixedColumnWidth()
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
            ...
            if (m_bTitleTips)
            {
                ...
                if (pCell)
                {
                    ...
                    if (!m_bRMouseButtonDown && szTipText && szTipText[0] && !pCell->IsEditing()
                        ...
                        && GetCellRect(idCurrentCell.row, idCurrentCell.col, CellRect))
                    {
                        ...
                        m_TitleTip.Show(TextRect, pCell->GetTipText(), 0, CellRect, pCell->GetFont(),
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        if (IsValid(cell))
        {
            CGridCellBase* pCell2 = GetCell(cell.row, cell.col);
            if (pCell2)
                pCell2->OnDblClick(pointClickedRel);
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_LeftClickDownCell == m_idCurrentCell && !(nFlags & MK_CONTROL) && bInTextArea
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    if (pCell)
    {
        pCell->OnClickUp(GetPointClicked(m_idCurrentCell.row, m_idCurrentCell.col, point));
    }
    ...
    else
    {
        ...
        CGridCellBase* pCell2 = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
        if (pCell2)
            pCell2->OnClick(GetPointClicked(m_idCurrentCell.row, m_idCurrentCell.col, point));
        SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, NM_CLICK);
    }
...
void CGridCtrl::SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight, int nLeftMargin, int nRightMargin,
...
void CGridCtrl::GetPrintMarginInfo(int& nHeaderHeight, int& nFooterHeight, int& nLeftMargin, int& nRightMargin,
    int& nTopMargin, int& nBottomMargin, int& nGap)
{
...
void CGridCtrl::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
    ...
    m_PrinterFont.CreateFont(nFontSize, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, OUT_CHARACTER_PRECIS,
        CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, strFontName);
    ...
    m_nPageHeight
        = m_LogicalPageSize.cy - GetFixedRowHeight() - (m_nHeaderHeight + m_nFooterHeight + 2 * m_nGap) * m_CharSize.cy;
...
void CGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect, CDC* pDC, BOOL& bFirst)
{
...
void CGridCtrl::PrintHeader(CDC* pDC, CPrintInfo* pInfo, int nOption)
{
...
BOOL CGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator /*=_T(',')*/)
{
    ...
    TRY
    {
        ...
        // Get first token
        for (token = buffer, end = buffer; *end && (*end != chSeparator) && (*end != _T('\n')); end++)
            ;
