...
LOGFONT* CTreeColumn::GetFont() const
{
    if (m_plfFont)
...
// returns:  success / fail
{
    ...
    if (aiTotalRows > 0)
    {
        ...
        for (int i1 = 0; i1 < aiTotalRows; i1++)
        {
            ...
            if (!m_pGrid->SetCellType(iCellRow, m_iColumnWithTree, m_pRuntimeClassTreeCell))
            {
                bRC = FALSE;
                break;
            }
...
*****************************************************************************/
int CTreeColumn::InsertTreeBranch(const unsigned char* apucTreeLevelAry, // Tree Level data array --
...
// returns:  nbr of rows deleted
{
    ...
    if (aiRow < m_iFixedRowCount || aiRow >= m_iRowCount)
        return 0;
...
void CTreeColumn::SetTreeUsesImages(BOOL abUseImages) // T=use images
{
    ...
    if (abUseImages && m_pGrid->GetImageList() == NULL)
...
{
    ASSERT(m_pGrid != NULL);
    ASSERT(aiRow >= 0 && aiRow < m_iRowCount);
    ASSERT(m_iColumnWithTree >= 0);
...
void CTreeColumn::TreeDataExpandOneLevel(int aiGridRow)
{
    ...
    ASSERT(aiGridRow >= m_iFixedRowCount && aiGridRow < m_iRowCount);
...
{
    ASSERT(m_pGrid != NULL);
    ASSERT(aiRow >= m_iFixedRowCount && aiRow < m_iRowCount);
    ASSERT(m_iColumnWithTree >= 0);
    ...
    *apbIsLastLeaf = FALSE;
    ...
    for (i1 = iStartPt; i1 < m_iRowCount; i1++)
    {
        ...
        if (!bIsNextShowing && ucLevelNext == ucLevelCurrent + 1)
        {
            *apbIsPlus = TRUE;
        }
    }
    ...
    if (i1 > iStartPt && !*apbIsPlus)
    {
        *apbIsMinus = TRUE;
    }

    if (!bIsNextShowing || ucLevelCurrent > ucLevelNext || i1 >= m_iRowCount) // hit last element
        *apbIsLastLeaf = TRUE;
