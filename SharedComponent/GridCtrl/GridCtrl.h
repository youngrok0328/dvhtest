...
#define GVN_STATECHANGED LVN_ODSTATECHANGED

typedef bool (*PVIRTUALCOMPARE)(CWnd* owner, int col, int row1, int row2);

class __INTEKPLUS_SHARED_GRIDCTRL_API__ CGridCtrl : public CWnd
{
...
public:
    ...
    BOOL Create(
        const RECT& rect, CWnd* parent, UINT nID, DWORD dwStyle = WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE);
...
public:
    ...
    static int CALLBACK pfnCellDoubleCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
...
public:
    ...
    void SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight, int nLeftMargin, int nRightMargin, int nTopMargin,
        int nBottomMargin, int nGap);

    void GetPrintMarginInfo(int& nHeaderHeight, int& nFooterHeight, int& nLeftMargin, int& nRightMargin,
...
public:
    ...
    CCellID GetMergeCellID(CCellID cell) const;
...
protected:
    ...
    virtual void PrintHeader(CDC* pDC, CPrintInfo* pInfo, int nOption = 0);
...
public:
    ...
    //{{
    // 2023.9.30 이현민
    // 아래 함수의 사용법은 IntekPlus.Shared.QpmReportAnalyzer 프로젝트의 RepeatabilityDataDlg 클래스를 참고하세요.
    // 소팅에 std::sort_stable() 함수를 사용하기 때문에, 두 개의 비교 연산을 따로 제공해야 제대로 동작합니다.
    // 단순히 Less 연산의 결과를 반전시키면 안됩니다.
    //}}
    void SetVirtualCompare(PVIRTUALCOMPARE predLess, PVIRTUALCOMPARE predGreater)
    {
        m_pfnVirtualCompareLess = predLess;
        m_pfnVirtualCompareGreater = predGreater;
    }
    int m_CurCol;
    void AllowReorderColumn(bool b = true)
    {
        m_AllowReorderColumn = b;
    }
    ...
    void SetLayer(int*
            pLayer); // coming from a previous GetLayer (ignored if not same number of column, or the same revision number)
    void ForceQuitFocusOnTab(bool b = true)
    {
        m_QuitFocusOnTab = b;
    } // use only if GetParent() is a CDialog
    void AllowSelectRowInFixedCol(bool b = true)
    {
        m_AllowSelectRowInFixedCol = b;
    } //
    //    allow acces?
...
private:
    ...
    PVIRTUALCOMPARE m_pfnVirtualCompareLess;
    PVIRTUALCOMPARE m_pfnVirtualCompareGreater;
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        GV_DISPINFO gvdi;
        gvdi.hdr.hwndFrom = m_hWnd;
        gvdi.hdr.idFrom = GetDlgCtrlID();
        gvdi.hdr.code = GVN_GETDISPINFO;
        gvdi.item.row = nRow;
        gvdi.item.col = nCol;
