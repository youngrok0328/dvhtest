...
class __INTEKPLUS_SHARED_GRIDCTRL_API__ CGridTreeCellBase : public CGridCellBase
{
...
public:
    ...
    {
        ASSERT(pTreeColumn != NULL);
        m_pTreeColumn = pTreeColumn;
    }
    CTreeColumn* GetTreeColumn()
    {
        return m_pTreeColumn;
    }
    ...
    void SetLevel(UCHAR ucLevel)
    {
        ...
        m_ucLevelAndHide |= ucLevel;
    }
    UCHAR GetLevel()
    {
        return UCHAR(m_ucLevelAndHide & 0x7F);
    }
    ...
    BOOL IsViewable()
    {
        return (m_ucLevelAndHide & 0x80) == 0x80;
    }

    // used only when communicating with CGridTreeCtrl
    void SetLevelAndHide(UCHAR ucLevelAndHide)
    {
        m_ucLevelAndHide = ucLevelAndHide;
    }
    UCHAR GetLevelAndHide()
    {
        return m_ucLevelAndHide;
    }
...
public:
    ...
    {
        ASSERT(nImage <= USHRT_MAX);
        m_sImage = (short)nImage;
    }
    ...
    virtual void SetData(LPARAM lParam)
    {
        if (m_pTreeColumn != NULL)
            m_pTreeColumn->SetParam(lParam);
    }
    virtual void SetFormat(DWORD nFormat)
    {
        if (m_pTreeColumn != NULL)
            m_pTreeColumn->SetFormat(nFormat);
    }
    virtual void SetTextClr(COLORREF clr)
    {
        if (m_pTreeColumn != NULL)
            m_pTreeColumn->SetFgClr(clr);
    }
    virtual void SetBackClr(COLORREF clr)
    {
        if (m_pTreeColumn != NULL)
            m_pTreeColumn->SetBkClr(clr);
    }
    ...
    virtual CPoint GetCoords() const
    {
...
public:
    virtual BOOL Edit(
        int /* nRow */, int /* nCol */, CRect /* rect */, CPoint /* point */, UINT /* nID */, UINT /* nChar */)
    {
        ASSERT(FALSE);
        return FALSE;
    }
...
protected:
    ...
    UCHAR m_ucLevelAndHide{0x80}; // high-order bit tells if cell is viewable or hidden;
        //  rest tell the tree level of this cell
    short m_sImage{-1}; // Index of the list view item뭩 icon
    int m_iRow{-1}; // Each cell needs to know position relative to neighbor cells
    int m_iCol{-1};
