...
#include "GridSortingCellCheck.h"
...
//
IMPLEMENT_DYNCREATE(CGridSortingCellCheck, CGridCellCheck)

CGridSortingCellCheck::CGridSortingCellCheck()
    : CGridCellCheck()
{
    m_bEnabled = TRUE;
}

BOOL CGridSortingCellCheck::Edit(int nRow, int nCol, CRect /* rect */, CPoint /* point */, UINT /* nID */, UINT nChar)
{
    if (m_bEnabled)
    {
        if (nChar == VK_SPACE)
...
LRESULT CGridSortingCellCheck::SendMessageToParent(int nRow, int nCol, int /*nMessage*/)
{
    CGridCtrl* pGrid = GetGrid();
    if (pGrid)
    {
...
void CGridSortingCellCheck::EndEdit()
{
}

BOOL CGridSortingCellCheck::Enable(BOOL bEnable /*=TRUE*/)
{
...
BOOL CGridSortingCellCheck::GetEnabled()
{
    return m_bEnabled;
}

// Returns the dimensions and placement of the checkbox in client coords.
CRect CGridSortingCellCheck::GetCheckPlacement()
{
...
BOOL CGridSortingCellCheck::Draw(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (m_bEnabled)
        pDC->DrawFrameControl(
            GetCheckPlacement(), DFC_BUTTON, (m_bChecked) ? DFCS_BUTTONCHECK | DFCS_CHECKED : DFCS_BUTTONCHECK);
    else
...
void CGridSortingCellCheck::OnClick(CPoint tt /*PointCellRelative*/)
{
...
void CGridSortingCellCheck::SetReadOnly(BOOL bmakereadonly)
{
    DWORD dwcurstate = GetState();

    if (bmakereadonly)
    {
        dwcurstate &= GVIS_READONLY;
    }
    else
    {
        dwcurstate &= ~GVIS_READONLY;
