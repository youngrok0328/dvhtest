...
public:
    CInPlaceList(CWnd* pParentWnd, // parent
...
CInPlaceList::CInPlaceList(CWnd* pParentWnd, CRect& rect, DWORD dwStyle, UINT nID, int nRow, int nColumn,
    COLORREF crFore, COLORREF crBack, CStringArray& Items, CString sInitText, UINT nFirstChar)
{
    ...
    */
    ...
    if (!Create(dwComboStyle, rect, pParentWnd, nID))
        return;
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    ...
    m_pEditWnd = new CInPlaceList(
        GetGrid(), rect, GetStyle(), nID, nRow, nCol, GetTextClr(), GetBackClr(), m_Strings, GetText(), nChar);
...
void CGridCellCombo::EndEdit()
{
    ...
    SetText(strTempText);
    for (int i = 0; i < (long)m_Strings.GetSize(); i++)
    {
        if (strTempText == m_Strings[i])
        {
            m_nSelect = i;
        }
    }
...
void CGridCellCombo::SetCurSel(int nCur)
{
    m_nSelect = nCur;

    if (m_Strings.GetSize() > nCur)
        SetText(m_Strings[nCur]);
    else
        ASSERT("CGridCellCombo::SizeOver");
}
...
void CGridCellCombo::AddString(CString str)
{
    m_Strings.Add(str);
}

void CGridCellCombo::RemoveAll()
{
    m_Strings.RemoveAll();
}
