...
//
class __INTEKPLUS_SHARED_GRIDCTRL_API__ CCellID
{
    // Attributes
...
public:
    explicit CCellID(int nRow = -1, int nCol = -1)
        : row(nRow)
        , col(nCol)
    {
    }
...
class __INTEKPLUS_SHARED_GRIDCTRL_API__ CCellRange
{
public:
...
{
    return ((m_nMinRow == rhs.m_nMinRow) && (m_nMinCol == rhs.m_nMinCol) && (m_nMaxRow == rhs.m_nMaxRow)
...
{
    return (m_nMinRow >= 0 && m_nMinCol >= 0 && m_nMaxRow >= 0 && m_nMaxCol >= 0 && m_nMinRow <= m_nMaxRow
        && m_nMinCol <= m_nMaxCol);
}
...
{
    return CCellRange(max(m_nMinRow, rhs.m_nMinRow), max(m_nMinCol, rhs.m_nMinCol), min(m_nMaxRow, rhs.m_nMaxRow),
