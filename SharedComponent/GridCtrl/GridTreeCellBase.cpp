...
BOOL CGridTreeCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd /*=TRUE*/)
{
    CRect RectCell(rect);
...
*****************************************************************************/
BOOL CGridTreeCellBase::DrawTreeCell(CDC* pDC, int nRow, int nCol, CRect* prect, BOOL /* bEraseBkgnd */)
{
    ...
    if (prect->Width() <= 0 || prect->Height() <= 0) // prevents imagelist item from drawing even
...
void CGridTreeCellBase::OnDblClick(CPoint PointCellRelative)
{
    ...
    if (GetState() & GVIS_READONLY || !m_pTreeColumn->GetGrid()->IsEditable())
    {
        if (!TreeHitPlusMinus(PointCellRelative)) // relative coordinates of mouse click
        {
            // returns:  T=hit a plus or minus;  F=Missed it or tree cell has no plus or minus
...
/*****************************************************************************
For mouse hit test want to know if user clicked on the "+_T(" / ")-" box. Also
for drawing the box
...
// returns:  T=hit a plus or minus;  F=Missed it or tree cell has no plus or minus
{
    ...
    if (aPointMouse.x >= RectBox.left && aPointMouse.x <= RectBox.right && aPointMouse.y >= RectBox.top
...
UINT CGridTreeCellBase::GetMargin() const
{
...
void CGridTreeCellBase::SetMargin(UINT nMargin)
{
    if (m_pTreeColumn != NULL)
        m_pTreeColumn->GetGrid()->SetDefCellMargin(nMargin);
}
    ...
    CRect aRect) // coordinates of bounding cell rectangle
{
    ...
    int aYCenter = aRect.top + ((aRect.bottom - aRect.top) / 2);
    ...
    if (m_iRow >= m_pTreeColumn->GetRowCount() - 1)
    {
        ...
        if (TryMoveTo(aRect, apDC, iXCenter, aRect.top))
            if (TryLineTo(aRect, apDC, iXCenter, aYCenter))
                TryLineTo(aRect, apDC, iXCenter + 16, aYCenter);
    ...
    BOOL bIsBranch = m_pTreeColumn->TreeCellHasPlusMinus(m_iRow, // row of Cell to check
        &bIsPlus, // returns:  T=Is a plus
        &bIsMinus, // returns:  T=Is a minus
        &bIsLastLeaf); // returns:  T=Is Last Leaf
    ...
    if (bIsBranch)
    {
        ...
        {
            apDC->Rectangle(RectBox.left, RectBox.top, RectBox.right, RectBox.bottom);
        }
    ...
    else if (ucLevelCurrent > 1)
    {
        ...
        CGridTreeCellBase* pGridTreeCellBase
            = (CGridTreeCellBase*)pGrid->GetCell(m_iRow + 1, m_pTreeColumn->GetColumnWithTree());
        ...
        // draw -
        if (TryMoveTo(aRect, apDC, iXCenter - m_pTreeColumn->GetDefTreeIndent(), aYCenter))
            TryLineTo(aRect, apDC, iXCenter, aYCenter);
    }
    ...
    for (int i1 = ucLevelCurrent - 1; i1 > 1; i1--)
    {
        ...
        {
            CGridTreeCellBase* pGridTreeCellBase
                = (CGridTreeCellBase*)pGrid->GetCell(i2, m_pTreeColumn->GetColumnWithTree());
            if (pGridTreeCellBase == NULL)
                return;
