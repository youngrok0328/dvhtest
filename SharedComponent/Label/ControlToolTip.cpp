...
CSize CControlToolTip::GetMultiLineTextExtent(CClientDC* pDC, LPCTSTR lpszText)
{
    ...
    _tcscpy_s(sz, 1024, lpszText);

    TCHAR* psz_next = NULL;
    TCHAR* psz = _tcstok_s(sz, _T("\n"), &psz_next);
...
{
    int iLines = 1;
    CString str = lpszText ? CString(lpszText) : m_strText;
...
BOOL CControlToolTip::Attach(CWnd* pControl)
{
    ...
    BOOL bRet = CreateEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST, CONTROLTOOLTIP_CLASSNAME, NULL, WS_BORDER | WS_POPUP, 0, 0,
        0, 0, NULL, NULL, NULL);
...
void CControlToolTip::SetText(LPCTSTR lpszText)
{
    m_strText = lpszText;
