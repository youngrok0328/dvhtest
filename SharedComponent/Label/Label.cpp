...
CLabel::~CLabel()
{
    ...
    if (m_hTabBorderBrush)
        ::DeleteObject(m_hTabBorderBrush);
...
void CLabel::OnPaint()
{
    ...
    if (!m_bTransparent)
    {
        if (m_bBitmap)
        {
            DrawBitmap(pDCMem, &rc);
        }
        else if (m_fillmode == Normal)
        {
            CBrush br, br2;
            ...
            // Tab Label이면서 선택되어 졌으면..
            if (m_bTabLabel && m_bTabSelected)
            {
                if (m_hTabBorderBrush != NULL)
                    br2.Attach(m_hTabBorderBrush);
                else
                    br2.Attach(m_hwndBrush);
                rc2 = CRect(
                    rc.left + m_nTabBorderWidth, rc.top + m_nTabBorderWidth, rc.right - m_nTabBorderWidth, rc.bottom);
                pDCMem->FillRect(rc2, &br2);
            }
...
CLabel& CLabel::SetTabCtrl(BOOL bSet)
{
    m_bTabLabel = bSet;
    return *this;
}

CLabel& CLabel::SetTabSelected(BOOL bSet)
{
    m_bTabSelected = bSet;
    return *this;
}

CLabel& CLabel::SetTabBorderWidth(int nWidth)
{
    m_nTabBorderWidth = nWidth;
    return *this;
}
...
CLabel& CLabel::SetTabClientColor(COLORREF crBordergnd)
{
    if (m_hTabBorderBrush)
        ::DeleteObject(m_hTabBorderBrush);

    m_hTabBorderBrush = ::CreateSolidBrush(crBordergnd);
...
void CLabel::SetFillByDegree(COLORREF crBkgnd, COLORREF crBkgndHigh, FillDirection nDirection)
{
    m_crLoColor = crBkgnd;
    m_crHiColor = crBkgndHigh;
...
//////////////////////////////////////////////////////////////////////////
//
// Function:		CLabel::FillByDegrees
//
// Description:		Internal help function to fill background by degrees
//
// INPUTS:
//
// RETURNS:
...
void CLabel::FillByDegrees(CDC* pDC, CRect* pRect, COLORREF crStart, COLORREF crEnd)
{
    ...
    if (m_nFillDirection == Horizontal)
        ...
        pDC->Rectangle(pRect->left, (int)(pRect->top * (1 - (double)m_nFillCurrentStep / m_nFillFullStep)),
    ...
    if (m_nFillDirection == Horizontal)
        ...
        pDC->Rectangle(pRect->left, pRect->top, pRect->right,
            (int)(pRect->bottom * (1 - (double)m_nFillCurrentStep / m_nFillFullStep)));
...
BOOL CLabel::SetBitmap(HBITMAP hBitmap, BOOL bFit, COLORREF rgbTransparent)
{
    UNREFERENCED_PARAMETER(rgbTransparent);
...
BOOL CLabel::SetBitmap(UINT nIDResource, BOOL bFit, COLORREF rgbTransparent)
{
    ...
    HBITMAP hBmp = (HBITMAP)::LoadImage(
        AfxGetInstanceHandle(), MAKEINTRESOURCE(nIDResource), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
    if (!hBmp)
        return FALSE;
    return CLabel::SetBitmap(hBmp, bFit, rgbTransparent);
}

BOOL CLabel::SetBitmap(LPCTSTR lpszResourceName, BOOL bFit, COLORREF rgbTransparent)
{
    m_nResourceID = -1;
    m_strResourceName = lpszResourceName;

    HBITMAP hBmp = (HBITMAP)::LoadImage(AfxGetInstanceHandle(), lpszResourceName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    if (!hBmp)
        return FALSE;
    return CLabel::SetBitmap(hBmp, bFit, rgbTransparent);
}

void CLabel::DrawBitmap(CDC* pDCMem, CRect* pRect)
{
    ...
    HBITMAP oldBitmap = (HBITMAP)::SelectObject(dcMem.m_hDC, m_hBitmap);

    pDCMem->StretchBlt(rect.left, rect.top, rect.Width(), rect.Height(), &dcMem, 0, 0, m_bmInfo.bmWidth - 1,
        m_bmInfo.bmHeight - 1, SRCCOPY);

    ::SelectObject(dcMem.m_hDC, oldBitmap);
}
