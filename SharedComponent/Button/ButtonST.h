...
#ifndef BTNST_AUTO_GRAY_ID
#define BTNST_AUTO_GRAY_ID (-1L)
#define BTNST_AUTO_GRAY_ICON (HICON)(BTNST_AUTO_GRAY_ID)
#define BTNST_AUTO_GRAY_BITMAP (HBITMAP)(BTNST_AUTO_GRAY_ID)
#endif
// Dummy identifier for 15% darker icon
#ifndef BTNST_AUTO_DARKER_ID
#define BTNST_AUTO_DARKER_ID (-2L)
#define BTNST_AUTO_DARKER_ICON (HICON)(BTNST_AUTO_DARKER_ID)
#define BTNST_AUTO_DARKER_BITMAP (HBITMAP)(BTNST_AUTO_DARKER_ID)
#endif

class __INTEKPLUS_SHARED_BUTTON_API__ CButtonST : public CButton
{
public:
    CButtonST();
    ~CButtonST() override;
    ...
    enum
    {
        ...
        BTNST_MAX_COLORS,
        // 이 밑은 항상 같은 색을 지정하고 싶을때 사용한다.
        BTNST_COLOR_BK, // 모든 조건에서 똑같은 배경 색깔을 적용한다.
    };
...
public:
    // Added by CHLee
    void SetJogButton(UINT nDownMsg, UINT nUpMsg);
    DWORD SetAutoRepeat(BOOL bSet, DWORD dwMilliseconds);
    ...
    DWORD SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut = NULL, int nCxDesiredOut = 0,
        int nCyDesiredOut = 0);
    ...
    void SizeToContent();
    // KWAK 폰트 사이즈 조절을 위해 추가
    void SetFontSize(int nSize);
    void SetFontBold(BOOL bBold);

#ifdef BTNST_USE_BCMENU
    DWORD SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle = TRUE, UINT nToolbarID = NULL,
        CSize sizeToolbarIcon = CSize(16, 16), COLORREF crToolbarBk = RGB(255, 0, 255), BOOL bRepaint = TRUE);
#else
