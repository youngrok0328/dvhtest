...
BOOL CCaptionButton::ButtonHitTest(CPoint point)
{
    if (m_bActive && m_bEnable)
    {
        ...
        if (::GetMenu(m_hWndCap))
        {
            hitPoint.y += GetSystemMetrics(SM_CYMENU);
        }
...
void CCaptionButton::ChangeButtonState()
{
    ...
    if (m_bBtnChecked == FALSE)
    {
        ::SendMessage(m_hWndCap, WMU_HITCAPBUTTON, (WPARAM)m_uiBtnCount, (LPARAM)0);
    }
}
...
BOOL CCaptionButton::PaintBmpCheckBtn(UINT uiBtnCount /*= 1*/)
{
    ...
    HBITMAP hBmp = (HBITMAP)::LoadImage(
        m_hContainerModule, MAKEINTRESOURCE(BmpID), IMAGE_BITMAP, m_rc.Width(), m_rc.Height(), LR_LOADMAP3DCOLORS);
    VERIFY(hBmp);
    ...
    dc->StretchBlt(m_rc.left, m_rc.top, m_rc.Width(), m_rc.Height(), &dcMemo, 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
...
void CCaptionButton::CalcCapRect(CRect* pCapRect)
{
    ...
    if (::GetMenu(m_hWndCap))
    {
        pCapRect->top += sizeCaption.cy;
        pCapRect->bottom += sizeCaption.cy;
    }
...
BOOL CCaptionButton::isWindowActive(BOOL& bActive)
{
    ...
    return static_cast<BOOL>(::DefWindowProc(m_hWndCap, WM_NCACTIVATE, bActive, 0L));
}
...
void CCaptionButton::SetBmpID(HINSTANCE hContainerModule,
    UINT iBtnNo, // No. of the button (R-L order)
    UINT ID_Bmp_up, UINT ID_Bmp_down /*= 0*/)
{
    m_hContainerModule = hContainerModule;
...
BOOL CCaptionButton::HookDlg()
{
    ...
    s_oldWndProc = (WNDPROC)::SetWindowLongPtr(
        CCaptionButton::m_hWndCap, GWLP_WNDPROC, (DWORD_PTR)CCaptionButton::HookedWindowProc);
    ...
    switch (message)
    {
        ...
        case WM_NCLBUTTONDOWN:
            ...
            for (UINT n = 1; n <= iMC; n++)
            {
                ...
                if (pBtn)
                {
                    if (pBtn->ButtonHitTest((CPoint)lParam))
                    {
                        pBtn->ChangeButtonState();
                        return 0;
                    }
                }
            }
