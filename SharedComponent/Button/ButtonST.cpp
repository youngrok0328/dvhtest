...
BOOL CButtonST::OnClicked()
{
    ...
    else
    {
...
#endif
        {
            ...
            DWORD dwRetValue
                = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD,
                    rWnd.left, rWnd.bottom, this, NULL);
#else
            ...
            DWORD dwRetValue = ::TrackPopupMenuEx(hSubMenu,
                TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom,
                m_hParentWndMenu, NULL);
#endif
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
    ...
    CRect rImage;
    PrepareImageRect(
        bHasTitle, rpItem, rpCaption, bIsPressed, m_csIcons[byIndex].dwWidth, m_csIcons[byIndex].dwHeight, &rImage);
...
void CButtonST::DrawTheBitmap(
    CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
    BYTE byIndex = 0;
    ...
    CRect rImage;
    PrepareImageRect(
        bHasTitle, rpItem, rpCaption, bIsPressed, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, &rImage);
    ...
    if (bIsDisabled && m_bShowDisabledBitmap)
    {
        ...
        pDC->DrawState(CPoint(rImage.left /*+1*/, rImage.top),
            CSize(m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight), hBitmap, DST_BITMAP | DSS_DISABLED);
...
void CButtonST::DrawTheText(
    CDC* pDC, LPCTSTR lpszText, RECT* /*rpItem*/, CRect* rpCaption, BOOL /*bIsPressed*/, BOOL /*bIsDisabled*/)
{
    ...
    CFont font, *oldfont;
    CString strFontName;
    pDC->GetTextFace(strFontName);
    font.CreateFont(m_nFontSize * 2, 0, 0, 0, m_dwBold, FALSE, FALSE, 0, 0, 0, 0, 0, 0, strFontName);
    ...
    rpCaption->OffsetRect(
        (centerRect.Width() - rpCaption->Width()) / 2, (centerRect.Height() - rpCaption->Height()) / 2);
    /* RFU
...
DWORD CButtonST::SetIcon(
    int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
    HICON hIconIn = NULL;
    HICON hIconOut = NULL;
    ...
    switch (nIconOut)
    {
        case NULL:
            break;
        case BTNST_AUTO_GRAY_ID:
            hIconOut = BTNST_AUTO_GRAY_ICON;
            break;
        case BTNST_AUTO_DARKER_ID:
            hIconOut = BTNST_AUTO_DARKER_ICON;
            break;
        default:
            hIconOut = (HICON)::LoadImage(
                hInstResource, MAKEINTRESOURCE(nIconOut), IMAGE_ICON, nCxDesiredOut, nCyDesiredOut, 0);
            break;
    } // switch
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
    ...
    if (hIconIn)
    {
        ...
        if (hIconOut)
        {
            if (hIconOut == BTNST_AUTO_GRAY_ICON)
            {
                hIconOut = CreateGrayscaleIcon(hIconIn);
            }
            else if (hIconOut == BTNST_AUTO_DARKER_ICON)
            {
...
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
    ...
    switch (nBitmapOut)
    {
        case NULL:
            break;
        case BTNST_AUTO_GRAY_ID:
            hBitmapOut = BTNST_AUTO_GRAY_BITMAP;
            break;
        case BTNST_AUTO_DARKER_ID:
            hBitmapOut = BTNST_AUTO_DARKER_BITMAP;
            break;
        default:
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
    ...
    if (hBitmapIn)
    {
        ...
        if (hBitmapOut == BTNST_AUTO_GRAY_BITMAP)
        {
            hBitmapOut
                = CreateGrayscaleBitmap(hBitmapIn, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight, crTransColorIn);
            m_csBitmaps[1].hBitmap = hBitmapOut;
            crTransColorOut = crTransColorIn;
        }
        else if (hBitmapOut == BTNST_AUTO_DARKER_BITMAP)
        {
...
//			Invalid color index.
//
// 090501 항상 같은 배경색을 지정하고 싶어서 수정함
DWORD CButtonST::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
    ...
    else
    {
        ...
        {
            case BTNST_COLOR_BK:
                m_crColors[BTNST_COLOR_BK_IN] = crColor;
                m_crColors[BTNST_COLOR_BK_OUT] = crColor;
                m_crColors[BTNST_COLOR_BK_FOCUS] = crColor;
                break;
            default:
...
DWORD CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
    ...
    byGreen = 0; //GetGValue(m_crColors[byColorIndex]);
...
//
#ifdef BTNST_USE_BCMENU
DWORD CButtonST::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon,
    COLORREF crToolbarBk, BOOL bRepaint)
{
    BOOL bRetValue = FALSE;
...
void CButtonST::SetJogButton(UINT nDownMsg, UINT nUpMsg)
{
    m_bJogBtn = TRUE;
...
void CButtonST::SetFontSize(int nSize)
{
    m_nFontSize = nSize;
}

void CButtonST::SetFontBold(BOOL bBold)
{
    m_dwBold = bBold ? FW_BOLD : FW_NORMAL;
}
