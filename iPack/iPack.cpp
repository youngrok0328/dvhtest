...
#if (IPVMLIB_MAJOR_VERSION == 9) && (IPVMLIB_MINOR_VERSION == 1) && (IPVMLIB_REVISION == 0)
#else
...
BEGIN_MESSAGE_MAP(CiPackApp, CWinAppEx)
...
CiPackApp::CiPackApp()
{
    static CMutex mutex(FALSE, _T("iPack singleton"), nullptr);

    if (mutex.Lock(0) == FALSE)
    {
        ::AfxMessageBox(_T("iPack.exe is running already!!"), MB_ICONWARNING | MB_OK);
        ::exit(0);
    }

    mutex.Lock();

    ::AfxMessageBox(_T("iPack.exe Debug!!"), MB_ICONWARNING | MB_OK);
    ...
    {
        ::AfxMessageBox(
            _T("Current system option is for TR vision.\nPlease check the system option."), MB_ICONWARNING | MB_OK);
        ::exit(0);
    }
    ...
    new BYTE[100]; // 메모리 누수 탐지 가능 여부 체크용.
}
...
BOOL CiPackApp::InitInstance()
{
    ...
    pDocTemplate = new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CiPackDoc),
        RUNTIME_CLASS(CMainFrame), // 주 SDI 프레임 창입니다.
        RUNTIME_CLASS(CiPackView));
    if (!pDocTemplate)
        return FALSE;
...
BOOL CiPackApp::PreTranslateMessage(MSG* pMsg)
{
