...
#include "MemDC.h"
#include "GridCtrl.h"
//#include "../../CommonUtilityModules/dPI_MsgDialog/SimpleMessage.h"

//#include "C:\download\gridctrl_merge\gridctrl_demo224\GridCtrlDemo.h"
...
//        SimpleMessage(_T("OLE initialization failed. Make sure that the OLE libraries are the correct version"));
#endif
    ...
    SetPrintMarginInfo(8, 4, 4, 4, 1, 1, 1);
#endif
}
...
//    HINSTANCE hInst = AfxGetInstanceHandle();
	...
	// 이 코드로 해 놓지 않으면
	// 영문 리소스를 사용할 때에 이 컨트롤을 사용하는
	// 다이얼로그를 띄우지 못하는 문제가 있음.
	// 2005.05.26 이현민
...
// Added by KiteFly
LRESULT CGridCtrl::OnImeChar(WPARAM wCharCode, LPARAM lParam)
{
    // EFW - BUG FIX
...
LRESULT CGridCtrl::OnCopy(WPARAM, LPARAM)
{
	OnEditCopy();
	return 0;
}
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	::SendMessage(pParentWnd->GetSafeHwnd(),WM_VSCROLL,NULL,NULL);
    EndEditing();
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
        ...
        CString strNext = (nIndex < 0)? strEmpty : strLine.Mid(nIndex + 1);
        ...
        while (!strLine.IsEmpty())
        {
            ...
            strLine = (nLineIndex >= 0)? strLine.Mid(nLineIndex + 1) : strEmpty;
...
// Sorts on a given column using the cell num
BOOL CGridCtrl::SortNumItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
    SetSortColumn(nCol);
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    pointClickedRel = GetPointClicked( m_idCurrentCell.row, m_idCurrentCell.col, point);
     ...
     CGridCellBase* pCell = GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
    if (pCell)
        pCell->OnClickUp(GetPointClicked( m_LeftClickDownCell.row, m_LeftClickDownCell.col, point));
    ...
    {
        pCell = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
        if (pCell)
...
void CGridCtrl::Print(int nOption,CPrintDialog* pPrntDialog /*=NULL*/)
{
    CDC dc;
...
//           return;
	...
	if(dc ==NULL)
	{
		AfxMessageBox(_T("Printer is not Installed or not Worked!!!!   "));
		return;
	}
    dc.m_bPrinting = TRUE;
    ...
    for (UINT page = Info.GetMinPage(); page <= Info.GetMaxPage() && bPrintingOK; page++)
    {
        ...
        OnPrint(&dc, &Info,nOption);                    // Print page
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo,int nOption)
{
    if (!pDC || !pInfo)
        return;
                                   ...
                                   CDC *pDC, BOOL& bFirst)
{
   ...
   for (int col =nStartColumn; col < nStopColumn; col++)
   {
      ...
      if (pCell)
	  {
			//Used for merge cells
			//by Huang Wei
			int printRow=m_nCurrPrintRow;
			if(!pCell->IsMerged())
			{
				if(!pCell->IsMergeWithOthers())
				{
                    pCell->PrintCell(pDC, printRow, col, rect);
				}
			}
			else
			{
...
void CGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo,int nOption)
{
	...
	switch(nOption)
	{
		case 0:
			...
			break;
		case 1:
			strCenter.Format(_T("Manual Inspect Report"));
			break;
		case 2:
			strCenter.Format(_T("GR&R Data Sheet"));
			break;
	}
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    CString strLeft;
    strLeft.Format(_T("Paged %d of %d"), pInfo->m_nCurPage, pInfo->GetMaxPage() );
    ...
    CString strCenter;
	strCenter.Format(_T("www.intekplus.com"));
    CRect rc(pInfo->m_rectDraw);
...
//        SimpleMessage(_T("Unable to save grid list"));
        return FALSE;
    }
    END_CATCH
...
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	...
	//---->2006.08.29 복기 TrackOut 만들면서 필요해서 추가.
	else if(str == "" && strCurrentText == "")
	{
