...
class VisionInspectionKOZ2D;
class VisionInspectionKOZ2DPara;
struct ProcessingDlgInfo;

//HDR_6_________________________________ Header body
//
class CDlgVisionInspectionKOZ2D : public CDialog
{
    DECLARE_DYNAMIC(CDlgVisionInspectionKOZ2D)

public:
    CDlgVisionInspectionKOZ2D(const ProcessingDlgInfo& procDlgInfo, VisionInspectionKOZ2D* pVisionInsp,
        CWnd* pParent = NULL); // 표준 생성자입니다.
    virtual ~CDlgVisionInspectionKOZ2D();
...
public:
    ...
    VisionInspectionKOZ2D* m_pVisionInsp;
    VisionInspectionKOZ2DPara* m_pVisionPara;
...
public:
    ...
    void DrawNotchEdgeSearchLine();

    void ClickedButtonImageCombine();
    void ClickedButtonNotchImageCombine();
    void ClickedButtonChippingImageCombine();
    void ShowImage(long nFrameIdx);
    void ShowChippingMaskImage();
    void ShowChippingThresholdImage();
