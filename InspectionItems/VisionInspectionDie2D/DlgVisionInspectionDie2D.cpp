...
//
enum enumDieAlignParamID
{
    DIE_ALIGN_PARAM_START = 1,
    DIE_ALIGN_PARAM_IMAGECOMBINE = DIE_ALIGN_PARAM_START,
    DIE_ALIGN_PARAM_SEARCH_DIRECTION,
    DIE_ALIGN_PARAM_EDGE_DIRECTION,
    DIE_ALIGN_PARAM_EDGE_DETECT_MODE,
    DIE_ALIGN_PARAM_FIRST_EDGE_VALUE,
    DIE_ALIGN_PARAM_SEARCH_LENGTH,
    DIE_ALIGN_PARAM_SEARCH_WIDTH,
    DIE_ALIGN_PARAM_SEARCH_GAP,
    DIE_ALIGN_PARAM_END,
};
...
IMPLEMENT_DYNAMIC(CDlgVisionInspectionDie2D, CDialog)

CDlgVisionInspectionDie2D::CDlgVisionInspectionDie2D(
    const ProcessingDlgInfo& procDlgInfo, VisionInspectionDie2D* pVisionInsp, CWnd* pParent /*=NULL*/)
    : CDialog(CDlgVisionInspectionDie2D::IDD, pParent)
...
void CDlgVisionInspectionDie2D::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgVisionInspectionDie2D, CDialog)
//ON_WM_CREATE()
ON_WM_CLOSE()
ON_WM_DESTROY()
ON_BN_CLICKED(IDC_BUTTON_INSPECT, &CDlgVisionInspectionDie2D::OnBnClickedButtonInspect)
ON_NOTIFY(TCN_SELCHANGE, IDC_TAB_RESULT, &CDlgVisionInspectionDie2D::OnTcnSelchangeTabResult)
ON_BN_CLICKED(IDC_BUTTON_CLOSE, &CDlgVisionInspectionDie2D::OnBnClickedButtonClose)
ON_CBN_SELCHANGE(IDC_COMBO_CURRPANE, &CDlgVisionInspectionDie2D::OnCbnSelchangeComboCurrpane)
ON_BN_CLICKED(IDC_BUTTON_PREIMAGE, &CDlgVisionInspectionDie2D::OnBnClickedButtonPreimage)
ON_BN_CLICKED(IDC_BUTTON_NEXTIMAGE, &CDlgVisionInspectionDie2D::OnBnClickedButtonNextimage)
ON_MESSAGE(UM_ROI_CHANGED, &CDlgVisionInspectionDie2D::OnRoiChanged)
ON_MESSAGE(UM_SHOW_DETAIL_RESULT, &CDlgVisionInspectionDie2D::OnShowDetailResult)
ON_MESSAGE(XTPWM_PROPERTYGRID_NOTIFY, &CDlgVisionInspectionDie2D::OnGridNotify)
ON_MESSAGE(UM_IMAGE_LOT_VIEW_PANE_CHANGED, &CDlgVisionInspectionDie2D::OnImageLotViewPaneSelChanged)
END_MESSAGE_MAP()

BOOL CDlgVisionInspectionDie2D::OnInitDialog()
{
...
int CDlgVisionInspectionDie2D::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CDlgVisionInspectionDie2D::OnDestroy()
{
    CDialog::OnDestroy();
...
void CDlgVisionInspectionDie2D::OnClose()
{
    CDialog::OnDestroy();
}

void CDlgVisionInspectionDie2D::SetInitDialog()
{
}

void CDlgVisionInspectionDie2D::SetInitParaWindow()
{
}

void CDlgVisionInspectionDie2D::SetInitButton()
{
    CRect rtDlg;
    GetClientRect(rtDlg);
...
void CDlgVisionInspectionDie2D::OnBnClickedButtonInspect()
{
...
void CDlgVisionInspectionDie2D::OnTcnSelchangeTabResult(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
...
void CDlgVisionInspectionDie2D::OnBnClickedButtonClose()
{
    OnCancel();
}

void CDlgVisionInspectionDie2D::ShowPrevImage()
{
...
void CDlgVisionInspectionDie2D::ShowNextImage()
{
    // Image Display 를 위한 테스트 코드
    if (m_pVisionInsp->getImageLotInsp().m_vecImages[m_pVisionInsp->GetCurVisionModule_Status()].size() == 0)
        return;
...
void CDlgVisionInspectionDie2D::ShowImage(BOOL bChange)
{
...
void CDlgVisionInspectionDie2D::OnCbnSelchangeComboCurrpane()
{
    m_imageLotView->Overlay_RemoveAll();
    m_imageLotView->Overlay_Show(TRUE);
...
void CDlgVisionInspectionDie2D::OnBnClickedButtonPreimage()
{
    ShowPrevImage();
}

void CDlgVisionInspectionDie2D::OnBnClickedButtonNextimage()
{
    ShowNextImage();
}

LRESULT CDlgVisionInspectionDie2D::OnRoiChanged(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
    GetROI();
    ShowCharBinImage();
...
BOOL CDlgVisionInspectionDie2D::ShowCharBinImage()
{
    return TRUE;
}

LRESULT CDlgVisionInspectionDie2D::OnShowDetailResult(WPARAM wparam, LPARAM /*lparam*/)
{
...
void CDlgVisionInspectionDie2D::UpdatePropertyGrid()
{
    m_propertyGrid->ResetContent();

    if (auto* category = m_propertyGrid->AddCategory(_T("Die align paramters")))
    {
        category->AddChildItem(new CCustomItemButton(_T("Image Combine"), TRUE, FALSE))
            ->SetID(DIE_ALIGN_PARAM_IMAGECOMBINE);
    ...
    if (auto* category = m_propertyGrid->AddCategory(g_szEDGEAlignParamName[0]))
    {
                ...
                m_pVisionPara->m_nSearchDirection))) //, (int *)m_pVisionPara->m_nSearchDirection)))
        {
            ...
            item->SetID(DIE_ALIGN_PARAM_SEARCH_DIRECTION);
        }
                ...
                m_pVisionPara->m_nEdgeDirection))) //, (int *)m_pVisionPara->m_nEdgeDirection)))
        {
            ...
            item->SetID(DIE_ALIGN_PARAM_EDGE_DIRECTION);
        }
...
LRESULT CDlgVisionInspectionDie2D::OnGridNotify(WPARAM wparam, LPARAM lparam)
{
    ...
    if (wparam == XTP_PGN_ITEMVALUE_CHANGED)
    {
        ...
        switch (item->GetID())
        {
            ...
            case DIE_ALIGN_PARAM_EDGE_DETECT_MODE:
                ...
                break;
            case DIE_ALIGN_PARAM_FIRST_EDGE_VALUE:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                    if (value->GetNumber() < 1 || value->GetNumber() > 30)
                    {
                        ...
                        m_propertyGrid->GetItem(DIE_ALIGN_PARAM_FIRST_EDGE_VALUE + 1)->SetValue(strTemp);
                    }
                    else
                        m_pVisionPara->m_nFirstEdgeValue = value->GetNumber();
                break;
            case DIE_ALIGN_PARAM_SEARCH_LENGTH:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    ...
                    m_propertyGrid->GetItem(DIE_ALIGN_PARAM_SEARCH_LENGTH + 1)->SetValue(strTemp);
                    DrawEdgeSearchROI();
                }
                break;
            case DIE_ALIGN_PARAM_SEARCH_WIDTH:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    ...
                    m_propertyGrid->GetItem(DIE_ALIGN_PARAM_SEARCH_WIDTH + 1)->SetValue(strTemp);
                    DrawEdgeSearchROI();
                }
                break;
            case DIE_ALIGN_PARAM_SEARCH_GAP:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    m_pVisionPara->m_nEdgeDetectGap_um = min(2000, max(20, value->GetNumber()));
                    strTemp.Format(_T("%d"), m_pVisionPara->m_nEdgeDetectGap_um);
                    m_propertyGrid->GetItem(DIE_ALIGN_PARAM_SEARCH_GAP + 1)->SetValue(strTemp);
                }
                break;
        }
    }
...
void CDlgVisionInspectionDie2D::ClickedButtonImageCombine()
{
...
void CDlgVisionInspectionDie2D::GetROI()
{
}

void CDlgVisionInspectionDie2D::SetROI()
{
}

LRESULT CDlgVisionInspectionDie2D::OnImageLotViewPaneSelChanged(WPARAM /*wparam*/, LPARAM /*lparam*/)
{
...
void CDlgVisionInspectionDie2D::DrawEdgeSearchROI()
{
    m_imageLotView->Overlay_RemoveAll();
    ...
    //Die 정보 가져오기
    if (m_pVisionInsp->GetDieInfoDB() == FALSE)
        return;

    long nDieNum = (long)m_pVisionInsp->m_vecsDieInfoDB.size();
    ...
    {
        m_pVisionInsp->MakeEdgeSearchROI(m_pVisionInsp->m_vecsDieInfoDB[nDie], vecrtEdgeSearchROI);
