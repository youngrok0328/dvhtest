...
//
VisionInspectionDie2DPara::VisionInspectionDie2DPara(VisionProcessing& parent)
    : m_ImageProcManagePara(parent)
{
}

VisionInspectionDie2DPara::~VisionInspectionDie2DPara(void)
{
}

BOOL VisionInspectionDie2DPara::LinkDataBase(BOOL bSave, CiDataBase& db)
{
    long version = 1;
    if (!db[_T("Version")].Link(bSave, version))
        version = 0;

    m_ImageProcManagePara.LinkDataBase(bSave, db[_T("{B2B3FA86-8965-4C08-81E8-0B721FBE1CD8}")]);
...
std::vector<CString> VisionInspectionDie2DPara::ExportAlgoParaToText(
    CString strVisionName, CString strInspectionModuleName) //kircheis_TxtRecipe
{
    ...
    vecstrImageCombineParameters = m_ImageProcManagePara.ExportImageCombineParaToText(
        strVisionName, strInspectionModuleName, _T("Die Align-Image Combine"));
