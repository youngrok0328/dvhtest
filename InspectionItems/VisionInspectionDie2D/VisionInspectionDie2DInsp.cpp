...
void VisionInspectionDie2D::AbsoluteRectToImageRectByBodyCenter(Ipvm::Rect32s& rtROI)
{
    Ipvm::Rect32r frtBodyRect = GetBodyRect();
    AbsoluteRectToImageRectByBodyCenter(frtBodyRect, rtROI);
}

void VisionInspectionDie2D::AbsoluteRectToImageRectByBodyCenter(Ipvm::Rect32r frtBodyRect, Ipvm::Rect32s& rtROI)
{
...
void VisionInspectionDie2D::ImageRectToAbsoluteRectByBodyCenter(Ipvm::Rect32s& rtROI)
{
    Ipvm::Rect32r frtBodyRect = GetBodyRect();
    ImageRectToAbsoluteRectByBodyCenter(frtBodyRect, rtROI);
}

void VisionInspectionDie2D::ImageRectToAbsoluteRectByBodyCenter(Ipvm::Rect32r frtBodyRect, Ipvm::Rect32s& rtROI)
{
    Ipvm::Point32r2 fptCenter = frtBodyRect.CenterPoint();
    rtROI.m_left -= (long)(fptCenter.m_x + .5f);
    rtROI.m_top -= (long)(fptCenter.m_y + .5f);
...
Ipvm::Rect32r VisionInspectionDie2D::GetBodyRect()
{
...
BOOL VisionInspectionDie2D::OnInspection()
{
...
BOOL VisionInspectionDie2D::DoInspection(const bool detailSetupMode, const enSideVisionModule i_ModuleStatus)
{
    //mc_Side Vision
    ...
    bResult &= DoInspDieOffsetX();
    bResult &= DoInspDieOffsetY();
    bResult &= DoInspDieOffsetTheta();
    bResult &= DoInspDieSizeX();
    bResult &= DoInspDieSizeY();
...
BOOL VisionInspectionDie2D::GetDieInfoDB()
{
    m_vecsDieInfoDB.clear();
    ...
    for (auto dieInfo_MapData : m_packageSpec.vecDMSChip)
    {
        SDieLidPatchInfoDB sDieInfo;
        sDieInfo.strName = dieInfo_MapData.strCompID;
        sDieInfo.strType = dieInfo_MapData.strCompType;
        sDieInfo.fOffsetX_um = dieInfo_MapData.fChipOffsetX * fMMtoum;
        sDieInfo.fOffsetY_um = -dieInfo_MapData.fChipOffsetY * fMMtoum;
        sDieInfo.fAngle_Deg = dieInfo_MapData.fChipAngle;

        sDieInfo.fWidth_um = dieInfo_MapData.fChipWidth * fMMtoum;
        sDieInfo.fLength_um = dieInfo_MapData.fChipLength * fMMtoum;
        sDieInfo.fHeight_um = dieInfo_MapData.fChipThickness * fMMtoum;

        float fLidOffsetX_Pxl = sDieInfo.fOffsetX_um * um2px.m_x;
        float fLidOffsetY_Pxl = sDieInfo.fOffsetY_um * um2px.m_y;
        float fLidSizeX_Pxl = sDieInfo.fWidth_um * um2px.m_x;
        float fLidSizeY_Pxl = sDieInfo.fLength_um * um2px.m_y;
        float fLidSizeHalfX_Pxl = fLidSizeX_Pxl * .5f;
        float fLidSizeHalfY_Pxl = fLidSizeY_Pxl * .5f;

        sDieInfo.rtSpecROI.m_left = (long)(fptPackageCenter_Pxl.m_x + fLidOffsetX_Pxl - fLidSizeHalfX_Pxl + .5f);
        sDieInfo.rtSpecROI.m_right = (long)(fptPackageCenter_Pxl.m_x + fLidOffsetX_Pxl + fLidSizeHalfX_Pxl + .5f);
        sDieInfo.rtSpecROI.m_top = (long)(fptPackageCenter_Pxl.m_y + fLidOffsetY_Pxl - fLidSizeHalfY_Pxl + .5f);
        sDieInfo.rtSpecROI.m_bottom = (long)(fptPackageCenter_Pxl.m_y + fLidOffsetY_Pxl + fLidSizeHalfY_Pxl + .5f);

        m_vecsDieInfoDB.push_back(sDieInfo);
    }

    return (m_vecsDieInfoDB.size() > 0);
}

BOOL VisionInspectionDie2D::CalcAlignedCenterLine(
...
BOOL VisionInspectionDie2D::DoAlignLid(const bool detailSetupMode)
{
    m_vecsDieAlignResult.clear();
    m_veclineDieCenterHor.clear();
    m_veclineDieCenterVer.clear();
    m_veclineDieLeft.clear();
    m_veclineDieTop.clear();
    m_veclineDieRight.clear();
    m_veclineDieBottom.clear();
    ...
    //Lid 정보 가져오기
    if (GetDieInfoDB() == FALSE)
        return FALSE;
    long nDieNum = (long)m_vecsDieInfoDB.size();
    ...
    Ipvm::LineEq32r lineHor, lineVer, lineLeft, lineTop, lineRight, lineBottom;

    m_veclineDieCenterHor.resize(nDieNum);
    m_veclineDieCenterVer.resize(nDieNum);

    m_veclineDieLeft.resize(nDieNum);
    m_veclineDieTop.resize(nDieNum);
    m_veclineDieRight.resize(nDieNum);
    m_veclineDieBottom.resize(nDieNum);

    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        ...
        //Make Search ROI
        MakeEdgeSearchROI(m_vecsDieInfoDB[nDie], vecrtEdgeSearchROI);
        vecrtSpecROI.push_back(m_vecsDieInfoDB[nDie].rtSpecROI);
        ...
        if (CalcDieAlignResult(vecfptLeftEdge, vecfptTopEdge, vecfptRightEdge, vecfptBottomEdge, lineLeft, lineTop,
                lineRight, lineBottom, lidAlignResult))
        {
            m_vecsDieAlignResult.push_back(lidAlignResult);
            ...
            m_veclineDieCenterHor[nDie] = lineHor;
            m_veclineDieCenterVer[nDie] = lineVer;

            m_veclineDieLeft[nDie] = lineLeft;
            m_veclineDieTop[nDie] = lineTop;
            m_veclineDieRight[nDie] = lineRight;
            m_veclineDieBottom[nDie] = lineBottom;
        }
        else
            bAlignResult = FALSE;
    ...
    SetDebugInfoItem(detailSetupMode, _T("Die Spec"), vecrtSpecROI);
    SetDebugInfoItem(detailSetupMode, _T("Die Edge Search ROI"), vecrtSearchROI);
    SetDebugInfoItem(detailSetupMode, _T("Die Edge Point Left"), vecfptDebugLeftEdge);
    SetDebugInfoItem(detailSetupMode, _T("Die Edge Point Top"), vecfptDebugTopEdge);
    SetDebugInfoItem(detailSetupMode, _T("Die Edge Point Right"), vecfptDebugRightEdge);
    SetDebugInfoItem(detailSetupMode, _T("Die Edge Point Bottom"), vecfptDebugBottomEdge);
    SetDebugInfoItem(detailSetupMode, _T("Detect Die Rect"), vecfrtAlignResult, TRUE);

    //{{//kircheis_USI_OA
    auto* psDebugInfo = m_DebugInfoGroup.GetDebugInfo(_T("Detect Die ROI"));
    if (psDebugInfo != NULL)
    {
    ...
    for (long i = 0; i < long(vecprtAlignResult.size()); i++)
    {
        ...
        CString name;
        name.Format(_T("Die Align Result [%s]"), (LPCTSTR)m_vecsDieInfoDB[i].strName);
...
BOOL VisionInspectionDie2D::MakeEdgeSearchROI(
    SDieLidPatchInfoDB lidInfoDB, std::vector<Ipvm::Rect32s>& o_vecrtSearchROI)
{
    ...
    Ipvm::Rect32s rtDieSpecROI = lidInfoDB.rtSpecROI;

    long nSearchWidthHalfLR_Pxl = (long)((float)rtDieSpecROI.Height() * (1.f - fWidthRatio) * .5f + .5f);
    long nSearchWidthHalfTB_Pxl = (long)((float)rtDieSpecROI.Width() * (1.f - fWidthRatio) * .5f + .5f);

    //LEFT
    o_vecrtSearchROI[LEFT] = rtDieSpecROI;
    o_vecrtSearchROI[LEFT].m_left -= nSearchLengthHalfLR_Pxl;
    o_vecrtSearchROI[LEFT].m_right = rtDieSpecROI.m_left + nSearchLengthHalfLR_Pxl;
    o_vecrtSearchROI[LEFT].m_top += nSearchWidthHalfLR_Pxl;
    o_vecrtSearchROI[LEFT].m_bottom -= nSearchWidthHalfLR_Pxl;

    //Top
    o_vecrtSearchROI[UP] = rtDieSpecROI;
    ...
    o_vecrtSearchROI[UP].m_bottom = rtDieSpecROI.m_top + nSearchLengthHalfTB_Pxl;

    //RIGHT
    o_vecrtSearchROI[RIGHT] = rtDieSpecROI;
    o_vecrtSearchROI[RIGHT].m_left = rtDieSpecROI.m_right - nSearchLengthHalfLR_Pxl;
...
BOOL VisionInspectionDie2D::CalcDieAlignResult(std::vector<Ipvm::Point32r2> vecfptLeftEdge,
    std::vector<Ipvm::Point32r2> vecfptTopEdge, std::vector<Ipvm::Point32r2> vecfptRightEdge,
    std::vector<Ipvm::Point32r2> vecfptBottomEdge, Ipvm::LineEq32r& o_dieAlignLeftLine,
    Ipvm::LineEq32r& o_dieAlignTopLine, Ipvm::LineEq32r& o_dieAlignRightLine, Ipvm::LineEq32r& o_dieAlignBottomLine,
    VisionAlignResult& o_dieAlignResult)
{
    ...
    o_dieAlignLeftLine = vecsLineEq[LEFT];
    o_dieAlignTopLine = vecsLineEq[UP];
    o_dieAlignRightLine = vecsLineEq[RIGHT];
    o_dieAlignBottomLine = vecsLineEq[DOWN];
...
BOOL VisionInspectionDie2D::DoInspDieOffsetX()
{
    auto* result = m_resultGroup.GetResultByName(g_szDieInspection2DName[DIE_INSPECTION_2D_OFFSET_X]);
    if (result == nullptr)
    {
        return FALSE;
    }
    ...
    long nDieSpecNum = (long)m_vecsDieInfoDB.size();
    long nDieNum = (long)m_vecsDieAlignResult.size();
    if (nDieSpecNum != nDieNum)
    {
        return FALSE;
    }
    if (nDieNum == 0 || nDieSpecNum == 0)
        return TRUE;

    result->Resize(nDieNum);

    Ipvm::Point32r2 fptPackageCenter = m_sEdgeAlignResult->m_center;
    Ipvm::Point32r2 fptDieCenter;
    Ipvm::Point32r2 fptProjPackageCenter, fptProjDieCenter;
    float fDistance;
    float fSpecOffset;
    float fError;
    ...
    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        auto dieAlignResult = m_vecsDieAlignResult[nDie];
        auto dieInfo = m_vecsDieInfoDB[nDie];
        fSpecOffset = dieInfo.fOffsetX_um;
        if (dieAlignResult.bAvailable == FALSE)
        {
            result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType,
                dieInfo.strType, 0.f, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, fSpecOffset,
                INVALID); //kircheis_VSV //kircheis_MED2.5
            //result->SetResult(nDie, INVALID);
            result->SetRect(nDie, dieInfo.rtSpecROI);
            continue;
        }

        fptDieCenter = dieAlignResult.m_center;

        Ipvm::Geometry::GetFootOfPerpendicular(linePackageCenter, fptPackageCenter, fptProjPackageCenter);
        Ipvm::Geometry::GetFootOfPerpendicular(linePackageCenter, fptDieCenter, fptProjDieCenter);
        Ipvm::Geometry::GetDistance(fptProjPackageCenter, fptProjDieCenter, fDistance);
        fDistance *= pixelToUm.m_x;
        if (fptProjPackageCenter.m_x > fptProjDieCenter.m_x)
            fDistance *= -1.f;
        fError = fDistance - fSpecOffset;

        result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType, dieInfo.strType,
            fError, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, fSpecOffset); //kircheis_MED2.5
        result->SetRect(nDie, dieAlignResult.getBodyRect32s());
    }
...
BOOL VisionInspectionDie2D::DoInspDieOffsetY()
{
    auto* result = m_resultGroup.GetResultByName(g_szDieInspection2DName[LID_INSPECTION_2D_OFFSET_Y]);
    if (result == nullptr)
    {
        return FALSE;
    }
    ...
    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        auto dieAlignResult = m_vecsDieAlignResult[nDie];
        auto dieInfo = m_vecsDieInfoDB[nDie];
        fSpecOffset = dieInfo.fOffsetY_um;
        if (dieAlignResult.bAvailable == FALSE)
        {
            result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType,
                dieInfo.strType, 0.f, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, fSpecOffset,
        ...
        Ipvm::Geometry::GetFootOfPerpendicular(linePackageCenter, fptDieCenter, fptProjDieCenter);
        Ipvm::Geometry::GetDistance(fptProjPackageCenter, fptProjDieCenter, fDistance);
        fDistance *= pixelToUm.m_y;
        if (fptProjPackageCenter.m_y > fptProjDieCenter.m_y)
            fDistance *= -1.f;
        fError = fDistance - fSpecOffset;
...
BOOL VisionInspectionDie2D::DoInspDieOffsetTheta()
{
    auto* result = m_resultGroup.GetResultByName(g_szDieInspection2DName[DIE_INSPECTION_2D_OFFSET_THETA]);
    if (result == nullptr)
    {
        return FALSE;
    }
    ...
    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        ...
        {
            result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType,
                dieInfo.strType, 0.f, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, 0.f,
                INVALID); //kircheis_VSV //kircheis_MED2.5
            //result->SetResult(nDie, INVALID);
            result->SetRect(nDie, dieInfo.rtSpecROI);
            continue;
        ...
        lineDieHor = m_veclineDieCenterHor[nDie];
        fError = (float)(atan(-lineDieHor.m_a / lineDieHor.m_b) - atan(-linePackageHor.m_a / linePackageHor.m_b))
            * ITP_RAD_TO_DEG;

        result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType, dieInfo.strType,
            fError, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um); //kircheis_MED2.5
        result->SetRect(nDie, dieAlignResult.getBodyRect32s());
    }
    result->UpdateTypicalValue(spec);
    result->SetTotalResult();
...
BOOL VisionInspectionDie2D::DoInspDieSizeX()
{
    auto* result = m_resultGroup.GetResultByName(g_szDieInspection2DName[DIE_INSPECTION_2D_SIZE_X]);
    if (result == nullptr)
    {
        return FALSE;
    }
    ...
    float fSpecSize(0.f), fError(0.f);
    Ipvm::LineEq32r lineLT, lineRB;
    ...
    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        ...
        auto dieAlignResult = m_vecsDieAlignResult[nDie];
        auto dieInfo = m_vecsDieInfoDB[nDie];
        fSpecSize = dieInfo.fWidth_um;

        if (dieAlignResult.bAvailable == FALSE)
        {
            result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType,
                dieInfo.strType, 0.f, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, fSpecSize,
                INVALID); //kircheis_VSV //kircheis_MED2.5
            //result->SetResult(nDie, INVALID);
            result->SetRect(nDie, dieInfo.rtSpecROI);
            continue;
        ...
        lineLT = m_veclineDieLeft[nDie];
        lineRB = m_veclineDieRight[nDie];

        //LT Line to RT
        fDist = 0.f;
        fptDest = dieAlignResult.fptRT;
        Ipvm::Geometry::GetDistance(lineLT, fptDest, fDist);
        fDistMax = (float)max(fDistMax, fDist);

        //LT Line to RB
        fDist = 0.f;
        fptDest = dieAlignResult.fptRB;
        Ipvm::Geometry::GetDistance(lineLT, fptDest, fDist);
        fDistMax = (float)max(fDistMax, fDist);

        //RB Line to LT
        fDist = 0.f;
        fptDest = dieAlignResult.fptLT;
        Ipvm::Geometry::GetDistance(lineRB, fptDest, fDist);
        fDistMax = (float)max(fDistMax, fDist);

        //RB Line to LB
        fDist = 0.f;
        fptDest = dieAlignResult.fptLB;
        Ipvm::Geometry::GetDistance(lineRB, fptDest, fDist);
        fDistMax = (float)max(fDistMax, fDist);
        ...
        result->SetValue_EachResult_AndTypicalValueBySingleSpec(nDie, dieInfo.strName, dieInfo.strType, dieInfo.strType,
            fError, *spec, dieInfo.fOffsetX_um, dieInfo.fOffsetY_um, fSpecSize); //kircheis_MED2.5
        result->SetRect(nDie, dieAlignResult.getBodyRect32s());
    }
    result->UpdateTypicalValue(spec);
    result->SetTotalResult();
...
BOOL VisionInspectionDie2D::DoInspDieSizeY()
{
    auto* result = m_resultGroup.GetResultByName(g_szDieInspection2DName[DIE_INSPECTION_2D_SIZE_Y]);
    if (result == nullptr)
    {
        return FALSE;
    }
    ...
    for (long nDie = 0; nDie < nDieNum; nDie++)
    {
        ...
        lineLT = m_veclineDieTop[nDie];
        lineRB = m_veclineDieBottom[nDie];
