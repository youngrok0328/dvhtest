...
//
VisionInspectionComponent3D::VisionInspectionComponent3D(VisionUnitAgent& visionUnit, CPackageSpec& packageSpec)
    : VisionInspection(_VISION_INSP_GUID_PASSIVE_3D, _VISION_INSP_NAME_PASSIVE_3D, visionUnit, packageSpec)
    , m_PackageInfo(new Chip::ChipInfoCollection)
    , m_VisionPara(new VisionInspectionComponent3DPara)
{
    m_pVisionInspDlg = NULL;

    m_pCompAlign = new CComponentAlign(this);
    ...
    m_defaultFixedInspectionSpecs.emplace_back(_T("{BBAB04AB-8851-4140-B293-89EE4E8039DA}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_COMP_MISSING], _T("CMISS"), _T("c"), _T("Num"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::COUNT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{9B003B14-C2A0-4A10-A1EE-296B5FDF1A8A}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_COMP_HEIGHT], _T("CHGT"), _T("c"), _T("%"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::PERCENT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{DA3DAC25-89E8-4A4D-AF9D-9E74EB26EAB7}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_COMP_QUALITY], _T("CQTY"), _T("c"), _T("%"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::PERCENT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{3296D87F-3ED8-4394-9C9A-73ABE90E8E1F}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_ELECTRODE_HEIGHT], _T("EHGT"), _T("c"), _T("%"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::PERCENT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{6609EE3A-1C06-48BE-BE82-F4799E994632}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_ELECTRODE_TILT], _T("ETLT"), _T("c"), _T("um"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::MEASURED, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{14699222-CDDE-4CF1-914E-BA1F2D36F4B1}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_BODY_HEIGHT], _T("BHGT"), _T("c"), _T("%"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::PERCENT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    m_defaultFixedInspectionSpecs.emplace_back(_T("{4D351F4B-6673-4C79-AD64-64917B65E68A}"),
        g_szCompInspection3DName[COMP_INSPECTION_3D_BOTY_TILT], _T("BTLT"), _T("c"), _T("um"),
        HostReportCategory::COMPONENT, II_RESULT_TYPE::MEASURED, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE, FALSE, FALSE);
    ...
    m_DebugInfoGroup.Add(_T("Passive Chip ROI"), enumDebugInfoType::PI_Rect);
    m_DebugInfoGroup.Add(_T("Passive Electrode ROI"), enumDebugInfoType::PI_Rect);
    m_DebugInfoGroup.Add(_T("Passive Body ROI"), enumDebugInfoType::PI_Rect);

    m_DebugInfoGroup.Add(_T("Substrate ROI"), enumDebugInfoType::PI_Rect);
    m_DebugInfoGroup.Add(_T("Valid Component ROI"), enumDebugInfoType::PI_Rect);
    m_DebugInfoGroup.Add(_T("Passive Valid Electrode ROI"), enumDebugInfoType::PI_Rect);
    m_DebugInfoGroup.Add(_T("Passive Valid Body ROI"), enumDebugInfoType::PI_Rect);
}

VisionInspectionComponent3D::~VisionInspectionComponent3D(void)
{
    CloseDlg();

    if (m_pCompAlign != NULL)
    {
        delete m_pCompAlign;
        m_pCompAlign = NULL;
    }

    delete m_PackageInfo;
    delete m_VisionPara;
}

void VisionInspectionComponent3D::ResetSpecAndPara()
{
    __super::ResetSpec();
}

void VisionInspectionComponent3D::CloseDlg()
{
...
long VisionInspectionComponent3D::ShowDlg(const ProcessingDlgInfo& procDlgInfo)
{
    ...
    m_pVisionInspDlg = new CDlgVisionInspectionComponent3D(procDlgInfo, this, parent);
    m_pVisionInspDlg->Create(CDlgVisionInspectionComponent3D::IDD, parent);
    ...
    {
        strErrorMSG.Format(_T("%d data of the component map data (width, length, thickness) has a problem.\nPlease ")
                           _T("check the [dimension info].\nAfter modified map data, re-open please."),
            nErrorCompNum);
...
void VisionInspectionComponent3D::GetOverlayResult(
    VisionInspectionOverlayResult* overlayResult, const bool detailSetupMode)
{
    ...
    long specSize = (long)m_VisionPara->m_vecVisionInspectionSpecs.size();

    for (long nInsp = 0; nInsp < specSize; nInsp++)
    {
        if (m_VisionPara->m_vecVisionInspectionSpecs[nInsp].m_use)
        {
            VisionInspectionResult* pResult
                = m_resultGroup.GetResultByName(m_VisionPara->m_vecVisionInspectionSpecs[nInsp].m_specName);
            ...
            if (m_nOverlayMode == OverlayType_ShowAll || m_nOverlayMode == OverlayType_Reject)
            {
                if (pResult->vecrtRejectROI.size())
                {
                    ...
                    if (m_VisionPara->m_vecVisionInspectionSpecs[nInsp].m_useMarginal
                        && pResult->vecrtMarginalROI.size())
                    {
...
void VisionInspectionComponent3D::AppendTextResult(CString& textResult)
{
...
void VisionInspectionComponent3D::ResetResult()
{
    __super::ResetResult();
    ...
    float f2DSacaleX = getScale().pixelToUm().m_x;
    float f2DSacaleY = getScale().pixelToUm().m_y;

    m_fptPixelperMM = Ipvm::Point32r2(1000.f / f2DSacaleX, 1000.f / f2DSacaleY);
}

BOOL VisionInspectionComponent3D::LinkDataBase(BOOL bSave, CiDataBase& db)
{
    if (!__super::LinkDataBase(bSave, db))
        return FALSE;

    // Link Parameter
    if (!m_VisionPara->LinkDataBase(bSave, db[_T("{6B0D1AB6-2CD4-47FA-84AD-047B456B3BEA}")]))
        return FALSE;
...
BOOL VisionInspectionComponent3D::SetPackageInfo_SpecLink()
{
    ...
    for (long i = 0; i < m_vecsPassiveInfoDB.size(); i++)
    {
        ...
        for (long index = 0; index < m_VisionPara->m_vecComp3DAlgoSpec.size(); index++)
        {
            auto& data = m_VisionPara->m_vecComp3DAlgoSpec[index];
...
void VisionInspectionComponent3D::UpdateSpec()
{
    ...
    if (nGroupInspNameNum != nPassiveTypeNum)
        return; //개수가 다를 이유는 없다
    ...
    while (SpecNum < MakeSpecNum)
    {
        ...
        m_variableInspectionSpecs.emplace_back(m_moduleGuid, ::CreateGUID(), m_strModuleName, _T(""), strSpecInital,
            _T("C"), _T("%"), HostReportCategory::COMPONENT, II_RESULT_TYPE::PERCENT, FALSE, 0.f, 0.f, 0.f, 0.f, FALSE,
            FALSE, FALSE);
    ...
    if (m_vecsPassiveTypes.size() != 1)
    {
        if (m_VisionPara->m_vecstrGroupInspName.size() != 0)
        {
            ...
            {
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strCompHeightName);
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strCompQualName);
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strElecHeightName);
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strElecTiltName);
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strBodyHeightName);
                InspectionName.push_back(m_VisionPara->m_vecstrGroupInspName[i].strBodyTiltName);
            }
        }
    }
    ...
    for (long idx = 0; idx < long(SpecNum); idx++)
    {
        ...
        if (InspectionName[idx].Find(_T("TILT")) != -1)
        {
            m_variableInspectionSpecs[idx].m_resultType = II_RESULT_TYPE::MEASURED;
            strUnit = _T("um");
        }
...
void VisionInspectionComponent3D::GetVisionInspectionSpecs()
{
...
void VisionInspectionComponent3D::SetVisionInspectionSpecs()
{
...
std::vector<CString> VisionInspectionComponent3D::ExportRecipeToText() //kircheis_TxtRecipe
{
