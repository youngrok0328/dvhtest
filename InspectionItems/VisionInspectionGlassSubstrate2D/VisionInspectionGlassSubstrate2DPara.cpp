...
//
VisionInspectionGlassSubstrate2DPara::VisionInspectionGlassSubstrate2DPara(VisionProcessing& parent)
    : m_ImageProcManagePara(parent)
{
}

VisionInspectionGlassSubstrate2DPara::~VisionInspectionGlassSubstrate2DPara(void)
{
}

BOOL VisionInspectionGlassSubstrate2DPara::LinkDataBase(BOOL bSave, CiDataBase& db)
{
    long version = 1;
    if (!db[_T("Version")].Link(bSave, version))
        version = 0;

    m_ImageProcManagePara.LinkDataBase(bSave, db[_T("{FBF0F6A8-C375-4289-A5A8-9776B1426FAF}")]);
...
std::vector<CString> VisionInspectionGlassSubstrate2DPara::ExportAlgoParaToText(
    CString strVisionName, CString strInspectionModuleName) //kircheis_TxtRecipe
{
    ...
    vecstrImageCombineParameters = m_ImageProcManagePara.ExportImageCombineParaToText(
        strVisionName, strInspectionModuleName, _T("Glass Substrate2D Align-Image Combine"));
