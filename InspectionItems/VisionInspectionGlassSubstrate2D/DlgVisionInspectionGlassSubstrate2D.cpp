...
{
    GLASS_SUBSTRATE_ALIGN_PARAM_START = 1,
    GLASS_SUBSTRATE_ALIGN_PARAM_IMAGECOMBINE = GLASS_SUBSTRATE_ALIGN_PARAM_START,
    GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_DIRECTION,
    GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DIRECTION,
    GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DETECT_MODE,
    GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE,
    GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_LENGTH,
    GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_WIDTH,
    GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_GAP,
    GLASS_SUBSTRATE_ALIGN_PARAM_END,
};
...
static LPCTSTR g_szGlassSubstrateParamName[] = {
...
IMPLEMENT_DYNAMIC(CDlgVisionInspectionGlassSubstrate2D, CDialog)

CDlgVisionInspectionGlassSubstrate2D::CDlgVisionInspectionGlassSubstrate2D(
    const ProcessingDlgInfo& procDlgInfo, VisionInspectionGlassSubstrate2D* pVisionInsp, CWnd* pParent /*=NULL*/)
    : CDialog(CDlgVisionInspectionGlassSubstrate2D::IDD, pParent)
...
void CDlgVisionInspectionGlassSubstrate2D::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgVisionInspectionGlassSubstrate2D, CDialog)
//ON_WM_CREATE()
ON_WM_CLOSE()
ON_WM_DESTROY()
ON_BN_CLICKED(IDC_BUTTON_INSPECT, &CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonInspect)
ON_NOTIFY(TCN_SELCHANGE, IDC_TAB_RESULT, &CDlgVisionInspectionGlassSubstrate2D::OnTcnSelchangeTabResult)
ON_BN_CLICKED(IDC_BUTTON_CLOSE, &CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonClose)
ON_CBN_SELCHANGE(IDC_COMBO_CURRPANE, &CDlgVisionInspectionGlassSubstrate2D::OnCbnSelchangeComboCurrpane)
ON_BN_CLICKED(IDC_BUTTON_PREIMAGE, &CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonPreimage)
ON_BN_CLICKED(IDC_BUTTON_NEXTIMAGE, &CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonNextimage)
ON_MESSAGE(UM_ROI_CHANGED, &CDlgVisionInspectionGlassSubstrate2D::OnRoiChanged)
ON_MESSAGE(UM_SHOW_DETAIL_RESULT, &CDlgVisionInspectionGlassSubstrate2D::OnShowDetailResult)
ON_MESSAGE(XTPWM_PROPERTYGRID_NOTIFY, &CDlgVisionInspectionGlassSubstrate2D::OnGridNotify)
ON_MESSAGE(UM_IMAGE_LOT_VIEW_PANE_CHANGED, &CDlgVisionInspectionGlassSubstrate2D::OnImageLotViewPaneSelChanged)
END_MESSAGE_MAP()

BOOL CDlgVisionInspectionGlassSubstrate2D::OnInitDialog()
{
...
int CDlgVisionInspectionGlassSubstrate2D::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CDlgVisionInspectionGlassSubstrate2D::OnDestroy()
{
    CDialog::OnDestroy();
...
void CDlgVisionInspectionGlassSubstrate2D::OnClose()
{
    CDialog::OnDestroy();
}

void CDlgVisionInspectionGlassSubstrate2D::SetInitDialog()
{
}

void CDlgVisionInspectionGlassSubstrate2D::SetInitParaWindow()
{
}

void CDlgVisionInspectionGlassSubstrate2D::SetInitButton()
{
    CRect rtDlg;
    GetClientRect(rtDlg);
...
void CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonInspect()
{
...
void CDlgVisionInspectionGlassSubstrate2D::OnTcnSelchangeTabResult(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
...
void CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonClose()
{
    OnCancel();
}

void CDlgVisionInspectionGlassSubstrate2D::ShowPrevImage()
{
...
void CDlgVisionInspectionGlassSubstrate2D::ShowNextImage()
{
    // Image Display 를 위한 테스트 코드
    if (m_pVisionInsp->getImageLotInsp().m_vecImages[m_pVisionInsp->GetCurVisionModule_Status()].size() == 0)
        return;
...
void CDlgVisionInspectionGlassSubstrate2D::ShowImage(BOOL bChange)
{
...
void CDlgVisionInspectionGlassSubstrate2D::OnCbnSelchangeComboCurrpane()
{
    m_imageLotView->Overlay_RemoveAll();
    m_imageLotView->Overlay_Show(TRUE);
...
void CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonPreimage()
{
    ShowPrevImage();
}

void CDlgVisionInspectionGlassSubstrate2D::OnBnClickedButtonNextimage()
{
    ShowNextImage();
}

LRESULT CDlgVisionInspectionGlassSubstrate2D::OnRoiChanged(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
    GetROI();
    ShowCharBinImage();
...
BOOL CDlgVisionInspectionGlassSubstrate2D::ShowCharBinImage()
{
    return TRUE;
}

LRESULT CDlgVisionInspectionGlassSubstrate2D::OnShowDetailResult(WPARAM wparam, LPARAM /*lparam*/)
{
...
void CDlgVisionInspectionGlassSubstrate2D::UpdatePropertyGrid()
{
    m_propertyGrid->ResetContent();

    if (auto* category = m_propertyGrid->AddCategory(_T("Glass Substrate align paramters")))
    {
        category->AddChildItem(new CCustomItemButton(_T("Image Combine"), TRUE, FALSE))
            ->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_IMAGECOMBINE);

        category->Expand();
    }

    if (auto* category = m_propertyGrid->AddCategory(g_szGlassSubstrateParamName[0]))
    {
        if (auto* item = category->AddChildItem(new CXTPPropertyGridItemEnum(g_szGlassSubstrateParamName[1],
                m_pVisionPara->m_nSearchDirection))) //, (int *)m_pVisionPara->m_nSearchDirection)))
        {
            ...
            item->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_DIRECTION);
        }
        if (auto* item = category->AddChildItem(new CXTPPropertyGridItemEnum(g_szGlassSubstrateParamName[2],
                m_pVisionPara->m_nEdgeDirection))) //, (int *)m_pVisionPara->m_nEdgeDirection)))
        {
            ...
            item->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DIRECTION);
        }
        if (auto* item = category->AddChildItem(new CXTPPropertyGridItemEnum(g_szGlassSubstrateParamName[3],
                m_pVisionPara->m_nEdgeDetectMode))) //, (int *)m_pVisionPara->m_nEdgeDetectMode)))
        {
            ...
            item->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DETECT_MODE);
        }

        category
            ->AddChildItem(
                new CXTPPropertyGridItemNumber(g_szGlassSubstrateParamName[4], m_pVisionPara->m_nFirstEdgeValue))
            ->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE);
        category
            ->AddChildItem(
                new CXTPPropertyGridItemNumber(g_szGlassSubstrateParamName[5], m_pVisionPara->m_nEdgeSearchLength_um))
            ->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_LENGTH);
        category
            ->AddChildItem(
                new CXTPPropertyGridItemNumber(g_szGlassSubstrateParamName[6], m_pVisionPara->m_nEdgeSearchWidthRatio))
            ->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_WIDTH);
        category
            ->AddChildItem(
                new CXTPPropertyGridItemNumber(g_szGlassSubstrateParamName[7], m_pVisionPara->m_nEdgeDetectGap_um))
            ->SetID(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_GAP);

        category->Expand();
    }

    if (m_pVisionPara->m_nEdgeDetectMode != EDGE_DETECT_MODE_FIRST)
        m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE + 1)->SetReadOnly(TRUE);
    else
        m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE + 1)->SetReadOnly(FALSE);
...
LRESULT CDlgVisionInspectionGlassSubstrate2D::OnGridNotify(WPARAM wparam, LPARAM lparam)
{
    ...
    if (wparam == XTP_PGN_ITEMVALUE_CHANGED)
    {
        ...
        switch (item->GetID())
        {
            case GLASS_SUBSTRATE_ALIGN_PARAM_IMAGECOMBINE:
                ClickedButtonImageCombine();
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_DIRECTION:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemEnum*>(item))
                    m_pVisionPara->m_nSearchDirection = value->GetEnum();
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DIRECTION:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemEnum*>(item))
                    m_pVisionPara->m_nEdgeDirection = value->GetEnum();
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_EDGE_DETECT_MODE:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemEnum*>(item))
                ...
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                    if (value->GetNumber() < 1 || value->GetNumber() > 30)
                    {
                        ...
                        m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_FIRST_EDGE_VALUE + 1)->SetValue(strTemp);
                    }
                    else
                        m_pVisionPara->m_nFirstEdgeValue = value->GetNumber();
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_LENGTH:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    ...
                    m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_LENGTH + 1)->SetValue(strTemp);
                    DrawEdgeSearchROI();
                }
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_WIDTH:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    ...
                    m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_WIDTH + 1)->SetValue(strTemp);
                    DrawEdgeSearchROI();
                }
                break;
            case GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_GAP:
                if (auto* value = dynamic_cast<CXTPPropertyGridItemNumber*>(item))
                {
                    m_pVisionPara->m_nEdgeDetectGap_um = min(2000, max(20, value->GetNumber()));
                    strTemp.Format(_T("%d"), m_pVisionPara->m_nEdgeDetectGap_um);
                    m_propertyGrid->GetItem(GLASS_SUBSTRATE_ALIGN_PARAM_SEARCH_GAP + 1)->SetValue(strTemp);
                }
                break;
        }
    }
...
void CDlgVisionInspectionGlassSubstrate2D::ClickedButtonImageCombine()
{
...
void CDlgVisionInspectionGlassSubstrate2D::GetROI()
{
}

void CDlgVisionInspectionGlassSubstrate2D::SetROI()
{
}

LRESULT CDlgVisionInspectionGlassSubstrate2D::OnImageLotViewPaneSelChanged(WPARAM /*wparam*/, LPARAM /*lparam*/)
{
...
void CDlgVisionInspectionGlassSubstrate2D::DrawEdgeSearchROI()
{
    m_imageLotView->Overlay_RemoveAll();
    ...
    //Glass Substrate 정보 가져오기
    Ipvm::Rect32s rtSpecROI;
    if (m_pVisionInsp->GetGlassSubstrateSpecROI(rtSpecROI) == FALSE)
        return;

    std::vector<Ipvm::Rect32s> vecrtSearchROI(0);
    std::vector<Ipvm::Rect32s> vecrtEdgeSearchROI(4);

    m_pVisionInsp->MakeEdgeSearchROI(rtSpecROI, vecrtEdgeSearchROI);
