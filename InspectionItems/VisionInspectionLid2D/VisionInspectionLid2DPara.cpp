...
//
VisionInspectionLid2DPara::VisionInspectionLid2DPara(VisionProcessing& parent)
    : m_ImageProcManagePara(parent)
{
}

VisionInspectionLid2DPara::~VisionInspectionLid2DPara(void)
{
}

BOOL VisionInspectionLid2DPara::LinkDataBase(BOOL bSave, CiDataBase& db)
{
    long version = 1;
    if (!db[_T("Version")].Link(bSave, version))
        version = 0;

    m_ImageProcManagePara.LinkDataBase(bSave, db[_T("{3829E8C1-DAD8-4836-A684-95D4B1BDA6FD}")]);
...
std::vector<CString> VisionInspectionLid2DPara::ExportAlgoParaToText(
    CString strVisionName, CString strInspectionModuleName) //kircheis_TxtRecipe
{
    ...
    vecstrImageCombineParameters = m_ImageProcManagePara.ExportImageCombineParaToText(
        strVisionName, strInspectionModuleName, _T("Lid Align-Image Combine"));
