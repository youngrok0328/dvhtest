...
/* dmtxregion.c */
int ClampIntRange(int value, int min, int max);
DmtxCompassEdge GetCompassEdge(DmtxImage *image, int x, int y, int edgeScanDirs);
DmtxEdgeSubPixel FindZeroCrossing(DmtxDecode *dec, int x, int y, DmtxCompassEdge *compare);
DmtxRay2 FollowEdge(DmtxDecode *dec, int x, int y, DmtxEdgeSubPixel edgeStart, int forward, int hough[], int *strongIdx);
double RightAngleTrueness(DmtxVector2 c0, DmtxVector2 c1, DmtxVector2 c2, double angle);
void SetCornerLoc(DmtxRegion *region, DmtxCornerLoc cornerLoc, DmtxVector2 point);
int MatrixRegionUpdateXfrms(DmtxDecode *dec, DmtxRegion *reg);

int MatrixRegionAlignFirstEdge(DmtxDecode *dec, DmtxRegion *reg,
      DmtxEdgeSubPixel *edgeStart, DmtxRay2 ray0, DmtxRay2 ray1);

int MatrixRegionAlignSecondEdge(DmtxDecode *dec, DmtxRegion *reg);
int MatrixRegionAlignRightEdge(DmtxDecode *dec, DmtxRegion *reg);
int MatrixRegionAlignTopEdge(DmtxDecode *dec, DmtxRegion *reg);

int MatrixRegionAlignCalibEdge(DmtxDecode *dec, DmtxRegion *region,
      DmtxEdgeLoc edgeLoc, DmtxMatrix3 preFit2Raw, DmtxMatrix3 postRaw2Fit);

int MatrixRegionAlignEdge(DmtxDecode *dec, DmtxRegion *region,
      DmtxMatrix3 postRaw2Fit, DmtxMatrix3 preFit2Raw, DmtxVector2 *p0,
      DmtxVector2 *p1, DmtxVector2 *pCorner, int *weakCount);

int StepAlongEdge(DmtxDecode *dec, DmtxRegion *region,
      DmtxVector2 *pProgress, DmtxVector2 *pExact, DmtxVector2 forward,
      DmtxVector2 lateral);

DmtxColor3 ReadModuleColor(DmtxImage *image, DmtxRegion *region,
      int symbolRow, int symbolCol, int sizeIdx);

int MatrixRegionFindSize(DmtxImage *img, DmtxRegion *reg);
int CountJumpTally(DmtxImage *img, DmtxRegion *reg, int xStart, int yStart, DmtxDirection dir);
/* void WriteDiagnosticImage(DmtxDecode *dec, DmtxRegion *reg, char *imagePath); */

/* dmtxdecode.c */
long DecodeDataStream(DmtxMessage *message, int sizeIdx, unsigned char *outputStart);
...
int DecodeCheckErrors(unsigned char *code, int sizeIdx, int fix);
static int GfProduct(int a, int b);
static int GfDoublify(int a, int b);

/* dmtxscangrid.c */
DmtxScanGrid InitScanGrid(DmtxImage *img, int smallestFeature);
DmtxPixelLoc IncrementPixelProgress(DmtxScanGrid *cross);
void SetDerivedFields(DmtxScanGrid *cross);
DmtxPixelLoc GetGridCoordinates(DmtxScanGrid *grid);
