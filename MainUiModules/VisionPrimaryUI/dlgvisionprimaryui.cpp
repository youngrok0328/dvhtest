...
#include "../../UserInterfaceModules/VisionCommonDialog/VisionCommonLogDlg.h"
...
void CDlgVisionPrimaryUI::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_STATIC_ACCESSMODE1, m_Label_AccessMode1);
    DDX_Control(pDX, IDC_STATIC_ACCESSMODE2, m_Label_AccessMode2);
    DDX_Control(pDX, IDC_STATIC_VERSION1, m_Label_Version1);
    DDX_Control(pDX, IDC_STATIC_VERSION2, m_Label_Version2);
...
BOOL CDlgVisionPrimaryUI::OnInitDialog()
{
    ...
    if (m_visionUnit.m_systemConfig.GetVisionType() != VISIONTYPE_SIDE_INSP)
    {
    ...
    //Ipvm::SocketMessaging::INIT_INFO initInfo =
    //{
    //
    //};
...
void CDlgVisionPrimaryUI::SetGeneralVisionMainUI()
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    CRect rtLogo;
    GetDlgItem(IDC_STATIC_LOGO)->GetWindowRect(rtLogo);
    GetDlgItem(IDC_STATIC_LOGO)->MoveWindow(rtClient.left, rtClient.top, rtLogo.Width(), rtLogo.Height());
    GetDlgItem(IDC_STATIC_LOGO)->GetWindowRect(rtLogo);
    ScreenToClient(rtLogo);

    const long labelHeight = 43;
    const long labelGap = 3;
    const long leftLabelX = rtClient.left;
    const long leftLabelWidth = 200;
    const long rightLabelX = rtClient.left + leftLabelWidth + labelGap;
    const long rightLabelWidth = rtLogo.Width() - leftLabelWidth - labelGap;

    long offsetY = rtLogo.bottom + labelGap;

    m_Label_JobFile.MoveWindow(leftLabelX, offsetY, leftLabelWidth, labelHeight);
    m_Label_JobFileName.MoveWindow(rightLabelX, offsetY, rightLabelWidth, labelHeight);
    offsetY += labelHeight + labelGap;

    m_Label_Version1.MoveWindow(leftLabelX, offsetY, leftLabelWidth, labelHeight);
    m_Label_Version2.MoveWindow(rightLabelX, offsetY, rightLabelWidth, labelHeight);
    offsetY += labelHeight + labelGap;

    m_Label_Connect1.MoveWindow(leftLabelX, offsetY, leftLabelWidth, labelHeight);
    m_Label_Connect2.MoveWindow(rightLabelX, offsetY, rightLabelWidth, labelHeight);
    offsetY += labelHeight + labelGap;

    m_Label_AccessMode1.MoveWindow(leftLabelX, offsetY, leftLabelWidth, labelHeight);
    m_Label_AccessMode2.MoveWindow(rightLabelX, offsetY, rightLabelWidth, labelHeight);
    offsetY += labelHeight + labelGap;

    GetDlgItem(IDC_STATIC_BGATOP)->SetWindowPos(nullptr, leftLabelX, offsetY, 0, 0, SWP_NOSIZE);
    GetDlgItem(IDC_STATIC_BGABOTTOM)->SetWindowPos(nullptr, leftLabelX, offsetY, 0, 0, SWP_NOSIZE);

    CRect rtTypeImage;
    GetDlgItem(IDC_STATIC_BGATOP)->GetWindowRect(rtTypeImage);
    offsetY += rtTypeImage.Height() + 3;

    m_LabelResult.MoveWindow(leftLabelX, offsetY, rtLogo.Width(), labelHeight);
    offsetY += labelHeight + labelGap;

    CRect rtTab(rtClient.left, offsetY, rtLogo.right, rtClient.bottom);
    ...
    CRect rtLotViewRegion, rtLotViewRegionRearSide;

    rtLotViewRegion.left = rtLogo.right + 5;
    rtLotViewRegion.top = rtClient.top;
    rtLotViewRegion.right = rtLotViewRegion.left + (rtClient.Width() - rtLogo.Width() - 5);
    rtLotViewRegion.bottom = rtClient.bottom;

    m_imageLotView = new ImageLotView(rtLotViewRegion, m_visionUnit, true, GetSafeHwnd());
    ...
    m_visionCommonLogDlg = new VisionCommonLogDlg(this, rtTab);

    m_visionCommonSpecDlg->ShowWindow(SW_SHOW);

    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    m_Label_JobFile.SetFontSize(23);
    ...
    m_Label_JobFileName.SetFontSize(23);
    ...
    m_Label_Version1.SetFontSize(23);
    m_Label_Version1.SetFontName(_T("Arial"));
    m_Label_Version1.SetFontBold(TRUE);
    m_Label_Version1.SetTextColor(RGB(0, 0, 0));
    m_Label_Version1.SetBkColor(RGB(200, 200, 250));
    m_Label_Version1.SetAlignment(CNewLabel::HORZ_CENTER | CNewLabel::VERT_CENTER);
    m_Label_Version1.SetText(_T("Version Info"));

    m_Label_Version2.SetFontSize(23);
    m_Label_Version2.SetFontName(_T("Arial"));
    m_Label_Version2.SetFontBold(TRUE);
    m_Label_Version2.SetTextColor(RGB(0, 0, 0));
    m_Label_Version2.SetBkColor(RGB(255, 255, 255));
    m_Label_Version2.SetAlignment(CNewLabel::HORZ_CENTER | CNewLabel::VERT_CENTER);

    m_Label_Connect1.SetFontSize(23);
    ...
    m_Label_Connect2.SetFontSize(23);
    ...
    m_Label_AccessMode1.SetFontSize(23);
    m_Label_AccessMode1.SetFontName(_T("Arial"));
    m_Label_AccessMode1.SetFontBold(TRUE);
    m_Label_AccessMode1.SetTextColor(RGB(0, 0, 0));
    m_Label_AccessMode1.SetBkColor(RGB(200, 200, 250));
    m_Label_AccessMode1.SetAlignment(CNewLabel::HORZ_CENTER | CNewLabel::VERT_CENTER);
    m_Label_AccessMode1.SetText(_T("Access Mode"));

    m_Label_AccessMode2.SetFontSize(23);
    m_Label_AccessMode2.SetFontName(_T("Arial"));
    m_Label_AccessMode2.SetFontBold(TRUE);
    m_Label_AccessMode2.SetTextColor(RGB(0, 0, 0));
    m_Label_AccessMode2.SetBkColor(RGB(255, 255, 255));
    m_Label_AccessMode2.SetAlignment(CNewLabel::HORZ_CENTER | CNewLabel::VERT_CENTER);

    m_LabelResult.SetFontSize(23);
...
void CDlgVisionPrimaryUI::SetSideVisionMainUI()
{
    CRect rtParentClient;
    GetParent()->GetClientRect(rtParentClient);
    ...
    ////// Side Live Status Label Test
    CRect rtSIdeFrontLiveStatus, rtSIdeRearLiveStatus;

    GetDlgItem(IDC_STATIC_SIDE_FRONT_PRIMARY)->GetWindowRect(rtSIdeFrontLiveStatus);
    GetDlgItem(IDC_STATIC_SIDE_REAR_PRIMARY)->GetWindowRect(rtSIdeRearLiveStatus);

    SetDlgItemText(IDC_STATIC_SIDE_FRONT_PRIMARY, _T("SIDE FRONT"));
    SetDlgItemText(IDC_STATIC_SIDE_REAR_PRIMARY, _T("SIDE REAR"));

    m_Label_Side_Front_Status.MoveWindow(leftLabelX, offsetY, (rtLogo.Width() / 2) - 2, labelHeight);

    m_Label_Side_Rear_Status.MoveWindow((rtLogo.Width() / 2) + 2, offsetY, (rtLogo.Width() / 2), labelHeight);
    offsetY += labelHeight + labelGap;

    // Selector 자리를 확보 후 Selection Label에 대한 색깔 부여
    SetPrimaryUISideVisionSection();
    ...
    rtLotViewRegion.top = rtClient.top;
    rtLotViewRegion.right = rtLotViewRegion.left + (rtClient.Width() - rtLogo.Width() - 5);
    rtLotViewRegion.bottom = (rtClient.bottom / 2) - 1;

    m_imageLotView = new ImageLotView(rtLotViewRegion, m_visionUnit, true, GetSafeHwnd());

    m_imageLotView->SetMessage_SelChangedPane(UM_IMAGE_LOT_VIEW_PANE_SEL_CHANGED);

    rtLotViewRegionRearSide.left = rtLotViewRegion.left;
    rtLotViewRegionRearSide.top = (rtClient.bottom / 2) + 1;
    rtLotViewRegionRearSide.right = rtLotViewRegion.right;
    rtLotViewRegionRearSide.bottom = rtClient.bottom;

    m_imageLotViewRearSide = new ImageLotView(
        rtLotViewRegionRearSide, m_visionUnit, true, GetSafeHwnd(), enSideVisionModule::SIDE_VISIONMODULE_REAR);
    ...
    m_visionCommonResultDlgRear
        = new VisionCommonResultDlg(this, rtTab, m_visionUnit, *m_imageLotViewRearSide, UM_SHOW_DETAIL_RESULT);
    m_visionCommonDetailResultDlgRear
        = new VisionCommonDetailResultDlg(this, rtTab, m_visionUnit, *m_imageLotViewRearSide);
    m_visionCommonTextResultDlgRear = new VisionCommonTextResultDlg(this, rtTab);
    m_visionCommonDebugInfoDlgRear = new VisionCommonDebugInfoDlg(
        this, rtTab, m_visionUnit, *m_imageLotViewRearSide, *m_visionCommonTextResultDlgRear);
...
void CDlgVisionPrimaryUI::OnJobChanged()
{
    ...
    if (m_visionUnit.m_pPackageSpec->m_deadBug)
    {
        GetDlgItem(IDC_STATIC_BGABOTTOM)->ShowWindow(SW_SHOW);
        GetDlgItem(IDC_STATIC_BGATOP)->ShowWindow(SW_HIDE);
    }
    else
    {
        GetDlgItem(IDC_STATIC_BGATOP)->ShowWindow(SW_SHOW);
        GetDlgItem(IDC_STATIC_BGABOTTOM)->ShowWindow(SW_HIDE);
    }
...
void CDlgVisionPrimaryUI::OnTimer(UINT_PTR nIDEvent)
{
    switch (nIDEvent)
    {
        case Timer_Send_Host_Live_Image:
        {
            ...
            {
                Ipvm::Image8u image(FrameGrabber::GetInstance().get_grab_image_width(idx_grabber, idx_camera),
                    FrameGrabber::GetInstance().get_grab_image_height(idx_grabber, idx_camera));
                ...
                SyncController::GetInstance().TurnOnLight(nSendFrameID, enSideVisionModule(m_visionMainAgent.GetSideVisionSection()));
...
void CDlgVisionPrimaryUI::SetLiveMain()
{
    ...
    SyncController::GetInstance().TurnOnLight(
        ...
        // 3D 복합광학계인 경우 카메라 그랩 모드도 전달 한다.
    if (SystemConfig::GetInstance().GetVisionType() == VISIONTYPE_3D_INSP
        && SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        int cameraGrabMode(0);
        if (auto* pMain = dynamic_cast<CVisionMain*>(&m_visionMainAgent))
        {
            if (pMain->m_pSlitBeam3DCommon != nullptr)
                cameraGrabMode = pMain->m_pSlitBeam3DCommon->m_cameraGrabMode; // 0: Specular, 1: Defused, 2: Both
        }
        FrameGrabber::GetInstance().live_on(GetSafeHwnd(), UM_GRAB_END_EVENT, 500L,
            enSideVisionModule(m_visionMainAgent.GetSideVisionSection()), cameraGrabMode);
    }
    else
    {
