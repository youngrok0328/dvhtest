...
void CDlgVisionPrimaryUI::OnMessageSocketDataReceived(const Ipvm::SocketMessaging::MSG_INFO& msgInfo)
{
    ...
    switch (nMessageID)
    {
        ...
        case MSG_PROBE_IMAGE_LIVE: // Host에서 보낼 Live 영상을 시작한다 ( Timer )
            ...
            if (m_visionUnit.m_systemConfig.m_nVisionType == VISIONTYPE_2D_INSP
                || m_visionUnit.m_systemConfig.m_nVisionType == VISIONTYPE_TR
                || m_visionUnit.m_systemConfig.m_nVisionType == VISIONTYPE_NGRV_INSP) //kircheis_SideVision고려
            {
        ...
        case MSG_PROBE_SET_ACCESS_MODE:
            DevelopmentLog::AddLog(DevelopmentLog::Type::TCP, _T("PrimaryUI-Received::MSG_PROBE_SET_ACCESS_MODE"));
            {
                ...
                ::SendMessage(m_visionMainAgent.m_hwndMainFrame, m_visionMainAgent.m_msgAccessModeChanged, 0, 0);
            }
            break;
