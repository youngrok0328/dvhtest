...
BatchManager g_batchManager[TR_EXECUTE_END];
    ...
    , m_bIsRearModuleInspection(false)
{
    ...
    m_batchManager = &g_batchManager[m_visionMainAgent.m_mainType];
}
...
BOOL CDlgPrimaryBatchList::OnInitDialog()
{
    ...
    m_pDlgVisionBatchInspectionUI->m_imageLotView->ShowImage(m_batchManager->m_lastFrameIndex);
    ...
    // Grid 설정
    Refresh();
    m_batchManager->UpdateGridCtrl_List(*m_pgridBatch);
    m_batchManager->UpdateGridCtrl_Batch(*m_pgridBatch);
...
void CDlgPrimaryBatchList::OnDblclkGridBatchList(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
    ...
    if (CellID.row >= 2 && CellID.row <= nGridNum - 1)
    {
        ...
        strDir = m_batchManager->m_vecBatchList[nImage].strFileDir;
...
void CDlgPrimaryBatchList::OnBnClickedButtonAutoInspect()
{
    ...
    m_batchManager->Inspect_StartReady(m_visionUnit);
    ...
    if (nGridNum > 1)
    {
        m_batchManager->m_nGridNum = 2;
        m_batchManager->m_nImageNum = 0;
...
bool CDlgPrimaryBatchList::DoInspection(long nGrid, long nPane, bool threadRun)
{
    ...
    CString strDir = m_batchManager->m_vecBatchList[dataIndex].strFileDir;
    CString strFilename = m_batchManager->m_vecBatchList[dataIndex].strFileName;
    CString strBarcode = m_batchManager->m_barcodeData.GetBarcode(nPane, strFilename);
    ...
    m_visionUnit.getTrayScanSpec().SetBatchKey(nGrid, m_batchManager->m_barcodeData.GetBarcode(nPane, strFullFilename));
...
BOOL CDlgPrimaryBatchList::SaveAllInspImage(VisionDeviceResult& devResult, const enSideVisionModule i_eSideVisionModule)
{
    ...
    if ((devResult.m_nTotalResult != NOT_MEASURED) && (devResult.m_nTotalResult != EMPTY))
    {
        ...
        for (i = 0; i < lotInspImage.m_vecImages[i_eSideVisionModule].size(); i++)
        {
            ...
            Ipvm::Image8u image(nImageSizeX, nImageSizeY, pbyImage, nImageSizeX);
    ...
    for (i = 0; i < nNumInsp; i++)
    {
                ...
                || devResult.m_vecVisionInspResult[i]->m_resultName == _T("Ball Bridge")))
        {
            ...
            for (frm = 0; frm < nFrmNum; frm++)
            {
                ...
                Ipvm::Image8u image(nImageWidth, nImageHeight, pbyImage, nImageWidth);
        ...
        switch (m_runState)
        {
            case BatchRunState::Idle:
                if (1)
                {
                    ...
                    if (m_batchManager->m_bInfiniteInspectionMode)
                    {
                        if (m_batchManager->m_nGridNum >= (long)(m_pgridBatch->GetRowCount()))
                        {
                            m_batchManager->m_nGridNum = 2;
                        ...
                        if (SystemConfig::GetInstance().Get_Enable_CPU_CALC_SAVE_LOG() == true
                            ...
                            m_batchManager->m_nGridNum = (long)(m_pgridBatch->GetRowCount()) + 1;
                    }

                    if (m_batchManager->m_nGridNum < (long)(m_pgridBatch->GetRowCount())
                        && m_batchManager->m_nGridNum > 0)
                    {
                    ...
                    else
                    {
                        if (m_visionMainAgent.IsAllBatchInspectionThreadsIdle())
                        {
                            ...
                            CDlgYieldViewer Dlg;
                            Dlg.SetVisionInspResult(m_batchManager->m_vecstBatchInspResult);
            ...
            case BatchRunState::ThreadReadyCheck:
                ...
                {
                    long nPane
                        = _ttoi(m_pgridBatch->GetItemText(m_batchManager->m_nGridNum, BatchPreColumn_PaneID)) - 1;

                    if (DoInspection(m_batchManager->m_nGridNum, nPane, true))
                    {
                        m_runState = BatchRunState::Idle;
                        m_batchManager->m_nGridNum++;
...
{
    //{{ //kircheis_KillProc
    long nBatchListSize = (long)m_batchManager->m_vecBatchList.size();
    if (nBatchListSize <= 0)
    {
        return FALSE;
    }
    ...
    for (auto& rnrDataSet : m_batchManager->m_mapRnRData)
    {
    ...
    m_batchManager->m_reportData.Save(m_batchManager->m_barcodeData.IsEnabled(), strFolderPath_RnR);
    m_batchManager->m_reportWarpageData.Save(m_batchManager->m_barcodeData.IsEnabled(), strFolderPath_RnR);
    m_batchManager->m_reportSurfaceData.Save(m_batchManager->m_barcodeData.IsEnabled(), strFolderPath_Surface);
    m_batchManager->m_reportBodyData.Save(m_batchManager->m_barcodeData.IsEnabled(), strFolderPath_RnR);
...
BOOL CDlgPrimaryBatchList::WriteAutomationResult()
{
    ...
    m_batchManager->m_reportData.SaveAutomationInfo(m_batchManager->m_barcodeData.IsEnabled(), strAutomationFolder);
    m_batchManager->m_reportSurfaceData.SaveAutomationInfo(
        m_batchManager->m_barcodeData.IsEnabled(), strAutomationFolder, m_visionUnit);
    m_batchManager->m_reportBodyData.SaveAutomationInfo(m_batchManager->m_barcodeData.IsEnabled(), strAutomationFolder);
...
void CDlgPrimaryBatchList::SaveRnRData(long nScan, long nPane, long dataIndex, VisionUnit& visionUnit)
{
    ...
    {
        auto& data = m_batchManager->m_mapRnRData[std::make_pair(result->m_resultName, key)];
...
void CDlgPrimaryBatchList::SetCurNum()
{
    ...
    CString str;
    str.Format(_T("%d/%d"), m_batchManager->m_nGridNum - 2, nGridMaxNum - 2);
...
void CDlgPrimaryBatchList::OnBnClickedButtonDelete()
{
    ...
    for (long rowIndex = range.GetMaxRow(); rowIndex >= range.GetMinRow(); rowIndex--)
    {
        ...
        m_batchManager->m_vecBatchList.erase(m_batchManager->m_vecBatchList.begin() + index);
...
void CDlgPrimaryBatchList::OnBnClickedButtonInfiniteInsp()
{
    ...
    m_batchManager->Inspect_StartReady(m_visionUnit);
    m_batchManager->m_bInfiniteInspectionMode = TRUE;
    if (nGridNum > 1)
    {
        //{{//kircheis_201711
...
void CDlgPrimaryBatchList::call_openImageFiles()
{
    ...
    while (pos)
    {
        ...
        for (long paneIndex = 0; paneIndex < paneCount; paneIndex++)
        {
            ...
            // Batch List 변수 Update
            m_batchManager->m_vecBatchList.push_back(BatchUnitInfo);
        }
    }
...
void CDlgPrimaryBatchList::call_inspection()
{
    ...
    m_batchManager->m_vecstBatchInspResult.clear();
    m_batchManager->m_vecstBatchInspResult.resize(ninspNum);
...
void CDlgPrimaryBatchList::OnDestroy()
{
    ...
    m_batchManager->m_lastFrameIndex = m_pDlgVisionBatchInspectionUI->m_imageLotView->GetCurrentImageFrame();
}
...
LRESULT CDlgPrimaryBatchList::OnInspectionEnd(WPARAM wParam, LPARAM lParam)
{
    ...
    m_batchManager->Inspect_DataCollect(visionUnit, dataIndex);
    m_batchManager->UpdateGridCtrl_Batch(*m_pgridBatch, rowIndex);
    m_pgridBatch->Refresh();
    ...
    if (m_batchManager->m_bInfiniteInspectionMode == false)
    {
        ...
        m_batchManager->m_reportData.Add(dataIndex, filePath, barcode, visionUnit);
        m_batchManager->m_reportWarpageData.Add(dataIndex, filePath, barcode, visionUnit);
        m_batchManager->m_reportSurfaceData.Add(dataIndex, filePath, barcode, visionUnit);
        m_batchManager->m_reportBodyData.Add(dataIndex, filePath, barcode, visionUnit);
    }
    SetCurNum();
...
void CDlgPrimaryBatchList::OnBnClickedButtonBarcode()
{
    ...
    m_batchManager->m_barcodeData.Reset();
    while (pos)
    {
        m_batchManager->m_barcodeData.AddFile(selectVisionDlg.m_visionIndex, Dlg.GetNextPathName(pos));
    }
