...
InspectionOrderer::InspectionOrderer(
    VisionMainTR* main, ImageClassifier* imageClassifier, VisionUnit** inlineUnits, long threadNum)
    : m_main(main)
...
{
    const int32_t mainType = m_items[0].m_visionUnit->GetVisionMainType();

    static long nLatestThreadID_InPocket = 0;
    static long nLatestThreadID_OTI = 0;
    const long nThreadID = (mainType == TR_EXECUTE_IN_POCKET) ? nLatestThreadID_InPocket : nLatestThreadID_OTI;
    if (mainType == TR_EXECUTE_IN_POCKET)//상단에서 [XXXX++] 연산자를 사용했을 때 오동작하는 것을 확인(3회 발생)해서 이와 같이 명시적으로 연산함.
    {
        nLatestThreadID_InPocket++;
    }
    else
    {
        nLatestThreadID_OTI++;
