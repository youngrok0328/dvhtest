...
class __VISION_MAIN_TR_CLASS__ CGrabItemFrameInfoNGRV //kircheis_NGRV // 얘를 벡터로 만들어 Recipe로 관리 하자.
{
public:
...
class __VISION_MAIN_TR_CLASS__ CNGRVImageSet //kircheis_NGRV // 영상 저장을 위해 관리하는 class
{
public:
    CNGRVImageSet();
...
class __VISION_MAIN_TR_CLASS__ VisionMainTR : public VisionMainAgent
{
public:
    VisionMainTR(SystemConfig& systemConfig, int32_t mainType, HANDLE flag_InlineStart, HANDLE flag_SnycStart);
    virtual ~VisionMainTR(void);
...
public: // 통신 관련
    ...
    bool Copy_NGRV_SingleRunInfo(const long& VisionType, const VisionScale& scale, const long& ImageSizeX,
        const long& ImageSizeY, const long& defectNumber, const std::vector<CString>& m_vecDefectName,
...
public:
    ...
    // Grab 함수들
    bool ManualGrab2D(VisionImageLot& imageLot, const enTRVisionmodule& TR_Visionmodule);
...
public:
    ...
    void SetSyncStartFlag();
    void SetInlineStartFlag(const bool& isOn);
...
private:
    ...
    int32_t m_trVisionTypeInRecipe;
...
private: //mc_Review image Save LOG
 ...
 private:
    HANDLE m_Flag_Inline_Start;
    HANDLE m_Flag_Sync_Start;
};
