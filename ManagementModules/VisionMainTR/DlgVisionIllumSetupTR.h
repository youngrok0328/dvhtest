...
//
class DlgVisionIllumSetupTR : public CDialog
{
    DECLARE_DYNAMIC(DlgVisionIllumSetupTR)

public:
    DlgVisionIllumSetupTR(VisionMainTR& visionMain, VisionUnit& visionUnit, CWnd* pParent,
        const enSideVisionModule i_eSideVisionModule
        = enSideVisionModule::SIDE_VISIONMODULE_FRONT); // 표준 생성자입니다.
    virtual ~DlgVisionIllumSetupTR();
