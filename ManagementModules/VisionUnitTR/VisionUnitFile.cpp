...
BOOL VisionUnit::OpenImageFile()
{
    ...
    if (OpenImageFile(Dlg.GetPathName(), isOfficialExtension))
    {
...
BOOL VisionUnit::SaveImageFile()
{
    if (GetAsyncKeyState(VK_CONTROL) && m_systemConfig.GetVisionType() == VISIONTYPE_3D_INSP)
    {
        ...
        const auto& zmap = m_imageLot->GetZmapImage();
        CFile file;
...
BOOL VisionUnit::OpenImageFile(CString strFileName, bool isOfficialExtension)
{
    if (isOfficialExtension)
    {
        // Official Extension은 Intekplus 고유 bmp 확장자가 아니라
        // 일단 bmp, jpg, png 등을 오픈하는 경우다
        // 평상시에는 사용하지 않고 Ctrl+Shift 키를 누른 상태에서만 동작한다
        // 이런 포멧은 여러 프레임을 지원하지 않으므로 첫 프레임만 복사한다

        if (m_imageLot->GetImageFrameCount() <= 0)
        {
            return FALSE;
        }

        Ipvm::Image8u image;
        if (Ipvm::ImageFile::Load(strFileName, image) != Ipvm::Status::e_success)
        {
            return FALSE;
        }
        if (m_imageLot->GetImageSizeX() != image.GetSizeX() || m_imageLot->GetImageSizeY() != image.GetSizeY())
        {
            return FALSE;
        }

        Ipvm::ImageProcessing::Copy(image, m_imageLot->GetImageFrame(0));
        return TRUE;
    }
