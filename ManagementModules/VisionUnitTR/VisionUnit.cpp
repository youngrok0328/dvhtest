...
#include "../../InspectionItems/VisionInspectionInPocketTR/VisionInspectionInPocketTR.h"
#include "../../InspectionItems/VisionInspectionOTITR/VisionInspectionOTITR.h"
...
#include "../../ProcessingItems/VisionProcessingCarrierTapeAlign/VisionProcessingCarrierTapeAlign.h"
...
#include "../../ProcessingItems/VisionProcessingSpecTR/VisionProcessingSpec.h"
#include "../../ProcessingItems/VisionProcessingTapeDetailAlign/VisionProcessingTapeDetailAlign.h"
...
{
    m_visionProcessingSpec_TapeReelLayoutEditor
        = new VisionProcessingSpec(_VISION_INSP_GUID_PACKAGE_SPEC, _T("Tape && Reel Layout"), *this, *m_pPackageSpec);
    m_VisionProcessingFOV = new VisionProcessingFOV(*this, *m_pPackageSpec);
    m_visionProcessingCarrierTapeAlign = new VisionProcessingCarrierTapeAlign(*this, *m_pPackageSpec);
    m_visionProcessingTapeDetailAlign = new VisionProcessingTapeDetailAlign(*this, *m_pPackageSpec);
    ...
    m_visionInspectionInPocket = new VisionInspectionInPocketTR(*this, *m_pPackageSpec);
    m_visionInspectionOTITR = new VisionInspectionOTITR(*this, *m_pPackageSpec);

    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionProcessingSpec_TapeReelLayoutEditor);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_VisionProcessingFOV);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionProcessingCarrierTapeAlign);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionProcessingTapeDetailAlign);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionInspection2DMatrix);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionInspectionInPocket);
    m_vecAllPredefineVisionModulesBuffer.push_back(m_visionInspectionOTITR);
    ...
    //{{//kircheis_TRV//Layout Editor와 FOV는 무조건 Default로 가져가야하는 아이템이다. 일단 깔고 시작
    m_vecVisionModules.push_back(m_visionProcessingSpec_TapeReelLayoutEditor); // Layout Editor for Tape & Reel
...
VisionUnit::~VisionUnit(void)
{
    ...
    delete m_tapeSpec;
    delete m_visionInspectionOTITR;
    delete m_visionInspectionInPocket;
    delete m_visionInspection2DMatrix;
    delete m_visionProcessingGlobalMaskMaker;
    delete m_visionProcessingTapeDetailAlign;
    delete m_visionProcessingCarrierTapeAlign;
    delete m_VisionProcessingFOV;
    delete m_visionProcessingSpec_TapeReelLayoutEditor;
...
void VisionUnit::InspectionClear()
{
    ...
    m_visionProcessingCarrierTapeAlign->ResetSpecAndPara();
    m_visionProcessingTapeDetailAlign->ResetSpecAndPara();
    m_visionProcessingGlobalMaskMaker->ResetSpecAndPara();
    m_visionInspection2DMatrix->ResetSpecAndPara();
    m_visionInspectionInPocket->ResetSpecAndPara();
    m_visionInspectionOTITR->ResetSpecAndPara();
    ...
    const enSideVisionModule i_eSideVisionModule)
{
    ...
    static const bool isTRV = m_systemConfig.IsVisionTypeTR() == 1 ? true : false;

    if (isTRV == true) //TR일 경우 검사 시작 전에 Fill Pocket 여부를 각 모듈에 전달한다.
    {
        const bool fillPocket = getImageLot().GetInfo().m_fillPocket;
        if (m_visionInspectionInPocket != nullptr)
            m_visionInspectionInPocket->SetReceivedFillPocketStatus(fillPocket);
        if (m_visionInspectionOTITR != nullptr)
            m_visionInspectionOTITR->SetReceivedFillPocketStatus(fillPocket);
    }
    ...
    for (long nid = 0; nid < nInspectionNum; nid++)
    {
        ...
        if (bInsp)
        {
            ...
            if (!bReturnValue)
            {
                if (dynamic_cast<VisionProcessingTapeDetailAlign*>(proc))
                {
...
BOOL VisionUnit::LinkDataBase(long jobVersion, BOOL bSave, CiDataBase& JobDB)
{
    ...
    m_tapeSpec->LinkDataBase(bSave, JobDB[_T("Tape Spec")]);

    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //{{ Fixed Inspection Items
    {
        m_visionProcessingSpec_TapeReelLayoutEditor->LinkDataBase(
            bSave, JobDB[m_visionProcessingSpec_TapeReelLayoutEditor->m_moduleGuid]);
        ...
        for (long nProc = 0; nProc < nProcNum; nProc++)
        {
            ...
            auto moduleName = proc->m_strModuleName;
...
{
    if (!m_tapeSpec->LinkDataBase(bSave, JobDB[_T("TAPE_SPEC")]))
        return false;
    ...
    if (!bSave)
    {
        m_tapeSpec->ApplyToTrayScanSpec(*m_trayScanSpec);
    }
...
void VisionUnit::GetAvailableFixedItemList(std::vector<VisionProcessing*>& availableFixedItems)
{
    ...
    availableFixedItems.push_back(m_visionProcessingCarrierTapeAlign);
    availableFixedItems.push_back(m_visionProcessingTapeDetailAlign);
    availableFixedItems.push_back(m_visionInspection2DMatrix);

    if (m_main.m_mainType == TR_EXECUTE_IN_POCKET)
    {
        // InPocket
        availableFixedItems.push_back(m_visionInspectionInPocket);
    }
    else
    {
        // OTI
        availableFixedItems.push_back(m_visionInspectionOTITR);
    }
...
const VisionTapeSpec* VisionUnit::getTapeSpec() const
{
    return m_tapeSpec;
}
...
VisionTapeSpec* VisionUnit::getTapeSpec()
{
    return m_tapeSpec;
}
