...
DlgVisionIllumSetup2D::DlgVisionIllumSetup2D(
    CVisionMain& visionMain, VisionUnit& visionUnit, CWnd* pParent, const enSideVisionModule i_eSideVisionModule)
    ...
    if (auto* category = m_propertyGrid->AddCategory(_T("Illuminations")))
    {
        ...
        switch (SystemConfig::GetInstance().GetVisionType())
        {
            ...
            case VISIONTYPE_SWIR_INSP:
                UseChannelCount = LED_ILLUM_CHANNEL_SWIR_DEFAULT; //kircheis_SWIR //NeedRefac
                break;
        }
...
BOOL DlgVisionIllumSetup2D::UpdateIllumChSetupUIforSWIR(long nFrame)
{
    m_propertyGrid->SetRedraw(FALSE);
    m_propertyGrid->ResetContent();
    ...
    {
        LPCTSTR channelNames[] = {
            _T("CH01 (1200nm) Layer1"),
            _T("CH02 (1200nm) Layer1"),
            _T("CH03 (1200nm) Layer1"),
            _T("CH04 (1200nm) Layer1"),
            _T("CH05 (1200nm) Layer1"),
            _T("CH06 (1650nm) Layer2"),
            _T("CH07 (1650nm) Layer2"),
            _T("CH08 (1650nm) Layer2"),
            _T("CH09 (1650nm) Layer2"),
            _T("CH10 (1200nm) Layer3"),
            _T("CH11 (1200nm) Layer3"),
            _T("CH12 (1200nm) Layer3"),
            _T("CH13 (1650nm) Coax.1"),
            _T("CH14 (1200nm) Coax.2"),
            _T("CH15"),
            _T("CH16"),
        };
        ...
        long UseChannelCount = LED_ILLUM_CHANNEL_SWIR_DEFAULT;
...
void DlgVisionIllumSetup2D::SetIllumiParameter(long nFrame)
{
    ...
    if (SystemConfig::GetInstance().IsNgrvSwirOptics() == true || SystemConfig::GetInstance().IsVisionTypeSWIR() == TRUE)
    {
        if (UpdateIllumChSetupUIforSWIR(nFrame) == FALSE)
            return;
    }
    else
    {
    ...
    if (wparam == XTP_PGN_ITEMVALUE_CHANGED)
    {
        ...
        else if (value)
        {
            ...
            if(SystemConfig::GetInstance().IsNgrvSwirOptics() == true)
                UpdateIllumChSetupUIforSWIR(m_nCurrentFrameIndex);
            else
...
void DlgVisionIllumSetup2D::GrabAndDisplayColorImage() //kircheis_WB
{
    ...
    const int& idx_camera = (int)m_eCurVisionModule;
    ...
    for (long nColor = COLOR_BLUE; nColor <= COLOR_RED; nColor++)
    {
        ...
        FrameGrabber::GetInstance().StartGrab2D(idx_grabber, idx_camera, &imageMems[nColor], 1, nullptr);
