...
//
void CVisionMain::SendVersionInfo()
{
    long nID = 0;
    DEFMSGPARAM Data;
    Data.P1 = -1;
...
void CVisionMain::iPIS_Send_ImageSize(long nSendImageSizeX, long nSendImageSizeY)
{
...
void CVisionMain::iPIS_Send_Image(const Ipvm::Image8u& image)
{
...
void CVisionMain::iPIS_Send_Image(const Ipvm::Image8u& imageFront, const Ipvm::Image8u& imageRear)
{
...
void CVisionMain::IPIS_Send_NGRV_Image(const Ipvm::Image8u3& image)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_2DAllFrame()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_Job(CiDataBase& jobDB, long nMsg) //CJobFile* 를 전송을 하는 모든 일을 처리함.
{
...
void CVisionMain::iPIS_Send_PackageSpec()
{
    static const bool bIs380Bridge = SystemConfig::GetInstance().GetHandlerType() == HANDLER_TYPE_380BRIDGE;
...
void CVisionMain::iPIS_Send_InspResult(VisionUnit& visionUnit, VisionDeviceResult& CurResult, BOOL& bSendSuccess)
{
...
void CVisionMain::iPIS_Send_InspResult(VisionUnit& visionUnit, SideVisionDeviceResult& CurResult, BOOL& bSendSuccess)
{
    DevelopmentLog::AddLog(
        DevelopmentLog::Type::TCP, _T("Send Insp Result : T%d, P%d"), CurResult.m_nTrayID, CurResult.m_nPocketID);
...
void CVisionMain::iPIS_Send_DLInspResult(VisionDeviceResult& CurResult, BOOL& bSendSuccess)
{
    DevelopmentLog::AddLog(
...
void CVisionMain::iPIS_Send_SystemParameters()
{
    static const bool bIs380Bridge = SystemConfig::GetInstance().GetHandlerType() == HANDLER_TYPE_380BRIDGE;
...
void CVisionMain::iPIS_Send_SystemParametersMatchResult(VisionSystemParameters& receivedSysParam)
{
    static const bool bIs380Bridge = SystemConfig::GetInstance().GetHandlerType() == HANDLER_TYPE_380BRIDGE;
...
VisionSystemParameters CVisionMain::MakeVisionSystemParameters()
{
    ...
    const static long nVisionType = systemConfig.GetVisionType(); //kircheis_SWIR
    ...
    const static bool bIsSWIRVision = (nVisionType == VISIONTYPE_SWIR_INSP); //kircheis_SWIR
    ...
    if (bIs2DVision == false && bIs3DVision == false && bIsNGRV == false && bIsSideVision == false
        && bIsSWIRVision == false) //kircheis_SWIR
    {
        CString strError;
...
VisionSystemParameters CVisionMain::MakeVisionSystemParametersFor2DandSide()
{
    VisionSystemParameters visionSysParams;
    auto& systemConfig = SystemConfig::GetInstance();
    ...
    static const bool bIsValidVisionType = (bIs2DVision || bIsSideVision || bIsSWIRVision); //kircheis_SWIR
...
void CVisionMain::AddVisionSystemParametersFor2DIllum(const BOOL bIsRef, VisionSystemParameters& o_visionParams)
{
    auto& systemConfig = SystemConfig::GetInstance();
    static const bool bIs2DVision = (systemConfig.GetVisionType() == VISIONTYPE_2D_INSP);
    static const bool bIsSWIRVision = (systemConfig.GetVisionType() == VISIONTYPE_SWIR_INSP); //kircheis_SWIR
    if (systemConfig.GetHandlerType() < HANDLER_TYPE_500I
        || (bIs2DVision == false && bIsSWIRVision == false)) //kircheis_SWIR
        return;
...
void CVisionMain::AddVisionSystemParametersForSideIllum(const BOOL bIsRef, VisionSystemParameters& o_visionParams)
{
    auto& systemConfig = SystemConfig::GetInstance();
    static const bool bIsSideVision = (systemConfig.GetVisionType() == VISIONTYPE_SIDE_INSP);
...
VisionSystemParameters CVisionMain::MakeVisionSystemParametersFor3D()
{
    VisionSystemParameters visionSysParams;
    auto& systemConfig = SystemConfig::GetInstance();
    ...
    //visionSysParams.Add(strFileName, strCategory, _T("Prev Job Name"),								false, systemConfig.m_strPrevJob);
...
VisionSystemParameters CVisionMain::MakeVisionSystemParametersForNGRV() //kircheis_820.190
{
    VisionSystemParameters visionSysParams;
    auto& systemConfig = SystemConfig::GetInstance();
...
CString CVisionMain::GetIllumCalTypeString(long nIllumCalType)
{
...
long CVisionMain::GetIllumCalTypeLong(CString strIllumCalType)
{
...
void CVisionMain::SetVisionSystemParameters(VisionSystemParameters visionSysParams)
{
    auto& systemConfig = SystemConfig::GetInstance();
    if (systemConfig.GetHandlerType() < HANDLER_TYPE_500I)
        return;
            ...
            else if (sysParam.m_strCategory == strCategorySetting)
            {
                ...
                else if (sysParam.m_strKeyName == _T("StitchGrabDirection"))
                {
                    ...
                    && (systemConfig.m_nVisionType == VISIONTYPE_2D_INSP
                        ...
                        || systemConfig.m_nVisionType == VISIONTYPE_SWIR_INSP)) //kircheis_SWIR
                {
...
void CVisionMain::iPIS_Send_SystemParametersChanged(BOOL bChanged)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_ChangeDisplay(short nTarget)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_MultiGrabProbePosZ(long nProbePosZ)
{
    static const bool bIS380Bridge = (SystemConfig::GetInstance().GetHandlerType() == HANDLER_TYPE_380BRIDGE);
...
void CVisionMain::iPis_Send_JobInfo()
{
    auto& primaryUnit = GetPrimaryVisionUnit();
    auto visionInspectionSpecs = primaryUnit.GetVisionInspectionSpecs();
...
void CVisionMain::GetIllumInfo(std::vector<CString>& vecstrIllumExposureTimeMS) //kircheis_MED3_SendInfo
{
    auto& systemConfig = SystemConfig::GetInstance();
    static const long nVisionType = systemConfig.GetVisionType();
...
void CVisionMain::iPIS_Send_Start_Handler(long nMessage)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::SendToHost_MarkImage(std::vector<CString>& vecstrMarkImagePath)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_HandlerStop()
{
    m_pMessageSocket->Write(MSG_PROBE_HANDLER_STOP, 0, NULL);
}

void CVisionMain::iPIS_Send_ErrorMessageForPopup(CString message)
{
    CMemFile memFile;
    ArchiveAllType ar((CFile*)&memFile, ArchiveAllType::store);
...
void CVisionMain::iPIS_Send_ErrorMessageForLog(CString category, CString message)
{
    CMemFile memFile;
    ArchiveAllType ar((CFile*)&memFile, ArchiveAllType::store);
...
void CVisionMain::iPIS_Send_JobdownloadAck(BOOL done, CString message)
{
    CMemFile memFile;
    ArchiveAllType ar((CFile*)&memFile, ArchiveAllType::store);
...
void CVisionMain::iPIS_Send_GrabRetry()
{
    m_pMessageSocket->Write(MSG_PROBE_3D_GRAB_RETRY, NULL, NULL);
}

void CVisionMain::iPIS_Send_GrabMove(long nGrabDir, long nMessage, float scanLength3D_mm, bool isGoldenDevice)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_MarkTeach_Skip()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_MarkTeach_Done()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::SetSpecNormalValue(std::vector<VisionInspectionSpec*>& specList)
{
...
std::vector<float> CVisionMain::GetPackageSizeSpec(CString strSpecName)
{
    auto& primaryUnit = GetPrimaryVisionUnit();
...
std::vector<float> CVisionMain::Get3DInspectionSpec(CString strSpecName)
{
    auto& primaryUnit = GetPrimaryVisionUnit();
    std::vector<float> vecfNominal;
    float fSpec = 0.f;
...
std::vector<float> CVisionMain::GetMarkInspectionSpec(CString strSepcName)
{
    std::vector<float> vecfNominal;
...
std::vector<float> CVisionMain::GetBallInspectionSpec(CString strSpecName)
{
    auto& primaryUnit = GetPrimaryVisionUnit();
...
std::vector<float> CVisionMain::GetBallPixelQualityInspectionSpec(CString strSpecName) //kircheis_BPQ
{
    std::vector<float> vecfNominal;
    float fSpec = 0.f;
...
void CVisionMain::GetSurfaceCriteriaSpec(
...
SurfaceSpec CVisionMain::GetCiriteriaSpec(
...
bool CVisionMain::IsInlineMode() const
{
...
void CVisionMain::iPIS_Send_WarningBTM2DRecipe() //NGRV Lot Start가 수신되었을 때, BTM2D가 NGRV Recipe가 아님을 알림
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_NGRV_GrabZposInfo(std::vector<NgrvGrabPositionInfo>
...
void CVisionMain::iPIS_Send_NGRV_CompleteImageSave(NgrvGrabbedImageInfo
...
void CVisionMain::iPIS_Send_NGRV_GrabFrameInfo(NgrvFrameInfo
...
void CVisionMain::iPIS_Send_PackageGrabInfo_Receive_Done()
{
    m_pMessageSocket->Write(MSG_PROBE_NGRV_PACKAGE_INFO_IN_TRAY_ACK, 0, NULL);
}

void CVisionMain::iPIS_Send_NGRV_AF_Info() //kircheis_NGRVAF //2D Vision만 사용. NGRV AF Setup 정보를 송부
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_NGRV_SingleRunInfo() // NGRV SingleRunInfo를 Host로 송부한다 - JHB_MED#6
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_2D_Vision_Image_Info_To_NGRV() // NGRV에 보낼 2D Vision 정보를 Host로 송부한다 - JHB_MED#6
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
    ...
    if (illum.m_2D_colorFrame == FALSE)
    {
        ...
        resizeImageSizeX = ((imageSizeX / 4 + 3) / 4) * 4; //kircheis_SWIR
        resizeImageSizeY = ((imageSizeY / 4 + 3) / 4) * 4; //kircheis_SWIR
...
void CVisionMain::iPIS_Send_NGRV_2DVisionInfo_Receive_Done()
{
...
void CVisionMain::iPIS_Send_NGRV_RunMode_Receive_Done()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
    ...
    //std::vector<CString> vecStrTxtRecipe = ExportRecipeToText();//kircheis_SWIR
...
void CVisionMain::iPIS_Send_NGRV_SingleRun_Visiontype_Receive_Done()
{
    m_pMessageSocket->Write(MSG_PROBE_NGRV_SINGLE_RUN_VISION_TYPE_INFO_ACK, 0, NULL);
}

void CVisionMain::iPIS_Send_TextRecipe() //kircheis_TxtRecipe
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_VisionLogFile(const CString i_strFileName, const CString i_strFilePath)
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_3D_Vision_StartIO_NotOff()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
...
void CVisionMain::iPIS_Send_VersionMismatchErrorPopup()
{
    if (m_pMessageSocket->IsConnected() == false)
        return;
