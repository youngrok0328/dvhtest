...
#include "../../MainUiModules//VisionBatchInspectionUI/VisionBatchInspectionUI.h"
...
#define DVH_ILLUMINATION_3D_VALUE 255
...
UINT CVisionMain::ThreadReconnectDLSW(LPVOID pParam)
{
    CVisionMain* pMain = (CVisionMain*)pParam;
...
UINT CVisionMain::ThreadExecuteDLSW(LPVOID pParam)
{
    CVisionMain* pMain = (CVisionMain*)pParam;
...
UINT CVisionMain::ThreadSendResultFunc(LPVOID pParam)
{
    CVisionMain* pMain = (CVisionMain*)pParam;
...
void CVisionMain::ThreadSendResult()
{
    BOOL bQuit = FALSE;
...
void CNGRVImageSet::SetImage(long nStitchID, std::vector<Ipvm::Image8u3> vecImageForSave, const BOOL& isNGRV_SingleRun)
{
    ...
    {
        long Realindex = isNGRV_SingleRun == TRUE ? m_vecnFrameID[nFrame] : nFrame;
...
CVisionMain::CVisionMain(SystemConfig& systemConfig)
...
void CVisionMain::TerminateDLSW()
{
...
long CVisionMain::GetThreadNum()
{
    return m_nThreadNum;
}

CVisionMain::~CVisionMain(void)
{
...
LPCTSTR CVisionMain::GetLastLinkDataBaseErrorMessage() const
{
    return m_lastLinkDataBaseErrorMessage;
}

BOOL CVisionMain::GetLastLinkDataBaseJobVersionChanged() const
{
    return m_lastLinkDataBaseJobVersionChanged;
}

bool CVisionMain::isInline() const
{
    return IsShowVisionInlineUI();
}

void CVisionMain::SetInlineStart(bool isBatchInspection)
{
...
void CVisionMain::SetInlineStop(bool isBatchInspection)
{
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
bool CVisionMain::ManualGrab3DSequence(VisionImageLot& imageLot)
{
    if (!m_systemConfig.m_bHardwareExist)
        return false;
    if (m_systemConfig.GetVisionType() != VISIONTYPE_3D_INSP)
        return false;

    const int& idx_grabber = 0;
    const int& idx_camera = 0;
    const bool& save_slitbeam_images = (bool)SystemConfig::GetInstance().m_saveSlitbeamOriginalImage;
    ...
    sGrab3DErrorLogMessageFormat.m_strLotID = _T("Tuning");
    ...
    Ipvm::AsyncProgress progress(_T("Wait start signal from handler"));
    ...
    {
        if (SyncController::GetInstance().GetStartSignal())
        {
    ...
    imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex());
    imageLot.SetRunTimeIndex(m_inline_grab->GetInlineRunCnt());

    imageLot.SetScales(m_systemConfig.GetScale(enSideVisionModule::SIDE_VISIONMODULE_FRONT));

    imageLot.SetScanAreaIndex(0);

    float heightRangeMin = 0.f;
    float heightRangeMax = 0.f;

    if (FrameGrabber::GetInstance().StartGrab3D(idx_grabber, idx_camera, 0, imageLot.GetZmapImage(0),
            imageLot.GetVmapImage(0), heightRangeMin, heightRangeMax, &progress))
    {
        imageLot.Set3DScanInfo(heightRangeMin, heightRangeMax);

        SyncController::GetInstance().SetGrabExposure(TRUE);
        SyncController::GetInstance().SetGrabAcquisition(TRUE);

        if (FrameGrabber::GetInstance().wait_grab_end(idx_grabber, idx_camera, &progress))
        {
            SyncController::GetInstance().SetGrabExposure(FALSE);
            SyncController::GetInstance().SetGrabAcquisition(FALSE);
            SyncController::GetInstance().TurnOffLight(enSideVisionModule::SIDE_VISIONMODULE_FRONT);

            FrameGrabber::GetInstance().wait_calc_end(idx_grabber, idx_camera, save_slitbeam_images, &progress);
            imageLot.UpdateGray3D(0);
        }
        else
        {
            ...
            Send_ManualGrab3D_Sequence_Error_LogMessage(_T("Tuning Grab"), sGrab3DErrorLogMessageFormat);

            DevelopmentLog::AddLog(DevelopmentLog::Type::ImageGrab, _T("LTSM Grab3D - Wait Grab Fail"));
            DevelopmentLog::AddLog(DevelopmentLog::Type::Warning, _T("LTSM Grab3D - Wait Grab Fail"));
        }
    }
    else
    {
        ...
        DevelopmentLog::AddLog(DevelopmentLog::Type::ImageGrab, _T("LTSM Grab3D - StartGrab3D Fail"));
        DevelopmentLog::AddLog(DevelopmentLog::Type::Warning, _T("LTSM Grab3D - StartGrab3D Fail"));
    }
...
void CVisionMain::ShowVisionInspSpec()
{
    ...
    {
        CDlgVisionInspSpecNGRV DlgVisionInspSpecNGRV((CVisionMain*)this); //kircheis_NGRV

        DlgVisionInspSpecNGRV.DoModal();

        primaryUnit.UpdateInspItem(DlgVisionInspSpecNGRV.m_availableFixedItemsBTM2D, enPackageType::UNKNOWN, FALSE);
    }

    SynchronizeJobFile();
}

void CVisionMain::ShowOverlayManager()
{
    // 이 코드가 없으면 Dialog 가 보이지 않음
    AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CVisionMain::ShowIlluminationTeachUI(HWND hwndParent)
{
    delete m_menuAccessUI;
...
void CVisionMain::ShowVisionPrimaryUI(HWND hwndParent)
{
    m_pVisionPrimaryUI->ShowVisionPrimaryDlg(hwndParent);
}

void CVisionMain::HideVisionPrimaryUI()
{
    m_pVisionPrimaryUI->HideVisionPrimaryDlg();
}

void CVisionMain::ShowBatchInspectionUI(HWND hwndParent)
{
    delete m_menuAccessUI;
...
void CVisionMain::SetFileVersion(CString strFileVersion)
{
    m_strDetailFileVersion = strFileVersion;

    m_pVisionPrimaryUI->Dlg_SetLabel_Version2(strFileVersion);
}

CString CVisionMain::GetFileVersion()
{
    return m_strDetailFileVersion;
}

// 영훈 20150512_DeleteMarkImages : Inline Start시 Job에 저장된 Mark 이미지를 모두 지우고 시작하도록 한다.
void CVisionMain::DeleteMarkMultiImages()
{
    for (long nThread = 0; nThread <= m_nThreadNum; nThread++)
    {
...
void CVisionMain::ReturnToPrimaryUI(BOOL bCallLinkDatabase)
{
    CloseAllUI();
...
void CVisionMain::CloseProcessingTeachingUI(BOOL bSendToHostJobMode)
{
    ...
    if (nSize)
    {
        ...
        // Job name 폴더 생성
        strFilePath += m_strJobFileName + _T("\\");
        ::CreateDirectory(strFilePath, NULL);
        ...
        int nImageSizeX = VisionreuseableMemory.GetInspImageSizeX();
        int nImageSizeY = VisionreuseableMemory.GetInspImageSizeY();
        ...
        for (long n = 0; n < nSize; n++)
        {
            ...
            for (long nMultiMarkID = 0; nMultiMarkID < NUM_OF_MARKMULTI; nMultiMarkID++)
            {
                if (pMarkInspection->m_VisionSpec[nMultiMarkID]->m_bTeachDone)
                {
                    ...
                    {
                        rtCharROI = Ipvm::Rect32s(pMarkInspection->m_VisionSpec[nMultiMarkID]->m_plTeachCharROI[nChar]
        ...
        for (long n = 0; n < nSize; n++)
        {
            ...
            Ipvm::ImageProcessing::RotateLinearInterpolation(imageMark, rtTotalTeachROI, center,
                -fAngle_rad * ITP_RAD_TO_DEG, Ipvm::Point32r2(0.f, 0.f), imageRotate);
            ...
            for (long nMultiMarkID = 0; nMultiMarkID < NUM_OF_MARKMULTI; nMultiMarkID++)
            {
                if (pMarkInspection->m_VisionSpec[nMultiMarkID]->m_bSaveTeachImage)
                {
                    ...
                    Ipvm::ImageFile::SaveAsBmp(Ipvm::Image8u(imageRotate, rtTotalTeachROI), vecstrFilePath[0]);
                    Ipvm::ImageFile::SaveAsBmp(Ipvm::Image8u(imageThreshold, rtTotalTeachROI), vecstrFilePath[1]);
...
bool CVisionMain::IsShowProcessingTeachingUI()
{
    return GetPrimaryVisionUnit().IsWindowVisible();
}

void CVisionMain::InitializeVisionInlineUI(HWND hwndParent)
{
...
bool CVisionMain::IsShowVisionInlineUI() const
{
...
void CVisionMain::SetColorImageToInlineUIforNGRV(Ipvm::Image8u3 image) //kircheis_NGRVINLINE
{
...
void CVisionMain::SendToHostMarkTeachImage() //kircheis_AutoTeach
{
...
void CVisionMain::CloseAccessUI()
{
    if (m_menuAccessUI)
    {
...
void CVisionMain::ShowProcessingTeachingUI(HWND hwndParent)
{
...
void CVisionMain::SaveDumpImageFiles(LPCTSTR dumpPath, LPCTSTR fileVersion)
{
...
BOOL CVisionMain::SaveJobFile(CString strFilename, SaveJobMode mode, BOOL bSendJob, BOOL bSendPackageSpec)
{
...
BOOL CVisionMain::SynchronizeJobFile(BOOL bCallLinkDatabase)
{
...
BOOL CVisionMain::OpenJobFile(CString strFilePath, CString strFileName)
{
    ...
    if (bValid)
    {
        ...
        m_systemConfig.SetPrevJob(strFilePath);
        OnJobChanged();
    }
...
void CVisionMain::SetVisionJobName(CString strFilePath, CString strFileName)
{
...
BOOL CVisionMain::LinkDataBase(BOOL bSave, CiDataBase& jobDB)
{
    ...
    //{{ //Recipe에 저장된 Vision Type과 현재 Vision의 Vision Type을 검증한다.
    CString errorMessageMatchedVisionType;
    bool isMatchedVisionType = CheckVisionTypeInRecipe(bSave, jobDB, errorMessageMatchedVisionType);
    if (bSave == FALSE && isMatchedVisionType == false)
    {
    ...
    //{{//kircheis_NGRV
    if (SystemConfig::GetInstance().IsVisionTypeNGRV() == TRUE && SystemConfig::GetInstance().IsNgrvColorOptics() == true)
    {
    ...
    {
        m_lastLinkDataBaseErrorMessage = _T("Failed to load Vision Inspection Parameters.");
        return FALSE;
    }
    ...
    if (!bSave)
    {
        for (long nThread = 0; nThread < m_nThreadNum; nThread++)
        {
            ...
            {
                m_lastLinkDataBaseErrorMessage = _T("Failed to load Vision Inspection Parameters in Thread Buffer.");
                return FALSE;
            }
...
BOOL CVisionMain::NGRV_FrameInfoLinkDataBase(
...
BOOL CVisionMain::NGRV_2DImageInfoLinkDataBase(BOOL bSave, CiDataBase& JobDB)
{
    auto& mainDB = JobDB[_T("VisionMain")];
    CString strKeyName;
...
BOOL CVisionMain::NGRV_AFInfoLinkDataBase(BOOL bSave, CiDataBase& JobDB)
{
    auto& mainDB = JobDB[_T("VisionMain")];
    CString strKeyName;
...
BOOL CVisionMain::LinkDataBase_PackageSpec(BOOL bSave, CiDataBase& jobDB)
{
    return GetPrimaryVisionUnit().LinkDataBase_PackageSpec(bSave, jobDB);
}

BOOL CVisionMain::OpenJobFileAgent()
{
    return OpenJobFile();
}

BOOL CVisionMain::OpenImageFile()
{
    return GetPrimaryVisionUnit().OpenImageFile();
}

void CVisionMain::SendDLResult(VisionDeviceResult& DeviceResult, BOOL& bSendSuccess)
{
    iPIS_Send_DLInspResult(DeviceResult, bSendSuccess);
}

void CVisionMain::SendResult(VisionUnit& visionUnit, VisionDeviceResult& DeviceResult, BOOL& bSendSuccess)
{
...
void CVisionMain::SendResult(VisionUnit& visionUnit, SideVisionDeviceResult& DeviceResult, BOOL& bSendSuccess)
{
    iPIS_Send_InspResult(visionUnit, DeviceResult, bSendSuccess);
}

void CVisionMain::SendImage(const Ipvm::Image8u& image) //kircheis_UPH_up
{
    iPIS_Send_Image(image);
}

void CVisionMain::SendImage(const Ipvm::Image8u& imageFront, const Ipvm::Image8u& imageRear) //Side Vision Inline 전송용
{
    iPIS_Send_Image(imageFront, imageRear);
}

void CVisionMain::Send2DAllFrame() //mc_MED#3.5 AllFrameSave
{
    iPIS_Send_2DAllFrame();
}

bool CVisionMain::isConnect() const //kircheis_UPH_up
{
    return m_pMessageSocket->IsConnected();
}

long CVisionMain::GetInlineRunCnt() const
{
    return m_inline_grab->GetInlineRunCnt();
}

void CVisionMain::SetInlineRunCnt(long inlineRunCnt)
{
    m_inline_grab->SetInlineRunCnt(inlineRunCnt);
}

void CVisionMain::SetScanTrayIndex(long trayIndex)
{
    m_inline_grab->SetScanTrayIndex(trayIndex);
}

void CVisionMain::SetInspectionRepeatIndex(long repeatIndex)
{
    m_nInspectionRepeatIndex = repeatIndex;
}

long CVisionMain::GetInspectionRepeatIndex()
{
    return m_nInspectionRepeatIndex;
}

void CVisionMain::SetScanSpec(float scanLength_mm)
{
    m_pSlitBeam3DCommon->m_scanLength_mm = scanLength_mm;
}

void CVisionMain::SetTrayScanSpec(const VisionTrayScanSpec& spec)
{
    for (long nThread = 0; nThread <= m_nThreadNum; nThread++)
    {
...
void CVisionMain::Iocheck()
{
    SyncController::GetInstance().PopupDioCheckDialog();
}

BOOL CVisionMain::VisionPrimaryUI_SaveImageFile()
{
    if (m_menuAccessUI)
    {
...
//  현식 마크티치관련 수정
void CVisionMain::OnMarkTeachDone()
{
    if (m_bMarkTeach)
    {
...
void CVisionMain::OnMarkTeachSkip()
{
    iPIS_MarkTeach_Skip();
}

void CVisionMain::OnImageGrab()
{
    if (GetAsyncKeyState(VK_MENU) < 0)
    {
        AFX_MANAGE_STATE(AfxGetStaticModuleState());
        CDlgSub2DVisionTune tune(nullptr, SystemConfig::GetInstance().GetVisionType());
    ...
    if (m_systemConfig.GetVisionType() == VISIONTYPE_2D_INSP
        || m_systemConfig.GetVisionType() == VISIONTYPE_SWIR_INSP) //kircheis_SWIR // NeedRefac
    {
        ManualGrab2D(imageLot, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
    }
    else if (m_systemConfig.GetVisionType() == VISIONTYPE_SIDE_INSP)
    {
        for (long nSideidx = enSideVisionModule::SIDE_VISIONMODULE_START;
            ...
            ManualGrab2D(imageLot, enSideVisionModule(nSideidx));
    }
    else if (m_systemConfig.GetVisionType() == VISIONTYPE_3D_INSP)
    {
        bool bImagetestResult = true;
        ...
        if (SystemConfig::GetInstance().GetUse3DCameraNoiseTest() == true)
        {
            ...
            {
                bImagetestResult = Check3DNoise(imageLot, true);
            }
            // CTRL만 누를 경우에는 기존 사이즈 그대로 테스트 진행
            else if (GetAsyncKeyState(VK_CONTROL) < 0)
            {
                bImagetestResult = Check3DNoise(imageLot, false);
            }

            // 오버레이로 결과 표시
            CString strTestResult;

            if (bImagetestResult == false)
            {
                strTestResult.Format(_T("Camera check fail"));

                auto imageView = m_pVisionPrimaryUI->m_pDlg->m_imageLotView->GetCoreView();
                imageView->WindowOverlayAdd(Ipvm::Point32s2(10, 60), CStringW(strTestResult), RGB(255, 255, 0), 20L);
            }
            else
            {
                strTestResult.Format(_T("Camera check pass"));
            }

            auto imageView = m_pVisionPrimaryUI->m_pDlg->m_imageLotView->GetCoreView();
            imageView->WindowOverlayClear();
            imageView->WindowOverlayAdd(Ipvm::Point32s2(10, 60), CStringW(strTestResult), RGB(255, 255, 0), 20L);
            imageView->WindowOverlayShow();
            return;
        }
        else
        {
            ...
            {
                ManualDualGrab3D(imageLot);
            }
            else
            {
                ManualGrab3D(imageLot);
            }
        }
    }
    else
    {
        ManualGrabNGRV(imageLot);
    }

    m_pVisionPrimaryUI->ShowCurrentImage(true);
}

bool CVisionMain::ManualGrab2D(VisionImageLot& imageLot, const enSideVisionModule i_eSideVisionModule)
{
    const int& idx_grabber = 0;
    ...
    for (long imageIndex = 0; imageIndex < imageLot.GetImageFrameCount(i_eSideVisionModule); imageIndex++)
    {
    ...
    imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), i_eSideVisionModule);
    imageLot.SetRunTimeIndex(m_inline_grab->GetInlineRunCnt(), i_eSideVisionModule);
    imageLot.SetScales(m_systemConfig.GetScale(i_eSideVisionModule));

    imageLot.SetScanAreaIndex(scanAreaIndex, i_eSideVisionModule);
    ...
    FrameGrabber::GetInstance().StartGrab2D(idx_grabber, idx_camera, &vecBufferMem[0], nNormalGrabCount, nullptr);
    ...
    if (nHighPosGrabCount > 0)
    {
        Sleep(300);

        FrameGrabber::GetInstance().StartGrab2D(
            idx_grabber, idx_camera, &vecBufferMem[nNormalGrabCount], nHighPosGrabCount, nullptr);
    ...
    imageLot.Paste2D(stitchGrabIndex, &vecBufferMem[0], (long)vecBufferMem.size(), 1, 1, i_eSideVisionModule);
    GetPrimaryVisionUnit().getImageLotInsp().Set(imageLot, i_eSideVisionModule);
...
bool CVisionMain::ManualGrab3D(VisionImageLot& imageLot)
{
    const int& idx_grabber = 0;
    const int& idx_camera = 0;
    const bool& save_slitbeam_images = (bool)SystemConfig::GetInstance().m_saveSlitbeamOriginalImage;
    ...
    iPIS_Send_GrabMove(0, MSG_PROBE_3DVISION_GRAB_MOVE);
    ...
    imageLot.SetScanAreaIndex(scanAreaIndex);
            ...
            imageLot.GetVmapImage(0), heightRangeMin, heightRangeMax, &progress))
    {
        ...
        if (FrameGrabber::GetInstance().wait_grab_end(idx_grabber, idx_camera, &progress))
        {
            ...
            imageLot.UpdateGray3D(stitchGrabIndex);
        }
    }
...
bool CVisionMain::ManualDualGrab3D(VisionImageLot& imageLot)
{
    const int& n_grabber1 = 0;
    const int& n_camera1 = 0;

    const int& n_grabber2 = 1;
    const int& n_camera2 = 0;
    ...
    //SyncController::GetInstance().SetilluminationMode(m_pSlitBeam3DCommon->m_cameraGrabMode); // 조명 모드 설정
    //SyncController::GetInstance().SetilluminationValue(DVH_ILLUMINATION_3D_VALUE); // 3D 조명 밝기 값 설정
    SyncController::GetInstance().TurnOnLight(0, SystemConfig::GetInstance().IsDual3DGrabMode());
    SyncController::GetInstance().SetReadySignal(FALSE);
    SyncController::GetInstance().SetGrabDirection(
        CAST_BYTE(SystemConfig::GetInstance().m_nGrabDirection), SystemConfig::GetInstance().IsDual3DGrabMode());
    ...
    bool bWaitGrabEnd = false;
    bool bWaitCalcEnd = false;

    if (m_pSlitBeam3DCommon->m_cameraGrabMode == VISION_3D_GRABMODE_SPECULAR)
    {
        imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        imageLot.SetRunTimeIndex(
            m_inline_grab->GetInlineRunCnt(), (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        imageLot.SetScales(m_systemConfig.GetScale(enSideVisionModule::SIDE_VISIONMODULE_FRONT),
            (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        imageLot.SetScanAreaIndex(scanAreaIndex, (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);

        // Grab Start
        if (FrameGrabber::GetInstance().StartGrab3D(n_grabber1, n_camera1, 0,
                imageLot.GetZmapImage((int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION),
                imageLot.GetVmapImage((int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION), heightRangeMin,
                heightRangeMax, &progress))
        {
            imageLot.Set3DScanInfo(
                heightRangeMin, heightRangeMax, (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);

            SyncController::GetInstance().SetGrabExposure(TRUE);
            SyncController::GetInstance().SetGrabAcquisition(TRUE);

            if (FrameGrabber::GetInstance().wait_grab_end(n_grabber1, n_camera1, &progress))
            {
                SyncController::GetInstance().SetGrabExposure(FALSE);
                SyncController::GetInstance().SetGrabAcquisition(FALSE);
                SyncController::GetInstance().TurnOffLight(SystemConfig::GetInstance().IsDual3DGrabMode());

                FrameGrabber::GetInstance().wait_calc_end(n_grabber1, n_camera1, save_slitbeam_images, &progress);

                imageLot.UpdateGray3D(stitchGrabIndex);

                Ipvm::ImageFile::SaveAsBmp(
                    imageLot.GetImageFrame(0, 0), _T("D:\\DebugImage\\1.DUAL_IMAGE_TOTAL_REFLECTION.bmp"));

                Ipvm::ImageFile::SaveAsBmp(
                    imageLot.GetImageFrame(1, 0), _T("D:\\DebugImage\\2.DUAL_IMAGE_TOTAL_REFLECTION_V.bmp"));
            }
        }
    ...
    else if (m_pSlitBeam3DCommon->m_cameraGrabMode == VISION_3D_GRABMODE_DEFUSED)
    {
        // Image-1
        imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        imageLot.SetRunTimeIndex(
        ...
        imageLot.SetScanAreaIndex(scanAreaIndex, (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        // Image-2
        imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), (int)enDualGrabImageType::DUAL_IMAGE_DIFFUSE_REFLECTION);
        imageLot.SetRunTimeIndex(
            m_inline_grab->GetInlineRunCnt(), (int)enDualGrabImageType::DUAL_IMAGE_DIFFUSE_REFLECTION);
        imageLot.SetScales(m_systemConfig.GetScale(enSideVisionModule::SIDE_VISIONMODULE_FRONT),
            (int)enDualGrabImageType::DUAL_IMAGE_DIFFUSE_REFLECTION);
        imageLot.SetScanAreaIndex(scanAreaIndex, (int)enDualGrabImageType::DUAL_IMAGE_DIFFUSE_REFLECTION);

        // Grab Start
        bGrabStart
            = FrameGrabber::GetInstance().StartDualGrab3D(0, imageLot, heightRangeMin, heightRangeMax, &progress);
        ...
        {
            imageLot.UpdateGray3D(stitchGrabIndex);
            imageLot.UpdateGray3D(stitchGrabIndex, 1);
            ...
            Ipvm::ImageFile::SaveAsBmp(
                imageLot.GetImageFrame(0, 1), _T("D:\\DebugImage\\3.DUAL_IMAGE_DIFFUSE_REFLECTION.bmp"));

            Ipvm::ImageFile::SaveAsBmp(
                imageLot.GetImageFrame(1, 1), _T("D:\\DebugImage\\4.DUAL_IMAGE_DIFFUSE_REFLECTION_V.bmp"));
        }

        /*if (bGrabStart)
        {
            bool bWaitGrabEndCam1 = false;
            bool bWaitGrabEndCam2 = false;
            bWaitGrabEndCam1 = FrameGrabber::GetInstance().wait_grab_end(n_grabber1, n_camera1, &progress);
            bWaitGrabEndCam2 = FrameGrabber::GetInstance().wait_grab_end(n_grabber2, n_camera2, &progress);

            if (bWaitGrabEndCam1 && bWaitGrabEndCam2)
                bWaitGrabEnd = true;
            ...
            if (bWaitGrabEnd)
            {
                ...
                bool bWaitCalcEnd1 = false;
                bool bWaitCalcEnd2 = false;

                bWaitCalcEnd1
                    = FrameGrabber::GetInstance().wait_calc_end(n_grabber1, n_camera1, save_slitbeam_images, &progress);
                bWaitCalcEnd2
                    = FrameGrabber::GetInstance().wait_calc_end(n_grabber2, n_camera2, save_slitbeam_images, &progress);

                if (bWaitCalcEnd1 && bWaitCalcEnd2)
                    bWaitCalcEnd = true;
                ...
                if (bWaitCalcEnd)
                {
                    ...
                    Ipvm::ImageFile::SaveAsBmp(
                        imageLot.GetImageFrame(0, 0),
                        _T("D:\\DebugImage\\1.DUAL_IMAGE_TOTAL_REFLECTION.bmp"));

                    Ipvm::ImageFile::SaveAsBmp(
                        imageLot.GetImageFrame(1, 0),
                        _T("D:\\DebugImage\\2.DUAL_IMAGE_TOTAL_REFLECTION_V.bmp"));

                    Ipvm::ImageFile::SaveAsBmp(
                        imageLot.GetImageFrame(0, 1),
                        _T("D:\\DebugImage\\3.DUAL_IMAGE_DIFFUSE_REFLECTION.bmp"));

                    Ipvm::ImageFile::SaveAsBmp(
                        imageLot.GetImageFrame(1, 1),
                        _T("D:\\DebugImage\\4.DUAL_IMAGE_DIFFUSE_REFLECTION_V.bmp"));
                }
            }
        }*/
    ...
    else if (m_pSlitBeam3DCommon->m_cameraGrabMode == VISION_3D_GRABMODE_INTEGRATED)
    {
        // Image-1
        imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), (int)enDualGrabImageType::DUAL_IMAGE_TOTAL_REFLECTION);
        imageLot.SetRunTimeIndex(
        ...
        imageLot.SetScanAreaIndex(scanAreaIndex, (int)enDualGrabImageType::DUAL_IMAGE_DIFFUSE_REFLECTION);
        // Image-3
        imageLot.SetTrayIndex(m_inline_grab->GetTrayIndex(), (int)enDualGrabImageType::DUAL_IMAGE_COMPLEX_REFLECTION);
        imageLot.SetRunTimeIndex(
            m_inline_grab->GetInlineRunCnt(), (int)enDualGrabImageType::DUAL_IMAGE_COMPLEX_REFLECTION);
        imageLot.SetScales(m_systemConfig.GetScale(enSideVisionModule::SIDE_VISIONMODULE_FRONT),
            (int)enDualGrabImageType::DUAL_IMAGE_COMPLEX_REFLECTION);
        imageLot.SetScanAreaIndex(scanAreaIndex, (int)enDualGrabImageType::DUAL_IMAGE_COMPLEX_REFLECTION);
        ...
        if (bGrabStart)
        {
            imageLot.UpdateGray3D(stitchGrabIndex);
            imageLot.UpdateGray3D(stitchGrabIndex, 1);
            imageLot.UpdateGray3D(stitchGrabIndex, 2);
            ...
            Ipvm::ImageFile::SaveAsBmp(
                imageLot.GetImageFrame(0, 2), _T("D:\\DebugImage\\5.DUAL_IMAGE_COMPLEX_REFLECTION.bmp"));

            Ipvm::ImageFile::SaveAsBmp(
                imageLot.GetImageFrame(1, 2), _T("D:\\DebugImage\\6.DUAL_IMAGE_COMPLEX_REFLECTION_V.bmp"));
        }
...
bool CVisionMain::ManualGrabNGRV(VisionImageLot& imageLot)
{
    auto& visionReusableMemory = *GetPrimaryVisionUnit().m_VisionReusableMemory;

    //{{ 메뉴얼 Grab시 총 Frame 개수와 Buffer 할당 개수를 맞춰줘야 함 - JHB_NGRV
    //BOOL bUseIR = FALSE;
    //long nGrabItemSize = (long)m_vecGrabItemFrameInfo.size();

    const static long nTotalFrameCount = 0;
    const static long nNormalWhiteFrameCount = 8;

    std::vector<long> vecnNormalWhiteFrameID(8);
    for (long i = 0; i < nNormalWhiteFrameCount; i++)
        vecnNormalWhiteFrameID[i] = i;

    std::vector<long> vecnUVFrameID(1);
    vecnUVFrameID[0] = NGRV_FrameID_UV;
    std::vector<long> vecnIRFrameID(1);
    vecnIRFrameID[0] = NGRV_FrameID_IR;

    HardwareSetupForNGRV(vecnNormalWhiteFrameID, FALSE, TRUE); // NGRV H/W Setup
    //}}
    ...
    ///////////////////////////////////////////
    // 아래 해당 부분 NGRV용으로 수정 필요 - JHB_NGRV
    long scanAreaIndex = 0;
    long stitchGrabIndex = 0;
    ...
    //if (m_isNGRVSingleRun == TRUE) // SingleRun일 경우, IR/UV Grab 안함
    //{
    //    FrameGrabber::GetInstance().StartGrabNGRV(&vecBufferMem[0], (short)nNormalWhiteFrameCount, nullptr);
    ...
    //else
    {
        ...
        FrameGrabber::GetInstance().StartGrab2D(
            idx_grabber, idx_camera, &vecBufferMem[0], nNormalWhiteFrameCount, nullptr);
        ...
        HardwareSetupForNGRV(vecnUVFrameID, FALSE, TRUE); // NGRV H/W Setup

        FrameGrabber::GetInstance().StartGrab2D(idx_grabber, idx_camera, &vecBufferMem[NGRV_FrameID_UV], 1, nullptr);
        SyncController::GetInstance().StartSyncBoard(FALSE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
        FrameGrabber::GetInstance().wait_grab_end(idx_grabber, idx_camera, nullptr);

        //IR Frame Grab
        FrameGrabber::GetInstance().SetDigitalGainValue(SystemConfig::GetInstance().m_fIR_Camera_Gain, FALSE);
        HardwareSetupForNGRV(vecnIRFrameID, FALSE, TRUE); // NGRV H/W Setup

        FrameGrabber::GetInstance().StartGrab2D(idx_grabber, idx_camera, &vecBufferMem[NGRV_FrameID_IR], 1, nullptr);
        SyncController::GetInstance().StartSyncBoard(FALSE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
        FrameGrabber::GetInstance().wait_grab_end(idx_grabber, idx_camera, nullptr);
    ...
    imageLot.PasteNGRV(stitchGrabIndex, &vecBufferMem[0], (long)vecBufferMem.size(), 1, 1,
        NGRV_FrameID_Reverse); // Grab 도중에 Color conversion을 하면 시간이 오래 걸리니, Grab을 다 한 후에 conversion 진행
    GetPrimaryVisionUnit().getImageLotInsp().Set(imageLot, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
...
bool CVisionMain::Check3DNoise(VisionImageLot& imageLot, bool bFullSizeInsp)
{
    UNREFERENCED_PARAMETER(imageLot);

    auto& visionReusableMemory = *GetPrimaryVisionUnit().m_VisionReusableMemory;

    bool bResult = true;

    // 비활성화 될 경우 true를 날려준다.
    if (SystemConfig::GetInstance().GetUse3DCameraNoiseTest() == false)
    {
        return true;
    }
    ...
    // 여러장 찍을수도 있으므로 일단 for loop의 구조를 취한다.
    for (long imageIndex = 0; imageIndex < 1; imageIndex++)
    {
        vecBufferMem.push_back(visionReusableMemory.GetByteMemory());
    }
    ...
    // 이미지를 표시한다.
    Ipvm::Image8u grabImage;
    bResult = FrameGrabber::GetInstance().Check3DNoise(0, 0, &vecBufferMem[0], grabImage, bFullSizeInsp);
    //m_pVisionPrimaryUI->m_pDlg->m_imageLotView->SetImage(grabImage, _T(""));
    ...
    if (bResult == false)
    {
        // 검사 실패시 경고 메세지 출력 및 로그 기록
        CString strMessage;
        strMessage.Format(_T("Camera noise detected.\nPlease check live image of camera"));
        DevelopmentLog::AddLog(DevelopmentLog::Type::Warning, _T("Camera noise check failed"));
        iPIS_Send_ErrorMessageForLog(_T("Camera Setting"), strMessage);
...
void CVisionMain::ResetLiveMain()
{
    m_pVisionPrimaryUI->ResetLiveMain();
}

void CVisionMain::OnGotoHost()
{
    iPIS_ChangeDisplay(0);
}

void CVisionMain::SetLiveMain()
{
    m_pVisionPrimaryUI->SetLiveMain();
}

void CVisionMain::OnLogin()
{
    m_pVisionPrimaryUI->OnLogin();
}

BOOL CVisionMain::VisionPrimaryUI_OpenJobFile()
{
    if (m_menuAccessUI)
    {
...
BOOL CVisionMain::VisionPrimaryUI_SaveJobFile()
{
    if (m_menuAccessUI)
    {
...
BOOL CVisionMain::VisionPrimaryUI_OpenImageFile()
{
    if (m_menuAccessUI)
    {
...
BOOL CVisionMain::OpenImageFile(CString strFileName)
{
    return GetPrimaryVisionUnit().OpenImageFile(strFileName);
}

BOOL CVisionMain::OpenImageFileAgent()
{
    return OpenImageFile();
}

BOOL CVisionMain::OpenImageFileAgent(CString strFileName)
{
    return OpenImageFile(strFileName);
}

BOOL CVisionMain::SetPackageType()
{
...
CString CVisionMain::GetPrevJob()
{
    return m_systemConfig.GetPrevJob();
}

void CVisionMain::ReviewImageViewer()
{
    m_pVisionPrimaryUI->ReviewImageViewer();
}

void CVisionMain::SystemSetup()
{
    //iPIS_Send_TextRecipe();//kircheis_TxtRecipeDebug
    m_pVisionPrimaryUI->SystemSetup();
}

CWnd* CVisionMain::GetVisionInlineDialog(long nSelect)
{
    if (m_pVisionInlineUI == nullptr || m_nThreadNum == 0)
        return (CWnd*)false;
...
BOOL CVisionMain::OnJobChanged()
{
    ...
    static const BOOL b2DIllum
        = ((systemConfig.GetVisionType() == VISIONTYPE_2D_INSP || systemConfig.GetVisionType() == VISIONTYPE_NGRV_INSP
               || systemConfig.GetVisionType() == VISIONTYPE_SIDE_INSP
               || systemConfig.GetVisionType() == VISIONTYPE_SWIR_INSP) //kircheis_SWIR
...
BOOL CVisionMain::HardwareSetup()
{
    ...
    switch (nVisionType)
    {
        ...
        {
            SyncController::GetInstance().SetTriggerEdgeMode(
                TRUE, TRUE, TRUE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
            ...
            SyncController::GetInstance().SetFrameCount(
                nTotalFrameCount - nHighPosGrabCount, nHighPosGrabCount, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
            ...
            for (int i = 0; i < nTotalFrameCount; i++)
            {
                SyncController::GetInstance().SetIllumiParameter(
                    i, m_illum2D.getIllum(i), true, false, FALSE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
            ...
            SyncController::GetInstance().SetFramePeriod(
                CAST_FLOAT(max(nDuration, nMaxExposure)), enSideVisionModule::SIDE_VISIONMODULE_FRONT);
        ...
        case VISIONTYPE_3D_INSP:
        {
            ...
            SlitIlluminator::GetInstance().Download(
                INSTRUCTION_CHANNEL0_INTENSITY, m_pSlitBeam3DCommon->m_illuminationValue);

            // Sync 보드 설정
            SyncController::GetInstance().SetEncoderScaling(m_pSlitBeam3DCommon->m_scanStep_um);
            SyncController::GetInstance().SetEncoderMultiply(systemConfig.m_slitbeamUseEncoderMultiply);
            SyncController::GetInstance().SetGrabPosition(static_cast<unsigned short>(
                systemConfig.m_slitbeamScanStartOffset_um / m_pSlitBeam3DCommon->m_scanStep_um + 0.5));
            SyncController::GetInstance().SetGrabDirection(CAST_BYTE(systemConfig.m_nGrabDirection),
                SystemConfig::GetInstance().IsDual3DGrabMode()); //kircheis_GrabDir

            // 복합 광학계 추가에 따른 신규 Sync 보드 설정
            SyncController::GetInstance().TurnOffLight(SystemConfig::GetInstance().IsDual3DGrabMode());
            ...
            {
                // Sync 보드 설정
                SyncController::GetInstance().SetInspectionMode(m_pSlitBeam3DCommon->m_cameraGrabMode);

                double illumTime_Specular = m_pSlitBeam3DCommon->m_illuminationTime_ms * 1000.f;
                double illumTime_Defused = m_pSlitBeam3DCommon->m_illuminationTimeDefused_ms * 1000.f;
                SyncController::GetInstance().SetilluminationTime(illumTime_Specular, illumTime_Defused);

                 // 조명 모드 설정
                SyncController::GetInstance().SetilluminationMode(m_pSlitBeam3DCommon->m_cameraGrabMode);
                // 3D 조명 밝기 값 설정
                SyncController::GetInstance().SetilluminationValue(m_pSlitBeam3DCommon->m_illuminationValue);

                for (int grabber_idx = 0; grabber_idx < 2; grabber_idx++)
                {
                    // 스캔 설정에 따라서 이미지 크기가 바뀌게 되므로 얘를 먼저 해 주어야 한다.
                    if (FrameGrabber::GetInstance().set_slitbeam_parameters(
                            grabber_idx, 0, *m_pSlitBeam3DCommon, systemConfig.m_slitbeamHeightScaling, true, true)
                        == false) //kircheis_3DGrabIssue
                    {
                        DevelopmentLog::AddLog(DevelopmentLog::Type::Warning, _T("Camera setting failed"));
                        iPIS_Send_ErrorMessageForLog(_T("Camera Setting"), _T("Camera setting failed"));
                        iPIS_Send_ErrorMessageForPopup(
                            _T("Camera setting failed\nPlease check the illumination settings"));
                    }

                    if (grabber_idx == 0)
                    {
                        FrameGrabber::GetInstance().set_exposure_time(
                            grabber_idx, 0, m_pSlitBeam3DCommon->m_illuminationTime_ms, validFramePeriod_ms);
                    }
                    else if (grabber_idx == 1 && m_pSlitBeam3DCommon->m_cameraGrabMode != VISION_3D_GRABMODE_SPECULAR)
                    {
                        FrameGrabber::GetInstance().set_exposure_time(
                            grabber_idx, 0, m_pSlitBeam3DCommon->m_illuminationTimeDefused_ms, validFramePeriod_ms);
                    }
                }
            }
            else
            {
                // 스캔 설정에 따라서 이미지 크기가 바뀌게 되므로 얘를 먼저 해 주어야 한다.
                if (FrameGrabber::GetInstance().set_slitbeam_parameters(
                        0, 0, *m_pSlitBeam3DCommon, systemConfig.m_slitbeamHeightScaling, true, true)
                    == false) //kircheis_3DGrabIssue
                {
                    DevelopmentLog::AddLog(DevelopmentLog::Type::Warning, _T("Camera setting failed"));
                    iPIS_Send_ErrorMessageForLog(_T("Camera Setting"), _T("Camera setting failed"));
                    iPIS_Send_ErrorMessageForPopup(_T("Camera setting failed\nPlease check the illumination settings"));
                }

                // 노출 시간을 설정한다.
                double validFramePeriod_ms = 0.;
                FrameGrabber::GetInstance().set_exposure_time(
                    0, 0, m_pSlitBeam3DCommon->m_illuminationTime_ms, validFramePeriod_ms);
            }

            // 호스트로 모터 구동 속도를 알려주자
            const double mm_per_sec = min(m_pSlitBeam3DCommon->m_scanStep_um / validFramePeriod_ms, 110.);

            //{{//kircheis_20230308//SharedInfo//Grab Parameter가 수정되면 여기는 무조건 들어오니까 여기에서 공용 변수에 넣는다.
            SystemConfig::GetInstance().SetGrabTimeoutCalcParam(
                (float)mm_per_sec, (float)m_pSlitBeam3DCommon->m_scanLength_mm);
            //}} //Set함수로 한번빼자

            m_pMessageSocket->Write(MSG_PROBE_SLITBEAM_SCAN_SPEED_MM_PER_SEC, sizeof(mm_per_sec), &mm_per_sec);
        }
        break;
		...
		for (long idx = 0; idx < nModuleNum; idx++)
		{
			...
			for (long nFrame = 0; nFrame < nFrameNum; nFrame++)
				vecnFrameID.push_back(m_vecGrabItemFrameInfo[idx].m_vecnFrameID[nFrame]);

			bUseIR = (vecnFrameID[nFrameNum - 1] == 9);
		}

		HardwareSetupForNGRV(vecnFrameID, bUseIR);*/
        }
        break;
        case VISIONTYPE_SIDE_INSP:
        {
            ...
            SyncController::GetInstance().SetTriggerEdgeMode(
                TRUE, TRUE, TRUE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
            SyncController::GetInstance().SetTriggerEdgeMode(
                TRUE, TRUE, TRUE, enSideVisionModule::SIDE_VISIONMODULE_REAR);

            for (long nSideidx = enSideVisionModule::SIDE_VISIONMODULE_START;
                 nSideidx < enSideVisionModule::SIDE_VISIONMODULE_END; nSideidx++)
            {
                SyncController::GetInstance().SetFrameCount(
                    nTotalFrameCount - nHighPosGrabCount, nHighPosGrabCount, enSideVisionModule(nSideidx));
                ...
                {
                    SyncController::GetInstance().SetIllumiParameter(
                        i, m_illum2D.getIllum(i), true, false, FALSE, enSideVisionModule(nSideidx));
                ...
                SyncController::GetInstance().SetFramePeriod(
                    CAST_FLOAT(max(nDuration, nMaxExposure)), enSideVisionModule(nSideidx));
                //}}
            }
        }
        break;
...
BOOL CVisionMain::HardwareSetupForNGRV(std::vector<long> vecnFrameID, BOOL bUseIR, BOOL bInline)
{
    ...
    if (m_isNGRVSingleRun == FALSE)
    {
        long nTotalFrameCount = (bInline ? (long)vecnFrameID.size() : m_illum2D.getTotalFrameCount());
        long nHighPosGrabCount = (bUseIR ? 1 : 0); //kircheis_MultiGrab //추후 Job에 있는 Hig Pos Grab Count를 쓰자
        SyncController::GetInstance().SetFrameCount(
            nTotalFrameCount - nHighPosGrabCount, nHighPosGrabCount, enSideVisionModule::SIDE_VISIONMODULE_FRONT);

        // Set 2D Illumination
        long nRealTotalFrameNum = m_illum2D.getTotalFrameCount();

        for (int i = 0; i < nTotalFrameCount; i++)
        {
            auto illum = bInline ? m_illum2D.getIllum(vecnFrameID[i]) : m_illum2D.getIllum(i);

            if (bInline)
            {
                if (vecnFrameID[i] > nRealTotalFrameNum)
                {
    ...
    {
        //long totalFrameCount = m_illum2D.getTotalFrameCount();
        long totalFrameCount = (long)vecnFrameID.size();
        SyncController::GetInstance().SetFrameCount(totalFrameCount, 0, enSideVisionModule::SIDE_VISIONMODULE_FRONT);

        // Set 2D Illumination
        for (int i = 0; i < totalFrameCount; i++)
        {
            auto illum = m_illum2D.getIllum(vecnFrameID[i]);
            //SyncController::GetInstance().SetIllumiParameter(i, m_illum2D.getIllum(i))
            SyncController::GetInstance().SetIllumiParameter(vecnFrameID[i], m_illum2D.getIllum(vecnFrameID[i]), true,
                false, FALSE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
...
BOOL CVisionMain::OnImagingConditionChanged()
{
    static const long nVisionType = m_systemConfig.GetVisionType();
...
Ipvm::Rect32s CVisionMain::GetReviewRegion(bool isManualInsp, VisionUnit& visionUnit)
{
...
const ImageSaveOptionData& CVisionMain::GetImageOption(bool isManualInsp) const
{
    if (isManualInsp)
    {
...
void CVisionMain::PostExecutionProcessing(bool isManualInsp, VisionUnit& visionUnit, VisionDeviceResult& deviceResult,
    const enSideVisionModule i_eSideVisionModule)
{
    ...
    deviceResult.SetInspectResults(visionUnit.m_threadIndex, nTrayIndex, nInspectionRepeatIndex, pocketID, nScanID,
        ...
        strMajorID, vecStrSub2DID, vecStrBaby2DID, visionWarpageShapeResult,
        vecLidWarpageShapeResult); //kircheis_InspLid
    ...
    DevelopmentLog::AddLog(DevelopmentLog::Type::Thread,
        _T("CVisionMain::PostExecutionProcessing ScanIndex %d, PaneIndex %d Pocket %d, Result %d"), nScanID, paneIndex,
...
void CVisionMain::SaveSystemInfo()
{
...
const InspectionAreaInfo& CVisionMain::getInspectionAreaInfo()
{
    return GetPrimaryVisionUnit().getInspectionAreaInfo();
}

bool CVisionMain::IsShowVisionPrimaryUI() const
{
    return m_pVisionPrimaryUI && m_pVisionPrimaryUI->IsWindowVisible();
}

void CVisionMain::TestSetup()
{
...
void CVisionMain::SetImageSaveOption(ImageSaveOptionData i_save)
{
...
void CVisionMain::SetImageSave(VisionUnit& visionUnit, const ImageSaveOptionData& option,
...
void CVisionMain::InlineImageSave_Review(VisionUnit& visionUnit, const ImageSaveOptionData& option,
...
void CVisionMain::InlineImageSave_Cropping(VisionUnit& visionUnit, const ImageSaveOptionData& option,
...
BOOL CVisionMain::RawImageSaveResult(
...
long CVisionMain::ConvertResult(const long i_nTotalResult)
{
...
void CVisionMain::RawImageSave(VisionUnit& visionUnit, const CString i_strImagePath, CString& o_strSaveRawImageFilePath)
{
    visionUnit.getImageLot().SaveTo(i_strImagePath, o_strSaveRawImageFilePath);
}

void CVisionMain::ResultTextSave(
...
BOOL CVisionMain::ImageSave_Review(VisionUnit& visionUnit, const ImageSaveOptionData& option,
...
BOOL CVisionMain::ImageSave_Review_AllFrame_2D(VisionUnit& visionUnit, const ImageSaveOptionData& option,
...
bool CVisionMain::SaveCombinedImage_2D(VisionUnit& visionUnit, const LPCTSTR moduleName, const LPCTSTR moduleGuid,
    ...
    if (!CString(moduleName).IsEmpty() || !CString(moduleGuid).IsEmpty())
    {
        if (auto* processing = visionUnit.GetVisionProcessingByGuid(moduleGuid))
        {
            ...
            if (visionUnit.GetParaData(moduleName, ImageCombinedPara))
            {
                ...
                Ipvm::ImageFile::SaveAsJpeg(CombineImage, imageFilePath, i_nReviewImageQuality);
            }
        }
    }
...
bool CVisionMain::SaveColorImage_2D(VisionUnit& visionUnit, const long i_nColorFrameidx_Red,
...
void CVisionMain::CroppingImageSave(VisionUnit& visionUnit, const ImageSaveOptionData& option, LPCTSTR moduleGuid,
...
void CVisionMain::CroppingSetImageFrame(VisionUnit& visionUnit, const ImageSaveOptionData& option, LPCTSTR moduleGuid,
...
void CVisionMain::CroppingSetRejectROI(
...
BOOL CVisionMain::InspectionResult(const long i_nThreadNum, const long i_nInspMin, const long i_nInspMax,
...
BOOL CVisionMain::NameToSaveImageforReview(ImageSaveOptionData& option, const long i_nFrameID, const long i_nInspMin,
...
void CVisionMain::NameToSaveImageforCropping(const CString i_strInspName, const CString i_strResultName,
...
void CVisionMain::PathToSaveImage(VisionUnit& visionUnit, const long i_nTrayIndex, const long i_nScanIndex,
...
void CVisionMain::DrawRectangle(Ipvm::Image8u3& image, const Ipvm::Rect32s& roi, COLORREF rgbColor, long inflate)
{
...
BOOL CVisionMain::GetFreeSpaceCheck(const long i_nFreeSpaceMinGB, LPCTSTR checkDiskPath)
{
...
void CVisionMain::CopyImage_From_OtherVisionUnit_Into_PrimaryVisionUnit(long oterhVisionUnitIndex)
{
...
bool CVisionMain::CheckFreeDiskForImageSave(bool sendErrorMessageToHost)
{
...
bool CVisionMain::IsJobTeachEnabled()
{
    return m_pVisionPrimaryUI->IsJobTeachEnabled();
}

bool CVisionMain::IsAllBatchInspectionThreadsIdle()
{
    return m_inline_grab->isIdleForBatch();
}

long CVisionMain::PushBatchImage(long batchKey, long paneIndex, LPCTSTR imageFilePath, LPCTSTR batchBarcode)
{
    if (m_nThreadNum < 1)
        return -2;
...
void CVisionMain::RegisterCallBack_InspectionEnd(void* userData, FUNC_INSPECTION_END_CALLBACK* callback)
{
    m_inline_grab->getInspectionOrderer().registerCallBack_InspectionEnd(userData, callback);
}

void CVisionMain::UnRegisterCallBack_InspectionEnd(FUNC_INSPECTION_END_CALLBACK* callback)
{
    m_inline_grab->getInspectionOrderer().unRegisterCallBack_InspectionEnd(callback);
}

VisionUnit& CVisionMain::GetPrimaryVisionUnit()
{
    return *m_primaryVisionUnit;
}

bool CVisionMain::CheckIntegrityOfCompMapData() //kircheis_Comp3DHeightBug
{
    auto& primaryUnit = GetPrimaryVisionUnit();
...
bool CVisionMain::Init_NGRV_SingleRunInfo()
{
    ...
    for (long nid = 0; nid < nInspectionNum; nid++)
    {
        ...
        if (dynamic_cast<VisionProcessingNGRV*>(proc))
        {
            ...
            {
                const long imageSize_X = ((VisionProcessingNGRV*)proc)->getNGRV_ref_image_size_X(VISION_BTM_2D);
                const long imageSize_Y = ((VisionProcessingNGRV*)proc)->getNGRV_ref_image_size_Y(VISION_BTM_2D);

                const float& scale_X = Get2DImageInfo_NGRV(VISION_BTM_2D).m_scaleX;
                const float& scale_Y = Get2DImageInfo_NGRV(VISION_BTM_2D).m_scaleY;
                VisionScale refimage_scale;
                refimage_scale.setScale(scale_X, scale_Y, 0.f);

                auto& Para_BTM = ((VisionProcessingNGRV*)proc)->m_VisionParaBTM;
                if (Copy_NGRV_SingleRunInfo(VISION_BTM_2D, refimage_scale, Para_BTM->m_defectNumber, imageSize_X,
                        imageSize_Y, Para_BTM->m_vecDefectName, Para_BTM->m_vecItemID, Para_BTM->m_vecDefectCode,
                        Para_BTM->m_vecDisposition, Para_BTM->m_vecDecision, Para_BTM->m_vecStitchCount,
                        Para_BTM->m_vecAFtype, Para_BTM->m_vecGrabHeightOffset_um, Para_BTM->m_vec2IllumUsage,
                        Para_BTM->m_vecGrabPosROI)
                    == false)
                {
                    return false;
                }
            ...
            {
                const long imageSize_X = ((VisionProcessingNGRV*)proc)->getNGRV_ref_image_size_X(VISION_TOP_2D);
                const long imageSize_Y = ((VisionProcessingNGRV*)proc)->getNGRV_ref_image_size_Y(VISION_TOP_2D);

                const float& scale_X = Get2DImageInfo_NGRV(VISION_TOP_2D).m_scaleX;
                const float& scale_Y = Get2DImageInfo_NGRV(VISION_TOP_2D).m_scaleY;
                VisionScale refimage_scale;
                refimage_scale.setScale(scale_X, scale_Y, 0.f);

                auto& Para_TOP = ((VisionProcessingNGRV*)proc)->m_VisionParaTOP;
                if (Copy_NGRV_SingleRunInfo(VISION_TOP_2D, refimage_scale, Para_TOP->m_defectNumber, imageSize_X,
                        imageSize_Y, Para_TOP->m_vecDefectName, Para_TOP->m_vecItemID, Para_TOP->m_vecDefectCode,
                        Para_TOP->m_vecDisposition, Para_TOP->m_vecDecision, Para_TOP->m_vecStitchCount,
                        Para_TOP->m_vecAFtype, Para_TOP->m_vecGrabHeightOffset_um, Para_TOP->m_vec2IllumUsage,
                        Para_TOP->m_vecGrabPosROI)
                    == false)
                {
                    return false;
                }
...
bool CVisionMain::Copy_NGRV_SingleRunInfo(const long& VisionType, const VisionScale& scale, const long& defectNumber,
    const long& imageSizeX, const long& imageSizeY, const std::vector<CString>& vecDefectName,
    ...
    const std::vector<Ipvm::Rect32s>& vecGrabPosROI)
{
    ...
    static const long cameraPixelCountX = FrameGrabber::GetInstance().get_grab_image_width();
    static const long cameraPixelCountY = FrameGrabber::GetInstance().get_grab_image_height();
    long defectImageSizeX = 0;
    long defectImageSizeY = 0;
    ...
    for (long index = 0; index < defectNumber; index++)
    {
        ...
        defectImageSizeX = cameraPixelCountX * vecStitchCount[index].m_x;
        defectImageSizeY = cameraPixelCountY * vecStitchCount[index].m_y;

        // b. Defect Name, Image center position, Z-offset
        onePointGrabInfo[index].m_defectPositionName = vecDefectName[index];
        onePointGrabInfo[index].m_itemID = vecItemID[index];
        onePointGrabInfo[index].m_defectCode = vecDefectCode[index];
        onePointGrabInfo[index].m_disposition = GetDispositionText(vecDisposition[index], true);
        onePointGrabInfo[index].m_decision = GetResultText(vecDecision[index], true, true);
        onePointGrabInfo[index].m_imageSizeX = defectImageSizeX;
        onePointGrabInfo[index].m_imageSizeY = defectImageSizeY;
        onePointGrabInfo[index].m_imageCenterX = defectImageSizeX / 2;
        onePointGrabInfo[index].m_imageCenterY = defectImageSizeY / 2;

        // Stitching X/Y 둘 중 하나라도 1 이상이면 각 ROI의 center와 전체 ROI의 center 좌표를 보내준다.
        if (vecStitchCount[index].m_x > 1 || vecStitchCount[index].m_y > 1)
        {
            long centerX(0), centerY(0);
            long stitchX = vecStitchCount[index].m_x;
            long stitchY = vecStitchCount[index].m_y;

            for (long nY = 0; nY < stitchY; nY++)
            {
                centerY = vecGrabPosROI[index].m_top + (vecGrabPosROI[index].Height() / (stitchY * 2) * (nY * 2 + 1));

                for (long nX = 0; nX < stitchX; nX++)
                {
                    centerX
                        = vecGrabPosROI[index].m_left + (vecGrabPosROI[index].Width() / (stitchX * 2) * (nX * 2 + 1));
        ...
        else // Stitching 안할 경우, 해당 ROI의 center 좌표를 보내준다.
        {
            eachCenterPoints.push_back(
                Ipvm::Point32s2(vecGrabPosROI[index].CenterPoint().m_x, vecGrabPosROI[index].CenterPoint().m_y));
        }

        for (long nSize = 0; nSize < eachCenterPoints.size(); nSize++)
        {
            // Real Image Center Position in pixel
            Ipvm::Point32r2 imageCenterPoint(
                static_cast<float>(imageSizeX * .5f), static_cast<float>(imageSizeY * .5f));
        ...
        onePointGrabInfo[index].m_grabHeightOffset_um = vecGrabHeightOffset_um[index];
        onePointGrabInfo[index].m_afType = vecAFtype[index];
        onePointGrabInfo[index].m_stitchX = vecStitchCount[index].m_x;
        onePointGrabInfo[index].m_stitchY = vecStitchCount[index].m_y;

        long illumSize = (long)vec2IllumUsage[index].size();
        for (long illumIndex = 0; illumIndex < illumSize; illumIndex++)
        {
            onePointGrabInfo[index].m_vecIllumUse.push_back(vec2IllumUsage[index][illumIndex]);
        }
    }
...
CString CVisionMain::GetRecipeFileVersion()
{
    return m_strRecipeFileVersion;
}

void CVisionMain::InlineImageSave_AllFrame(VisionUnit& visionUnit, const ImageSaveOptionData& option,
    std::vector<CString>& o_vecstr2DReviewAll_FrameFilePath, CString& o_str2DReview_AllFrameColorImageFilePath,
...
ImageSaveStatus CVisionMain::CreateImageSave_Directory(VisionUnit& visionUnit, const long i_nTrayIndex,
...
void CVisionMain::SetSampleImageOption(const long i_nSampleImageOption)
{
    m_nPassDeviceSaveCount = i_nSampleImageOption;
}

void CVisionMain::ResetDevicePassCount()
{
    m_nDevicePassCount = 0;
}

bool CVisionMain::GetSend2DAllFrame()
{
...
std::vector<CString> CVisionMain::getAllFrame_2D_FilePath(const bool i_bisRawFilePath)
{
...
void CVisionMain::ResetInlineGrabTime()
{
    m_inline_grab->ResetInline_GrabTimeLOG();
}

void CVisionMain::SaveInlineGrabTime()
{
    m_inline_grab->SaveInline_GrabTimeLOG();
}

void CVisionMain::ConvertDeviceResultToBuffer(BYTE* pbyBuffer, VisionDeviceResult& deviceResult, BOOL bSave)
{
    if (pbyBuffer == NULL)
        return;
...
void CVisionMain::SaveReviewImage_DeubgLOG(VisionUnit& visionUnit, const CString i_strRejectNames,
...
BOOL CVisionMain::ClassifyInspItemInfo(std::vector<CString> vecstrReceivedInspItemName) //kircheis_NGRV
{
...
BOOL CVisionMain::ClassifyInspItemInfo(std::vector<CGrabItemFrameInfoNGRV> vecGrabItemFrameInfoNGRV) //kircheis_NGRV
{
    long nVisionSize = (long)m_vec2InspItemNGRV.size();
    for (long nVision = 0; nVision < nVisionSize; nVision++)
    {
...
void CVisionMain::SetNGRV_SaveOption(CString strInspModuleName, BOOL bSave) //kircheis_NGRV
{
...
BOOL CVisionMain::GetNGRV_SaveOption(CString strInspModuleName) //kircheis_NGRV
{
    long nSize = (long)m_vecGrabItemFrameInfo.size();
...
long CVisionMain::GetVisionID_InspModule(CString strInspModuleName) //kircheis_NGRV
{
...
BOOL CVisionMain::GetFrameInfoNGRV(
...
BOOL CVisionMain::GetFrameInfoNGRV(CString i_strInspModuleName, BOOL& o_bUseIR, BOOL& o_bUseUV,
...
void CVisionMain::GetNGRVFrameInfo(NgrvFrameInfo& o_ngrvFrameInfo) //kircheis_NGRV
{
...
void CVisionMain::GetNGRVGrabPositionInfo(std::vector<NgrvGrabPositionInfo>& o_vecGrabPositionInfo) //kircheis_NGRV
{
...
void CVisionMain::SetReadySignal(BOOL bActive)
{
    SyncController::GetInstance().SetReadySignal(bActive);
}

void CVisionMain::SaveGrabInfoLog(std::vector<NgrvPackageGrabInfo> i_vecPackageGrabInfo, BOOL i_bFlip)
{
...
std::vector<CString> CVisionMain::ExportRecipeToText() //kircheis_TxtRecipe
{
...
std::vector<CString> CVisionMain::ExportRecipe2DIllumToText() //kircheis_TxtRecipe
{
    std::vector<CString> vecStrTotalText(0);
...
std::vector<CString> CVisionMain::ExportRecipe3DIllumToText() //kircheis_TxtRecipe
{
    std::vector<CString> vecStrTotalText(0);
...
void CVisionMain::Send_ManualGrab3D_Sequence_Error_LogMessage(
    const CString i_strCategory, const sGrab3DErrorLogMessageFormat i_sGrab3DErrorForamt)
{
    ...
    iPIS_Send_ErrorMessageForLog(i_strCategory, strSendErrorLogMessage);
}

void CVisionMain::SendToAutomationBatchEnd(long nInspTime) //SDY automation을 위해 Batch inspection end 메세지를 보낸다.
{
    // automation 관련 함수
...
void CVisionMain::SetSideVisionSection(long i_nSideSection)
{
    m_nSideVisionSection = i_nSideSection;
    m_primaryVisionUnit->SetSideVisionSection(i_nSideSection);
}

long CVisionMain::GetSideVisionSection()
{
...
void CVisionMain::SetPackageInfo_NGRV(std::vector<NgrvPackageGrabInfo> packageInfo)
{
    m_primaryVisionUnit->SetPackageInfo_NGRV(packageInfo);
}

std::vector<NgrvPackageGrabInfo>& CVisionMain::GetPackageInfo_NGRV()
{
    return m_primaryVisionUnit->GetPackageInfo_NGRV();
}

void CVisionMain::SetAFInfo_NGRV(NgrvAfRefInfo afInfo)
{
    m_primaryVisionUnit->SetAFInfo_NGRV(afInfo);
}

NgrvAfRefInfo& CVisionMain::GetAFInfo()
{
    return m_primaryVisionUnit->GetAFInfo();
}

void CVisionMain::SetGrabInfo_NGRV(std::vector<NgrvGrabInfo> grabInfo)
{
    m_primaryVisionUnit->SetGrabInfo_NGRV(grabInfo);
}

std::vector<NgrvGrabInfo>& CVisionMain::GetGrabInfo_NGRV()
{
    return m_primaryVisionUnit->GetGrabInfo_NGRV();
}

void CVisionMain::Set2DImageInfo_NGRV(long visionID, Ngrv2DImageInfo imageInfo)
{
    m_primaryVisionUnit->Set2DImageInfo_NGRV(visionID, imageInfo);
}

Ngrv2DImageInfo& CVisionMain::Get2DImageInfo_NGRV(long visionID)
{
    return m_primaryVisionUnit->Get2DImageInfo_NGRV(visionID);
}

void CVisionMain::SetNGRVSingleRunInfo(long visionID, NgrvSingleRunInfo singleRunInfo)
{
    m_primaryVisionUnit->SetNGRVSingleRunInfo(visionID, singleRunInfo);
}

NgrvSingleRunInfo& CVisionMain::GetNGRVSingleRunInfo(long visionID)
{
    return m_primaryVisionUnit->GetNGRVSingleRunInfo(visionID);
}

BOOL CVisionMain::GetFrameInfoNGRV_SingleRun(CString i_strInspModuleName, BOOL& o_bUseIR, BOOL& o_bUseUV,
...
BOOL CVisionMain::GetFrameInfoNGRV_SingleRun(
    CString i_strInspModuleName, BOOL& o_bUseIR, std::vector<long>& o_vecFrameID) //kircheis_NGRV
{
    o_bUseIR = false;
    o_vecFrameID.clear();
...
void CVisionMain::SetSocketInfo(Ipvm::SocketMessaging* pMessageSocket)
{
    m_primaryVisionUnit->SetSocketInfo(pMessageSocket);
}

bool CVisionMain::CheckVisionTypeInRecipe(const BOOL& bSave, CiDataBase& jobDB, CString& errorText)
{
    errorText.Empty();
    static const long currentVisionType = SystemConfig::GetInstance().GetVisionType();
    //Side Vision일 때에는 Side 1 or 2도 확인해야하니까 미리 파악하여 변수에 저장
    static const bool isSideVision = (currentVisionType == VISIONTYPE_SIDE_INSP) ? true : false; 
    ...
    if (bSave == false)
    {
        ...
        {
            if (isHiddenKeyDown == false && isSideVision == true)
            {
                //Side Vision일 때, Recipe의 Vision Type이 Side 1 or 2인지 확인
                if (!mainDB[RECIPE_KEY_VALUE_SIDE_VISION_NUMBER].Link(bSave, inRecipeSideVisionNumber))
                {
                    inRecipeSideVisionNumber = SIDE_VISIONNUMBER_NOSIDEVISION;
                    //Recipe에 Side Vision Number에 대한 정보가 없으면, true 반환
                    return true;
                }
                if (currentSideVisionNumber != inRecipeSideVisionNumber)
                {
                    errorText.Format(_T("The side vision number in the recipe (%d) is different from the current side ")
                                     _T("vision number (%d)."),
                        inRecipeSideVisionNumber+1, currentSideVisionNumber+1);
                    return false;
                }
            }
            return true;
        ...
        else
        {
            ...
            errorText.Format(_T("The vision type in the recipe (%ls) is different from the current vision type (%ls)."),
                (LPCTSTR)inRecipeVisionTypeText, (LPCTSTR)currentVisionTypeText);
            return false;
        }
    }
    else
    {
        ...
        if (isSideVision == true)
        {
            mainDB[RECIPE_KEY_VALUE_SIDE_VISION_NUMBER].Link(bSave, inRecipeSideVisionNumber);
        }
    }
