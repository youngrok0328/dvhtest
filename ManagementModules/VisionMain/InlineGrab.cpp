...
//
InlineGrab::InlineGrab(CVisionMain* main, long grabBufferNum, VisionUnit** unitBuffers, long unitNum)
    : m_main(main)
    ...
    if (SystemConfig::GetInstance().IsHardwareExist())
    {
        switch (SystemConfig::GetInstance().GetVisionType())
        {
            ...
            case VISIONTYPE_3D_INSP:
                ...
                {
                    m_fovImageGrabber = new FOVImageGrabber3D(main, (int)enDualGrabImageType::DUAL_IMAGE_END);
                }
                else
                {
            ...
            case VISIONTYPE_NGRV_INSP:
            {
                if (SystemConfig::GetInstance().IsNGRVInspectionMode() == true)
                    ...
                    m_fovImageGrabber = new FOVImageGrabberNGRV(main);
            }
            break;
...
void InlineGrab::initialize3D(long logicalScanLength_px)
{
    ...
    if (SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        for (int index = 0; index < en3DVisionGrabMode::VISION_3D_GRABMODE_END; index++)
        {
            m_main->GetPrimaryVisionUnit().getImageLot().Setup3D(sensorSizeX, sensorSizeY, m_imageBufferSize, index);
        }
    }
    else
    {
...
void InlineGrab::initializeColorVision(long totalFrameCount, long highPosFrameCount)
{
    ...
    if (m_fovImageGrabber != nullptr)
    {
        (dynamic_cast<FOVImageGrabberNGRV*>(m_fovImageGrabber))->allocateBuffer(totalFrameCount, highPosFrameCount);
    }
