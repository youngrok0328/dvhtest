...
//CPP_6_________________________________ Preprocessor macrosd
#ifdef _DEBUG
...
//
FOVImageGrabber3D::FOVImageGrabber3D(CVisionMain* visionMain, long i_bufferSize)
...
bool FOVImageGrabber3D::buffer_grab(long bufferIndex, const FOVID& fovID, const enSideVisionModule i_eSideVisionModule)
{
    ...
    {
        if (FrameGrabber::GetInstance().StartGrab3D(idx_grabber, idx_camera, 0, buffer->m_imageZmap,
                buffer->m_imageVmap, buffer->m_heightRangeMin, buffer->m_heightRangeMax, progress.get()))
        {
            ...
            if (FrameGrabber::GetInstance().wait_grab_end(idx_grabber, idx_camera, progress.get()))
            {
                        ...
                        idx_grabber, idx_camera, save_slibeam_images, progress.get()))
                {
                    ...
                    sGrab3DErrorLogMessageFormat.m_strGrabberCalbeConnectStatus
                        = siGrab3D_Error_Log.m_strCxpLinkCableError;
                    ...
                    sGrab3DErrorLogMessageFormat.m_fPC_UsagePhysicalMemory
                        = siGrab3D_Error_Log.m_fPC_UsagePhysicalMemory;
            ...
            else
            {
                ...
                DevelopmentLog::AddLog(DevelopmentLog::Type::ImageGrab,
                    _T("Inline Grab3D - Wait Grab Fail (T%d:FOV%d)"), fovID.m_trayIndex, fovID.m_fovIndex);
    ...
    if (bCameraGrabSuccess && bUseGpuCalcRetry) //kircheis_3DCalcRetry
    {
        ...
        if (!bGpuCalcSuccess)
        {
            ...
            if (!FrameGrabber::GetInstance().re_calc_cuda_3D(idx_grabber, idx_camera))
            {
                bGpuCalcSuccess = grab3D_Image_Verification(buffer->m_imageZmap, buffer->m_imageVmap, progress.get());
