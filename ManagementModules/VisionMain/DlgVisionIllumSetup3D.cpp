...
enum
{
    ...
    PROPERTY_GRID_ITEM_GRAB_MODE,
    PROPERTY_GRID_ITEM_GRAB_IMAGEVIEW,
    PROPERTY_GRID_ITEM_EXPOSURE_HIGH_TIME_DEFUSED,
...
DlgVisionIllumSetup3D::DlgVisionIllumSetup3D(CVisionMain& visionMain, CWnd* pParent)
...
BOOL DlgVisionIllumSetup3D::OnInitDialog()
{
    ...
    if (SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        ...
        {
            FrameGrabber::GetInstance().set_live_image_size(grabber_idx, 0, para.m_cameraVerticalBinning, true);
        }
    }
    else
    {
    ...
    FrameGrabber::GetInstance().live_on(
        GetSafeHwnd(), UM_GRAB_END_EVENT, 500L, enSideVisionModule::SIDE_VISIONMODULE_FRONT, para.m_cameraGrabMode);
...
void DlgVisionIllumSetup3D::UpdatePropertyGrid()
{
    ...
    if (SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        if (auto* category = m_propertyGrid->AddCategory(_T("3D Grab Parameters")))
        {
            if (auto* item = category->AddChildItem(new CXTPPropertyGridItemEnum(
                    _T("3D Grab Mode"), para.m_cameraGrabMode, (int*)&para.m_cameraGrabMode)))
            {
                item->GetConstraints()->AddConstraint(_T("Specular"), VISION_3D_GRABMODE_SPECULAR);
                item->GetConstraints()->AddConstraint(_T("Defused"), VISION_3D_GRABMODE_DEFUSED);
                item->GetConstraints()->AddConstraint(_T("Both"), VISION_3D_GRABMODE_INTEGRATED);

                item->SetID(PROPERTY_GRID_ITEM_GRAB_MODE);
            }

            if (auto* item = category->AddChildItem(new CXTPPropertyGridItemEnum(
                    _T("Current Image View"), m_currentCameraView, (int*)&m_currentCameraView)))
            {
                if (para.m_cameraGrabMode == VISION_3D_GRABMODE_SPECULAR)
                {
                    item->GetConstraints()->AddConstraint(_T("Specular"), 0);
                }
                else if (para.m_cameraGrabMode == VISION_3D_GRABMODE_DEFUSED)
                {
                    item->GetConstraints()->AddConstraint(_T("Defused"), 0);
                }
                else if (para.m_cameraGrabMode == VISION_3D_GRABMODE_INTEGRATED)
                {
                    item->GetConstraints()->AddConstraint(_T("Specular"), 0);
                    item->GetConstraints()->AddConstraint(_T("Defused"), 1);
                }

                item->SetID(PROPERTY_GRID_ITEM_GRAB_IMAGEVIEW);
            }
    ...
    {
        if (auto* item
            = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(_T("Illumination time (Specular)"),
        ...
        if (SystemConfig::GetInstance().IsDual3DGrabMode())
        {
            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(
                    _T("Illumination time (Defused)"), para.m_illuminationTimeDefused_ms, _T("%.3lf ms"),
                    para.m_illuminationTimeDefused_ms, 0.016, 200., 0.001)))
            {
                item->SetID(PROPERTY_GRID_ITEM_EXPOSURE_HIGH_TIME_DEFUSED);
            }
        }
    ...
    if (auto* category = m_propertyGrid->AddCategory(_T("Specular Camera Algorithm parameters")))
    {
        if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(
    ...
    if (SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        if (auto* category = m_propertyGrid->AddCategory(_T("Defused Camera1 Algorithm parameters")))
        {
            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(_T("Noise threshold"),
                    para.m_noiseThreshold_Defused1, _T("%.1lf gv"), para.m_noiseThreshold_Defused1, 0.1, 30., 0.1)))
            {
            }

            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(
                    _T("Noise threshold (Final filtering)"), para.m_noiseThreshold_FinalFiltering_Defused1,
                    _T("%.1lf gv"), para.m_noiseThreshold_FinalFiltering_Defused1, 0.1, 30., 0.1))) //kircheis_SDK127
            {
            }

            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomNumber<BYTE>(_T("Slitbeam thickness"),
                    para.m_beamThickness_Defused1_um, _T("%d um"), para.m_beamThickness_Defused1_um, 50, 250)))
            {
            }
        ...
        if (auto* category = m_propertyGrid->AddCategory(_T("Defused Camera2 Algorithm parameters")))
        {
            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(_T("Noise threshold"),
                    para.m_noiseThreshold_Defused2, _T("%.1lf gv"), para.m_noiseThreshold_Defused2, 0.1, 30., 0.1)))
            {
            }

            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomFloat<double>(
                    _T("Noise threshold (Final filtering)"), para.m_noiseThreshold_FinalFiltering_Defused2,
                    _T("%.1lf gv"), para.m_noiseThreshold_FinalFiltering_Defused2, 0.1, 30., 0.1))) //kircheis_SDK127
            {
            }

            if (auto* item = category->AddChildItem(new XTPPropertyGridItemCustomNumber<BYTE>(_T("Slitbeam thickness"),
                    para.m_beamThickness_Defused2_um, _T("%d um"), para.m_beamThickness_Defused2_um, 50, 250)))
            {
            }
...
LRESULT DlgVisionIllumSetup3D::OnGrabEndEvent(WPARAM /*wparam*/, LPARAM /*lparam*/)
{
    ...
    if (para.m_cameraGrabMode == VISION_3D_GRABMODE_SPECULAR || para.m_cameraGrabMode == VISION_3D_GRABMODE_DEFUSED)
    {
    ...
    else if (para.m_cameraGrabMode == VISION_3D_GRABMODE_INTEGRATED)
    {
        ...
        /*
        Ipvm::ImageFile::SaveAsBmp(grabImage, _T("D:\\Temp\\grabImage.bmp"));
        Ipvm::ImageFile::SaveAsBmp(grabImage2, _T("D:\\Temp\\grabImage2.bmp"));
        Ipvm::ImageFile::SaveAsBmp(combineImage, _T("D:\\Temp\\combineImage.bmp"));
        */
...
LRESULT DlgVisionIllumSetup3D::OnGridNotify(WPARAM wparam, LPARAM lparam)
{
    ...
    if (wparam == XTP_PGN_ITEMVALUE_CHANGED)
    {
        ...
        switch (id)
        {
            case PROPERTY_GRID_ITEM_GRAB_MODE:
            {
                if(auto* Value = dynamic_cast<CXTPPropertyGridItemEnum*>(item))
                {
                    ...
                    FrameGrabber::GetInstance().live_on(GetSafeHwnd(), UM_GRAB_END_EVENT, 500L,
                        enSideVisionModule::SIDE_VISIONMODULE_FRONT, para.m_cameraGrabMode);
                }
            }
            break;

            case PROPERTY_GRID_ITEM_GRAB_IMAGEVIEW:
            {
                FrameGrabber::GetInstance().live_off();
                FrameGrabber::GetInstance().live_on(GetSafeHwnd(), UM_GRAB_END_EVENT, 500L,
                    enSideVisionModule::SIDE_VISIONMODULE_FRONT, para.m_cameraGrabMode);
            ...
            case PROPERTY_GRID_ITEM_EXPOSURE_HIGH_TIME_DEFUSED:
            {
                double validFramePeriod_ms = 0.;
                FrameGrabber::GetInstance().set_exposure_time(
                    1, 0, m_visionMain.m_pSlitBeam3DCommon->m_illuminationTimeDefused_ms, validFramePeriod_ms);
            }
            break;
...
void DlgVisionIllumSetup3D::OnDestroy()
{
    ...
    if (SystemConfig::GetInstance().IsDual3DGrabMode())
    {
        ...
        {
            FrameGrabber::GetInstance().set_live_image_size(
                grabber_idx, 0, m_visionMain.m_pSlitBeam3DCommon->m_cameraVerticalBinning, false);
        }
    }
    else
    {
