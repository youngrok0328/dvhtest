...
#include "../SharedCommunicationModules/VisionHostCommon/ImageSaveOption.h" //JHB_NGRV save image format
#include "../SharedCommunicationModules/VisionHostCommon/ImageSaveOptionData.h" //JHB_NGRV save image format
...
//
LRESULT ViewVision::OnConnectionUpdatedFromHost(WPARAM wParam, LPARAM lParam)
{
...
LRESULT ViewVision::OnAccessModeChanged(WPARAM wParam, LPARAM lParam)
{
    m_parent->AccessModeChanged();
    return 0;
}

LRESULT ViewVision::OnDataReceivedFromHost(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (nMessageID)
    {
        ...
        case MSG_PROBE_TRAY_SCAN_SPEC:
            ...
            /* TR에서는 TrayScanSpec을 Host에게 받지 않고 스스로 만들어서 사용한다.
            if (1)
            {
                CMemFile memFile;
                memFile.Attach((byte*)pData, nMessageLength);
        ...
        {
            /* TR에서는 Package Spec 교환을 Host를 통하지 않고 직접 한다.
