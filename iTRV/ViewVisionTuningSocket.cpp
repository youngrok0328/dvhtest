...
//
LRESULT ViewVision::OnTuningSocketConnectionUpdatedFromHost(WPARAM wParam, LPARAM lParam)
{
    ...
    else if (evtInfo.m_disconnected)
    {
        ...
        SyncController::GetInstance().TurnOffLight(enTRVisionmodule::TR_VISIONMODULE_INPOCKET);
        SyncController::GetInstance().TurnOffLight(enTRVisionmodule::TR_VISIONMODULE_OTI);
...
LRESULT ViewVision::OnTuningSocketDataReceivedFromHost(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (nMessageID)
    {
        case MSG_H2V::GRAB_EACH_CHANNEL_2D:
        {
            ...
            switch (data->m_nCurSideVisionidx)
            {
                case enTRVisionmodule::TR_VISIONMODULE_INPOCKET:
                    nIllumChannelMax = LED_ILLUM_CHANNEL_INPOCKET_DEFAULT;
                    break;
                case enTRVisionmodule::TR_VISIONMODULE_OTI:
                    nIllumChannelMax = LED_ILLUM_CHANNEL_OTI_DEFAULT;
                    break;
                default:
                    break;
            }
            ...
            for (long channel = 0; channel < nIllumChannelMax; channel++)
            {
                ...
                SyncController::GetInstance().SetIllumiParameter(0, illuminations_ms,
                    ...
                    enTRVisionmodule(data->m_nCurSideVisionidx));
                ...
                unsigned int width = FrameGrabber::GetInstance().get_grab_image_width(data->m_nCurSideVisionidx, 0);
                unsigned int height = FrameGrabber::GetInstance().get_grab_image_height(data->m_nCurSideVisionidx, 0);

                Ipvm::Image8u image(width, height);
                BYTE* imageMem = image.GetMem();

                grab_data.m_imageSizeX = (long)width;
                grab_data.m_imageSizeY = (long)height;
                ...
                *sendData = grab_data;
                ...
                SyncController::GetInstance().SetFrameCount(1, 0, enTRVisionmodule(data->m_nCurSideVisionidx));
                FrameGrabber::GetInstance().StartGrab2D((int)data->m_nCurSideVisionidx, 0, &imageMem, 1, nullptr);
                SyncController::GetInstance().StartSyncBoard(FALSE, true, enTRVisionmodule(data->m_nCurSideVisionidx));
                FrameGrabber::GetInstance().wait_grab_end((int)data->m_nCurSideVisionidx, 0, nullptr);
        ...
        case MSG_H2V::SET_ILLUMINATION_TABLE_2D:
        {
            ...
            if (file.Open(strResultPath, CFile::modeCreate | CFile::modeWrite))
            {
                ...
                //ackData.m_result = SyncController::GetInstance().CalibrateIllumination2D();
            }
            else
            {
                ackData.m_result = false;
        ...
        case MSG_H2V::SINGLE_GRAB_2D:
        {
            ...
            SyncController::GetInstance().SetIllumiParameter(0, illumValues_ms, data->m_useIlluminationCompensation,
                false, false, enTRVisionmodule(data->m_nCurSideVisionidx));
            SystemConfig::GetInstance().m_bIsCalIllum2D = FALSE;
            ...
            unsigned int width = FrameGrabber::GetInstance().get_grab_image_width((int)data->m_nCurSideVisionidx, 0);
            unsigned int height = FrameGrabber::GetInstance().get_grab_image_height((int)data->m_nCurSideVisionidx, 0);

            Ipvm::Image8u image(width, height);
            BYTE* imageMem = image.GetMem();

            grab_data.m_imageSizeX = width;
            grab_data.m_imageSizeY = height;
            grab_data.m_messageLength = sizeof(MSG_V2H_SINGLE_GRAB_IMAGE) + image.GetWidthBytes() * image.GetSizeY();
        ...
        case MSG_H2V::SINGLE_SCAN_3D:
        {
            ...
            //auto retValue = m_visionMain.ManualGrab3DSequence(imageLot);

            const auto& zmapImage = imageLot.GetZmapImage();
            const auto& vmapImage = imageLot.GetVmapImage();
        ...
        case MSG_H2V::START_LIVE_2D:
        {
            ...
            SyncController::GetInstance().SetIllumiParameter(0, illumValues_ms, data->m_useIlluminationCompensation,
                false, false, enTRVisionmodule(data->m_nCurSideVisionidx));
            SyncController::GetInstance().TurnOnLight(0, enTRVisionmodule(data->m_nCurSideVisionidx));
            FrameGrabber::GetInstance().live_on(
                GetSafeHwnd(), UM_VISION_TUNING_LIVE_2D, 500, enTRVisionmodule(data->m_nCurSideVisionidx));
        }
        break;
        ...
        case MSG_H2V::STOP_LIVE:
        {
            ...
            FrameGrabber::GetInstance().set_live_image_size(
                0, 0, m_visionMain.m_pSlitBeam3DCommon->m_cameraVerticalBinning, false);
            SyncController::GetInstance().TurnOffLight(enTRVisionmodule(data->m_nCurSideVisionidx));
        ...
        case MSG_H2V::SET_2D_CAMERA_OFFSET_Y:
        {
            ...
            FrameGrabber::GetInstance().set_grab_image_offset_Y(
                0, (int)data->m_nCurSideVisionidx, data->m_nCameraOffsetY);
        }
        break;
...
LRESULT ViewVision::OnTuningLive2DEvent(WPARAM wparam, LPARAM lparam)
{
    ...
    if (SystemConfig::GetInstance().GetVisionType() == VISIONTYPE_2D_INSP
        || SystemConfig::GetInstance().GetVisionType() == VISIONTYPE_SIDE_INSP
        || SystemConfig::GetInstance().GetVisionType() == VISIONTYPE_TR) //kircheis_SideVision고려
    {
...
LRESULT ViewVision::OnTuningLive3DEvent(WPARAM wparam, LPARAM lparam)
{
    UNREFERENCED_PARAMETER(wparam);
    UNREFERENCED_PARAMETER(lparam);
