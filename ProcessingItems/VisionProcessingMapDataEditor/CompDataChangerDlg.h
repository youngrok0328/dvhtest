...
{
    CommonItemID_DMS_TYPE_CHIP = 1,
    CommonItemID_DMS_TYPE_PASSIVE,
    CommonItemID_DMS_TYPE_ARRAY,
    CommonItemID_DMS_TYPE_HEATSYNC,
    CommonItemID_DMS_TYPE_PATCH, //kircheis_POI
};
...
enum PropertyGridItemID_HEATSYNC
{
    ...
    HeatSync_ID_LENGTH,
    HeatSync_ID_ANGLE,
    HeatSync_ID_THICKNESS,
    HeatSync_ID_OFFSETX,
    HeatSync_ID_OFFSETY,
};

enum PropertyGridItemID_PATCH
{
    Patch_ID_WIDTH = 1,
    Patch_ID_LENGTH,
    Patch_ID_ANGLE,
...
class CCompDataChangerDlg : public CDialog
{
    DECLARE_DYNAMIC(CCompDataChangerDlg)

public:
    CCompDataChangerDlg(CWnd* pParent /* = nullptr*/, CPackageSpec* i_pPackageSpec, Chip::DMSChip* i_pDmsChipInfo,
        ...
        BOOL i_bIsImportMode, BOOL i_bAddDataMode, BOOL i_bOffsetEdit = FALSE); // 표준 생성자입니다.
    virtual ~CCompDataChangerDlg();
    ...
    enum
    {
        IDD = IDD_DLG_COMPDATA_CHANGER
    };
#endif

public:
    CXTPPropertyGrid* m_propertyGrid;
    CListBox m_ListDMSData;
    ...
    Package::Component m_compSelected;
...
private:
    ...
    void SetPatchDMSGrid(); //kircheis_POI
    ...
    void ModifyDMSData(long i_nChipType, CString i_strChipType);

    void UpdatePassiveType(CString strCompType);
    CString ChangeTheChipType(
