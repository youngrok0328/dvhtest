...
class DlgVisionProcessingNGRV_TOP2D : public CDialog
{
    DECLARE_DYNAMIC(DlgVisionProcessingNGRV_TOP2D)

public:
    DlgVisionProcessingNGRV_TOP2D(
        VisionProcessingNGRV* processor, VisionProcessingNGRVPara& para, CWnd* pParent = nullptr); // 표준 생성자입니다.
    virtual ~DlgVisionProcessingNGRV_TOP2D();

    // 대화 상자 데이터입니다.
    enum
    {
        IDD = IDD_DLG_NGRV_TOP_TAB
    };
...
public:
    ...
    afx_msg void OnItemButtonClickTOP(NMHDR* pNotifyStruct, LRESULT*);
    afx_msg void OnDefectNameChangedTOP(NMHDR* pNotifyStruct, LRESULT*);
    afx_msg LRESULT OnGrabEndEvent(WPARAM wparam, LPARAM lparam);
    afx_msg LRESULT OnGridNotify(WPARAM wParam, LPARAM lParam);
    afx_msg void OnBnClickedButtonChangeFrameBack();
    afx_msg void OnBnClickedButtonChangeFrameFoward();
    afx_msg void OnBnClickedButtonLiveTop();
    afx_msg void OnBnClickedButtonGrabTop();
    afx_msg void OnItemClickTop(NMHDR* pNotifyStruct, LRESULT* result);
