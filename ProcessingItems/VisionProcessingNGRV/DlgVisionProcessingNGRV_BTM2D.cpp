...
#include "../../SharedCommunicationModules/VisionHostCommon/NgrvOnePointGrabInfo.h"
...
{
    COLUMN_START = 0,
    COLUMN_DEFECT_NAME_BTM = COLUMN_START,
    COLUMN_ITEM_SETUP_BTM,
    COLUMN_MOVE_POS_BTM,
    COLUMN_END,
};
...
IMPLEMENT_DYNAMIC(DlgVisionProcessingNGRV_BTM2D, CDialog)

DlgVisionProcessingNGRV_BTM2D::DlgVisionProcessingNGRV_BTM2D(
    VisionProcessingNGRV* processor, VisionProcessingNGRVPara& para, CWnd* pParent /*=nullptr*/)
    : CDialog(IDD_DLG_NGRV_BTM_TAB, pParent)
...
void DlgVisionProcessingNGRV_BTM2D::DoDataExchange(CDataExchange* pDX)
{
    CDialog::DoDataExchange(pDX);
    DDX_Control(pDX, IDC_LIST_NGRV_BTM2D, m_parameterList);
    DDX_Control(pDX, IDC_BUTTON_CHANGE_FRAME_BW, m_frameBW);
    DDX_Control(pDX, IDC_BUTTON_CHANGE_FRAME_FW, m_frameFW);
    DDX_Control(pDX, IDC_EDIT_FRAME_NUM, m_frameNum);
    DDX_Control(pDX, IDC_BUTTON_LIVE, m_buttonLive);
    DDX_Control(pDX, IDC_BUTTON_GRAB, m_buttonGrab);
}

BEGIN_MESSAGE_MAP(DlgVisionProcessingNGRV_BTM2D, CDialog)
ON_WM_CREATE()
ON_WM_DESTROY()
ON_MESSAGE(UM_GRAB_END_EVENT, &DlgVisionProcessingNGRV_BTM2D::OnGrabEndEvent)
ON_MESSAGE(XTPWM_PROPERTYGRID_NOTIFY, OnGridNotify)
ON_NOTIFY(NM_CLICK, IDC_LIST_NGRV_BTM2D, &DlgVisionProcessingNGRV_BTM2D::OnItemClickBTM)
ON_NOTIFY(XTP_NM_GRID_ITEMBUTTONCLICK, IDC_LIST_NGRV_BTM2D, &DlgVisionProcessingNGRV_BTM2D::OnItemButtonClickBTM)
ON_NOTIFY(XTP_NM_GRID_VALUECHANGED, IDC_LIST_NGRV_BTM2D, &DlgVisionProcessingNGRV_BTM2D::OnDefectNameChangedBTM)
ON_BN_CLICKED(IDC_BUTTON_CHANGE_FRAME_BW, &DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonChangeFrameBw)
ON_BN_CLICKED(IDC_BUTTON_CHANGE_FRAME_FW, &DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonChangeFrameFw)
ON_BN_CLICKED(IDC_BUTTON_LIVE, &DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonLive)
ON_BN_CLICKED(IDC_BUTTON_GRAB, &DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonGrab)
END_MESSAGE_MAP()

// DlgVisionProcessingNGRV_BTM2D 메시지 처리기

BOOL DlgVisionProcessingNGRV_BTM2D::OnInitDialog()
{
...
int DlgVisionProcessingNGRV_BTM2D::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void DlgVisionProcessingNGRV_BTM2D::OnDestroy()
{
    CDialog::OnDestroy();
...
void DlgVisionProcessingNGRV_BTM2D::OnInitGrid()
{
    m_grid->ResetContent();

    if (auto* category = m_grid->AddCategory(_T("Item Control Parameter")))
    {
        category->AddChildItem(new CCustomItemButton(_T("Add Item"), TRUE, FALSE))->SetID(ITEM_ADD_BTM);
        category->AddChildItem(new CCustomItemButton(_T("Delete Item"), TRUE, FALSE))->SetID(ITEM_DELETE_BTM);
        category->AddChildItem(new CCustomItemButton(_T("Apply"), TRUE, FALSE))->SetID(ITEM_APPLY_BTM);

        m_grid->FindItem(ITEM_ADD_BTM)->SetHeight(50);
        m_grid->FindItem(ITEM_DELETE_BTM)->SetHeight(50);
        m_grid->FindItem(ITEM_APPLY_BTM)->SetHeight(50);
...
void DlgVisionProcessingNGRV_BTM2D::OnInitList()
{
    ...
    m_parameterList.AddColumn(new CXTPGridColumn(COLUMN_DEFECT_NAME_BTM, _T("Defect Name"), length * 2));
    m_parameterList.AddColumn(new CXTPGridColumn(COLUMN_ITEM_SETUP_BTM, _T("Item Setup"), length));
    m_parameterList.AddColumn(new CXTPGridColumn(COLUMN_MOVE_POS_BTM, _T("Move Position"), length));
...
void DlgVisionProcessingNGRV_BTM2D::OnInitImage()
{
...
void DlgVisionProcessingNGRV_BTM2D::ResetListControl()
{
...
void DlgVisionProcessingNGRV_BTM2D::AddItem()
{
    ...
    m_para.m_vecItemID.resize(newNum);
    m_para.m_vecDefectCode.resize(newNum);
    m_para.m_vecDisposition.resize(newNum);
    m_para.m_vecDecision.resize(newNum);
    ...
    for (long index = oldNum; index < newNum; index++)
    {
        ...
        m_para.m_vecDefectCode[index] = _T("");
        m_para.m_vecDisposition[index] = DISPOSITION_NULL; // Default to PASS
        m_para.m_vecDecision[index] = NOT_MEASURED; // Default to Measured
...
void DlgVisionProcessingNGRV_BTM2D::DeleteItem()
{
...
void DlgVisionProcessingNGRV_BTM2D::ApplyItem()
{
    ...
    m_processor->m_visionUnit.SetNGRVSingleRunInfo(VISION_BTM_2D, singleRunInfo);
}

LRESULT DlgVisionProcessingNGRV_BTM2D::OnGridNotify(WPARAM wParam, LPARAM lParam)
{
...
void DlgVisionProcessingNGRV_BTM2D::OnItemButtonClickBTM(NMHDR* pNotifyStruct, LRESULT*)
{
    ...
    if (pItemNotify->pItemControl->GetCaption() == _T("Setup"))
    {
        ...
        DlgVisionProcessingNGRV_DetailSetup dlg(m_processor, selectedDefectID, TRUE,
...
void DlgVisionProcessingNGRV_BTM2D::OnDefectNameChangedBTM(NMHDR* pNotifyStruct, LRESULT*)
{
...
bool DlgVisionProcessingNGRV_BTM2D::IsRoiSizeOverThanImageSize()
{
    const auto& scale = m_processor->getScale();
...
bool DlgVisionProcessingNGRV_BTM2D::SetROI()
{
...
void DlgVisionProcessingNGRV_BTM2D::ChangeROItitle()
{
    auto& imageLotView = *m_processor->m_pVisionInspDlg->m_imageLotView;
...
void DlgVisionProcessingNGRV_BTM2D::SetDefectROIinfo()
{
    if (m_para.m_defectNumber < 0)
    {
        return;
...
bool DlgVisionProcessingNGRV_BTM2D::GetROI()
{
    if (m_para.m_defectNumber < 0)
    {
        return FALSE;
...
/// Live 관련 함수
bool DlgVisionProcessingNGRV_BTM2D::StartLive(long frameNum)
{
    ...
    if (SystemConfig::GetInstance().IsHardwareExist() == TRUE)
    {
        SyncController::GetInstance().SetIllumiParameter(
            frameNum, illum.getIllum(frameNum), true, false, FALSE, enSideVisionModule::SIDE_VISIONMODULE_FRONT);
...
bool DlgVisionProcessingNGRV_BTM2D::StartGrab(long defectNum)
{
...
LRESULT DlgVisionProcessingNGRV_BTM2D::OnGrabEndEvent(WPARAM /*wparam*/, LPARAM /*lparam*/)
{
...
void DlgVisionProcessingNGRV_BTM2D::ShowPreviousImage()
{
...
void DlgVisionProcessingNGRV_BTM2D::ShowNextImage()
{
    // Image Display 를 위한 테스트 코드
    if (m_processor->getImageLotInsp().m_vecColorImages.size() == 0)
        return;
...
void DlgVisionProcessingNGRV_BTM2D::ShowImage(BOOL change)
{
...
void DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonChangeFrameBw()
{
    ShowPreviousImage();

    CString temp("");
    temp.Format(_T("frame_%d"), m_imageID);
    SetDlgItemText(IDC_EDIT_FRAME_NUM, temp);

    temp.Empty();
}

void DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonChangeFrameFw()
{
    ShowNextImage();
...
void DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonLive()
{
    if (m_isLive == FALSE)
    {
        StartLive(m_imageID);
...
void DlgVisionProcessingNGRV_BTM2D::OnBnClickedButtonGrab()
{
    if (m_isLive == TRUE)
    {
        FrameGrabber::GetInstance().live_off();
...
void DlgVisionProcessingNGRV_BTM2D::OnItemClickBTM(NMHDR* pNotifyStruct, LRESULT* result)
{
...
BOOL DlgVisionProcessingNGRV_BTM2D::SetupToGrab(std::vector<long> frameID, BOOL isUseIR, BOOL isInline)
{
...
void DlgVisionProcessingNGRV_BTM2D::iPIS_Send_SingleRun_MoveTo()
{
    ...
    auto& grabInfo = m_processor->m_visionUnit.GetNGRVSingleRunInfo(VISION_BTM_2D);
