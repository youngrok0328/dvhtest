...
#include "XTPPropertyGridItemCustomItems.h"
...
{
    const TCHAR szFilters[] = _T("Icon files (*.ico)|*.ico|All Files (*.*)|*.*||");
...
{
public:
    CCustomItemChildsAll(CString strCaption)
        : CXTPPropertyGridItemNumber(strCaption)
    {
    }
    virtual void OnValueChanged(CString strValue)
    {
        ...
        CCustomItemChilds* pParentWnd = ((CCustomItemChilds*)m_pParent);
        CRect& rc = pParentWnd->m_rcValue;
        rc.left = rc.right = rc.top = rc.bottom = GetNumber();
        pParentWnd->OnValueChanged(pParentWnd->RectToString(rc));
    }
};
...
{
public:
    CCustomItemChildsPad(CString strCaption, LONG& nPad)
        : CXTPPropertyGridItemNumber(strCaption)
        , m_nPad(nPad)
    {
    }
    virtual void OnValueChanged(CString strValue)
    {
        ...
        CCustomItemChilds* pParentWnd = ((CCustomItemChilds*)m_pParent);
        m_nPad = GetNumber();
        pParentWnd->m_itemAll->SetNumber(0);
        pParentWnd->OnValueChanged(pParentWnd->RectToString(pParentWnd->m_rcValue));
    }
    LONG& m_nPad;
};
...
void CCustomItemColor::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* /*pButton*/)
{
    ...
    pColorPopup->Create(rcItem, m_pGrid,
        CPS_XTP_RIGHTALIGN | CPS_XTP_USERCOLORS | CPS_XTP_EXTENDED | CPS_XTP_MORECOLORS | CPS_XTP_SHOW3DSELECTION
...
CCustomItemFileBox::CCustomItemFileBox(CString strCaption)
    : CXTPPropertyGridItem(strCaption)
{
    m_nFlags = xtpPropertyGridItemHasExpandButton | xtpPropertyGridItemHasEdit;
}
...
CCustomItemButton::CCustomItemButton(CString strCaption, BOOL bFullRowButton, BOOL bValue)
    : CXTPPropertyGridItem(bFullRowButton ? _T("") : (LPCTSTR)strCaption)
{
...
void CCustomItemButton::CreateButton()
{
    if (IsVisible())
    {
        ...
        m_wndButton.EnableWindow(!GetReadOnly());
...
void CCustomItemMenu::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* pButton)
{
    ...
    UINT nCmd
        = CXTPCommandBars::TrackPopupMenu(&menu, TPM_RETURNCMD | TPM_NONOTIFY, rc.right, rc.top, pButton->GetGrid(), 0);
#else
    UINT nCmd = menu.TrackPopupMenu(TPM_RETURNCMD, rc.right, rc.top, pButton->GetGrid(), 0);
#endif
