...
#define DPI_PACKAGESPEC_API __declspec(dllexport)
#else
#define DPI_PACKAGESPEC_API __declspec(dllimport)
#endif
...
#if defined(MARKUP_SAFESTR) // VC++ safe strings
#define MCD_SSZ(sz) sz,(sizeof(sz)/sizeof(MCD_CHAR))
#define MCD_PSZCPY(sz,p) _tcscpy_s(MCD_SSZ(sz),p)
#define MCD_PSZNCPY(sz,p,n) strncpy_s(MCD_SSZ(sz),p,n)
#define MCD_SPRINTF _stprintf_s
...
// afx.h provides CString, to avoid "WINVER not defined" #include stdafh.m_x in Markup.cpp
#include <afx.h>
...
class DPI_PACKAGESPEC_API CMarkup
{
public:
