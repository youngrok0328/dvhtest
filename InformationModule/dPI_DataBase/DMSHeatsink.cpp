...
namespace Chip
{
BOOL DMSHeatsink::LinkDataBase(BOOL bSave, CiDataBase& db, long nCount)
{
    long nSize(0);
    CString strTemp;

    strTemp.Format(_T("DMSHeatsink_%d_strName"), nCount);
    if (!db[strTemp].Link(bSave, strCompType))
        strCompType = _T("Default");

    if (!bSave)
    {
        strTemp.Format(_T("DMSHeatsink_frtPad_%d_Num"), nCount);
    ...
    {
        strTemp.Format(_T("DMSHeatsink_frtPad_%d_%d_left"), nCount, i);
        if (!db[strTemp].Link(bSave, vecfrtPad[i].m_left))
            vecfrtPad[i].m_left = 0.f;
        strTemp.Format(_T("DMSHeatsink_frtPad_%d_%d_top"), nCount, i);
        if (!db[strTemp].Link(bSave, vecfrtPad[i].m_top))
            vecfrtPad[i].m_top = 0.f;
        strTemp.Format(_T("DMSHeatsink_frtPad_%d_%d_right"), nCount, i);
        if (!db[strTemp].Link(bSave, vecfrtPad[i].m_right))
            vecfrtPad[i].m_right = 0.f;
        strTemp.Format(_T("DMSHeatsink_frtPad_%d_%d_bottom"), nCount, i);
        if (!db[strTemp].Link(bSave, vecfrtPad[i].m_bottom))
            vecfrtPad[i].m_bottom = 0.f;
    }

    strTemp.Format(_T("DMSHeatsink_frtChip_%d_left"), nCount);
    if (!db[strTemp].Link(bSave, frtChip.m_left))
        frtChip.m_left = 0.f;
    strTemp.Format(_T("DMSHeatsink_frtChip_%d_top"), nCount);
    if (!db[strTemp].Link(bSave, frtChip.m_top))
        frtChip.m_top = 0.f;
    strTemp.Format(_T("DMSHeatsink_frtChip_%d_right"), nCount);
    if (!db[strTemp].Link(bSave, frtChip.m_right))
        frtChip.m_right = 0.f;
    strTemp.Format(_T("DMSHeatsink_frtChip_%d_bottom"), nCount);
    if (!db[strTemp].Link(bSave, frtChip.m_bottom))
        frtChip.m_bottom = 0.f;

    if (!bSave)
    {
        strTemp.Format(_T("DMSHeatsink_sfrtPad_%d_Num"), nCount);
        if (!db[strTemp].Link(bSave, nSize))
            nSize = 0;
